...
def build_workout_payload(
    sport: str, title: str, steps: list[dict[str, Any]], note: str | None = None
) -> dict[str, Any]:
    """Our step tree → Garmin's workout DTO.

    ``note`` becomes the workout-level ``description`` (spec 082), which is a
    different field from the per-step ``description`` set in :func:`_step_dto` —
    neither touches the other. Empty or blank omits the key rather than sending
    ``""``, so a workout with no note looks to Garmin exactly as it did before.
    ...
    if isinstance(note, str) and note.strip():
        payload["description"] = note.strip()
    return payload
...
def create_workout(
    ...
    payload = build_workout_payload(sport, title, steps, note)
...
def schedule_workout(api: Any, workout_id: str, day: _dt.date) -> dict[str, Any]:
    ...
    A 404 here (spec 092) is reported as ``reason: "workout_not_found"``, NOT
    the generic ``unsupported_endpoint`` :func:`create_workout` uses for the
    same sentinel: by the time this is called with an existing id, a
    ``create_workout`` for this account has already succeeded at least once,
    so a 404 on the *schedule* path is evidence about *this workout id* (it
    was deleted upstream, most likely) rather than "this account cannot
    schedule workouts at all". The web tier uses this to self-heal — clear the
    stale id and push a fresh create+schedule — instead of parking the row as
    permanently unsupported.
    """
    path = _SCHEDULE_PATH.format(workout_id=workout_id)
    result = _write(api, path, "POST", {"date": day.isoformat()})
    if result is _UNSUPPORTED:
        return {"supported": False, "reason": "workout_not_found"}
