...
def test_schedule_against_a_missing_workout_reports_workout_not_found(
    client: TestClient,
) -> None:
    """spec 092: a 404 on the schedule path is evidence about THIS id, not the

    account, so it gets a distinct reason from create's ``unsupported_endpoint``
    — the web tier uses it to self-heal (clear the stale id, push fresh).
    """
    _login(client)
    FakeAPIClient.write_behavior = "not_found"
    ...
    assert resp.status_code == 200
    assert resp.json() == {"supported": False, "reason": "workout_not_found"}


def test_create_against_a_missing_endpoint_still_reports_unsupported_endpoint(
    client: TestClient,
) -> None:
    """Regression guard: create's 404 handling is unchanged by spec 092 — only

    schedule's distinguishes "this workout id is gone" from "no endpoint at
...
# --- workout-level description (spec 082) -------------------------------------

def test_note_becomes_the_workout_level_description(client: TestClient) -> None:
    _login(client)
    body = {**_run_intervals(), "note": "Easy Z2 — HR <=143.\n\nvia OpenVitals"}

    resp = client.post("/workouts", json=body, headers=headers(USER_A))

    assert resp.status_code == 200
    assert _last_payload()["description"] == "Easy Z2 — HR <=143.\n\nvia OpenVitals"


def test_a_blank_note_omits_the_description_entirely(client: TestClient) -> None:
    """Absent, null and whitespace must all look to Garmin like the pre-082 payload."""
    _login(client)

    for note in (None, "", "   \n  "):
        resp = client.post(
            "/workouts", json={**_run_intervals(), "note": note}, headers=headers(USER_A)
        )
        assert resp.status_code == 200
        assert "description" not in _last_payload()

    resp = client.post("/workouts", json=_run_intervals(), headers=headers(USER_A))
    assert resp.status_code == 200
    assert "description" not in _last_payload()


def test_the_workout_description_does_not_disturb_step_descriptions(
    client: TestClient,
) -> None:
    """Two different fields at two different levels; neither overwrites the other."""
    _login(client)
    session = _run_intervals()
    session["steps"][0]["note"] = "Spokojnie, na luzie"

    resp = client.post(
        "/workouts", json={**session, "note": "Cel: objętość"}, headers=headers(USER_A)
    )

    assert resp.status_code == 200
    payload = _last_payload()
    assert payload["description"] == "Cel: objętość"
    assert payload["workoutSegments"][0]["workoutSteps"][0]["description"] == (
        "Spokojnie, na luzie"
    )


def test_an_over_long_note_is_refused_by_the_request_model(client: TestClient) -> None:
    """422 from the bound, not a malformed call to Garmin."""
    _login(client)

    resp = client.post(
        "/workouts",
        json={**_run_intervals(), "note": "x" * 1025},
...
def test_the_log_never_carries_the_note(client: TestClient, caplog: Any) -> None:
    _login(client)

    with caplog.at_level("INFO"):
        client.post(
            "/workouts",
            json={**_run_intervals(), "note": "kolano boli od wtorku"},
            headers=headers(USER_A),
        )

    text = "\n".join(record.getMessage() for record in caplog.records)
    assert "kolano" not in text
