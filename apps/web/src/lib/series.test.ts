...
import { bucketNounKey, bucketSeries } from './series';
...
describe('bucketNounKey', () => {
  it('names what one point means, through the catalog rather than a literal', () => {
    // Asserting via `t()` keeps this a test of the mapping, not of one language's wording.
    expect(t(bucketNounKey('day'))).toBe('dzień');
    expect(t(bucketNounKey('week'))).toBe('tydzień');
    expect(t(bucketNounKey('month'))).toBe('miesiąc');
  });
});
