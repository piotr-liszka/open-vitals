...
describe('TrendChart', () => {
  ...
  it('labels every tick a sparse x axis carries', () => {
    ...
    // 13 non-blank labels exist, but the last one (day 360, not the very last day-365 slot) sits
    // close enough to the right edge to anchor "end" rather than "middle" — so its true (leftward)
    // footprint is counted, and the neighbour it would otherwise have touched is thinned out.
    expect(container.querySelectorAll('text.axis-tick.x')).toHaveLength(12);
  });
