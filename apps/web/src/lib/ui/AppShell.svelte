<script lang="ts">
  ...
  interface Props {
     ...
     * the identical `<NavLinks />`, so the shell now renders the primary nav itself, and only the
     * styleguide — which showcases the shell with fake links — overrides it (spec 063).
     */
    nav?: Snippet;
    /**
  ...
  let { title, nav, actions, brand, footer, range = 'auto', children }: Props = $props();
  ...
  const buildTitle = $derived(
    __BUILD_SHA__
      ? i18n.t('shell.builtAtCommit', { at: buildStamp, sha: __BUILD_SHA__ })
      : i18n.t('shell.builtAt', { at: buildStamp })
  );
...
<div class="shell" class:mobile-open={mobileOpen}>
  <aside
    id="app-sidebar"
    class="sidebar"
    aria-label={i18n.t('shell.sidebarLabel')}
  ...
  >
    <div class="brand">
      ...
      {:else}
        <span class="brand-lockup">
          <span class="brand-text">{i18n.t('shell.brand')}</span>
        </span>
      {/if}
    </div>
    <nav class="nav" bind:this={navEl}>
      {#if nav}{@render nav()}{:else}<NavLinks />{/if}
    </nav>
    ...
    <div class="build" title={buildTitle}>
      <span class="build-label">{i18n.t('shell.version')}</span>
  ...
  {#if mobileOpen}
    <button type="button" class="scrim" aria-label={i18n.t('shell.closeMenu')} onclick={closeMobile}></button>
  {/if}

  <div class="frame">
    <header class="topbar">
      <!--
        `menu-btn` + the hidden-sidebar toggle + the title share ONE nowrap flex row (spec 034/047
        follow-up): title shrinks and ellipsizes here rather than the whole cluster wrapping, which
        used to strand `.topbar-actions` on an accidental third line on phones whenever the title was
        longer than a couple of words (e.g. "Analysis · Running") — the exact "navbar eats the
        screen" complaint. Isolating the always-shrinkable title in its own flex context makes the
        shrink-before-wrap behaviour reliable instead of depending on every sibling's width.
      -->
      <div class="topbar-row">
        <button
          type="button"
          class="menu-btn"
          aria-label={mobileOpen ? i18n.t('shell.closeMenu') : i18n.t('shell.openMenu')}
...
<style>
  ...
  .brand {
    ...
    flex-shrink: 0;
    /* The row is exactly one topbar tall: a wordmark that wrapped used to spill out of it
       (spec 034). Nothing here may grow the row. */
    overflow: hidden;
  }
  ...
  */
  ...
  /* Menu button + (optional) sidebar-recovery toggle + title: never wraps internally, so the title
     is the only thing that ever shrinks here (see the markup comment above `.topbar-row`). */
  .topbar-row {
    display: flex;
    align-items: center;
    gap: var(--space-3);
    min-width: 0;
    /*
      `flex-basis: 0`, not `auto`: flex-wrap assigns items to lines using each item's UNSHRUNK
      hypothetical size, which for `auto` is this row's full content width (menu button + the
      whole title text) — wide enough, on its own, to push `.topbar-actions` onto a wrapped third
      row before shrinking ever gets a say. A `0` basis makes this row hypothetically tiny so it
      shares line 1 with `.topbar-actions`; `flex-grow: 1` then hands it whatever space is left,
      and the title (its own `flex`/`min-width: 0`/ellipsis below) shrinks to fit that space.
    */
    flex: 1 1 0%;
  }
