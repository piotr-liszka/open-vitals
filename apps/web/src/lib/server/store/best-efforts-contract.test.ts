...
export function bestEffortsContract(name: string, make: () => Promise<Fixture>): void {
  describe(`${name} — best-efforts contract`, () => {
    ...
    describe('the record progression (spec 087)', () => {
      /** Seed `[activityId, day, durationS]` triples at one distance key. */
      async function seed(
        store: LocalStore,
        userId: string,
        ...
        for (const [id, day, durationS] of rows) {
          await store.putActivities(userId, [activity(userId, id, { sport })]);
          await store.putStreams(userId, id, { speed: [3, 3], time: [0, 1] });
          await store.putActivityBestEfforts(userId, {
            activityId: id,
            sport,
            day,
            version: BEST_EFFORTS_VERSION,
            efforts: [effort({ key, metres, durationS })]
          });
        }
      }

      it('keeps only the efforts that were a record when they were set', async () => {
        const { store, userId } = await make();
        await seed(store, userId, [
          ['r1', '2026-01-01', 1600], // first at this distance — always a record
          ['r2', '2026-02-01', 1700], // slower than the standing record: not a record
          ['r3', '2026-03-01', 1550], // beats it
          ['r4', '2026-04-01', 1560], // slower again
          ['r5', '2026-05-01', 1500] // beats it
        ]);

        const rows = await store.listBestEffortProgression(userId);
        expect(rows.map((r) => r.activityId)).toEqual(['r1', 'r3', 'r5']);
        // Chronological, and each row carries the same shape the leaderboard returns.
        expect(rows.map((r) => r.durationS)).toEqual([1600, 1550, 1500]);
        expect(rows[0]).toMatchObject({ key: '5k', metres: 5000, sport: 'running', day: '2026-01-01' });
      });

      it('does not count an EQUAL time as a new record', async () => {
        const { store, userId } = await make();
        await seed(store, userId, [
          ['first', '2026-01-01', 1500],
          ['equal', '2026-06-01', 1500],
          ['faster', '2026-07-01', 1499]
        ]);
        // The record belongs to whoever set it first, exactly as `listTopBestEfforts` breaks its ties.
        expect((await store.listBestEffortProgression(userId)).map((r) => r.activityId)).toEqual([
          'first',
          'faster'
        ]);
      });

      it('breaks a same-day tie by activity id, deterministically', async () => {
        const { store, userId } = await make();
        await seed(store, userId, [
          ['b', '2026-01-01', 1500],
          ['a', '2026-01-01', 1600]
        ]);
        // Ordered `a` then `b` inside the day, so the slower `a` is the first record and `b` beats it.
        expect((await store.listBestEffortProgression(userId)).map((r) => r.activityId)).toEqual(['a', 'b']);
      });

      it('is the effort itself when a distance has exactly one', async () => {
        const { store, userId } = await make();
        await seed(store, userId, [['only', '2026-02-02', 1450]]);
        const rows = await store.listBestEffortProgression(userId);
        expect(rows).toHaveLength(1);
        expect(rows[0]).toMatchObject({ activityId: 'only', durationS: 1450 });
      });

      it('tracks each distance key independently, ordered by distance then day', async () => {
        const { store, userId } = await make();
        await seed(store, userId, [['long1', '2026-01-05', 3400]], '10k', 10_000);
        await seed(store, userId, [
          ['short1', '2026-01-01', 1600],
          ['short2', '2026-03-01', 1500]
        ]);
        await seed(store, userId, [['long2', '2026-02-05', 3600]], '10k', 10_000);

        const rows = await store.listBestEffortProgression(userId);
        // 5 km before 10 km; inside a distance, chronological. `long2` was slower, so it is not here.
        expect(rows.map((r) => r.activityId)).toEqual(['short1', 'short2', 'long1']);
      });

      it('filters by sport family', async () => {
        const { store, userId } = await make();
        await seed(store, userId, [['run1', '2026-01-01', 1500]]);
        await seed(store, userId, [['walk1', '2026-01-02', 3000]], '5k', 5000, 'walking');

        const runs = await store.listBestEffortProgression(userId, { sports: sportKeysInGroup('run') });
        expect(runs.map((r) => r.activityId)).toEqual(['run1']);
        // The filter runs BEFORE the progression, like the pg WHERE clause: unfiltered, the two
        // sports share one partition, so the slower walk never beat the standing run record and is
        // not a record at all. Filtering is what makes each family's own progression its own.
        expect((await store.listBestEffortProgression(userId)).map((r) => r.activityId)).toEqual(['run1']);
        expect(
          (await store.listBestEffortProgression(userId, { sports: ['walking'] })).map((r) => r.activityId)
        ).toEqual(['walk1']);
      });

      it('is empty for a user with no efforts at all', async () => {
        const { store, userId } = await make();
        expect(await store.listBestEffortProgression(userId)).toEqual([]);
        expect(await store.listBestEffortProgression(userId, { sports: ['running'] })).toEqual([]);
      });

      it('never lets one user’s progression include another’s (AGENTS.md §2 rule 2)', async () => {
        const { store, userId, otherUserId } = await make();
        await seed(store, userId, [['mine', '2026-01-01', 1800]]);
        // Dramatically faster, and must not suppress the other account's own first record.
        await seed(store, otherUserId, [['theirs', '2026-02-01', 900]]);

        expect((await store.listBestEffortProgression(userId)).map((r) => r.activityId)).toEqual(['mine']);
        expect((await store.listBestEffortProgression(otherUserId)).map((r) => r.activityId)).toEqual([
          'theirs'
        ]);
      });
    });
