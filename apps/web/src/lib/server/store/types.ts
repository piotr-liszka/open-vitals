...
export interface ActivitySummary {
  ...
  /**
   * Garmin's own `workoutId` on the activity (spec 081) — the SCHEDULED workout this session was
   * started from, as a string (Garmin types it as a number in some payload shapes, and the column
   * it joins against, `authored_workouts.garmin_workout_id`, is text). Null whenever the athlete
   * just pressed start rather than picking the session off the watch's scheduled list, so it is a
   * bonus link and never something a read may depend on.
   *
   * Internal: it is the join key, not something an MCP client or the UI reads.
   */
...
/**
 * The record PROGRESSION read (spec 087): which efforts were a personal best AT THE MOMENT they were
 * set. Unbounded in time on purpose — a window would lose the standing record the window starts from,
 * and the answer is a handful of rows per distance either way.
 */
export interface BestEffortProgressionQuery {
  /** Restrict to these Garmin sport keys (expand a family with `sportKeysInGroup`). */
  readonly sports?: readonly string[];
}
...
export interface CoverageSnapshot {
  ...
  readonly activities: {
    ...
    /**
     * How many synced activities carry Garmin's `workoutId` (spec 081) — i.e. were started from a
     * scheduled workout and can therefore be linked to a plan by identity rather than by guesswork.
     * Surfaced on /dane because it is the only way, short of a database shell, to see whether that
     * link is actually arriving for this account.
     */
    readonly withWorkoutId: number;
...
export interface AuthoredWorkout {
  ...
  /**
   * True once a `createWorkout` call has sent this row's CURRENT title/steps/note/sport to Garmin and
   * no edit has invalidated that since (spec 092). False at creation (nothing sent yet) and false
   * again the moment any content-affecting field changes. What lets `pushWorkout` tell "the id is
   * merely stale because the athlete edited" apart from "the id is merely stale because Garmin's copy
   * was deleted externally" — `garminWorkoutId` alone cannot express either distinction.
   */
  readonly contentPushed: boolean;
...
export interface LocalStore {
  ...
  /**
   * The record progression (spec 087): ONLY the efforts that beat everything set before them for
   * their distance key, ordered by distance then day ascending.
   *
   * This is what makes "what did the prediction say on every day of the last year" one query instead
   * of 365 as-of queries: the fastest-effort-so-far is a step function, and these are its steps.
   * Ties resolve the way `listTopBestEfforts` resolves them — earlier day wins, activity id breaks
   * the rest — so an EQUAL time is not a new record.
   */
  listBestEffortProgression(userId: string, query?: BestEffortProgressionQuery): Promise<RankedBestEffort[]>;
