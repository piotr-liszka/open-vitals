...
export function createMemoryStore(): LocalStore {
  ...
  /**
   * Every stored effort of one user, joined to its activity — the shared FROM clause behind both
   * effort reads (spec 087). The two differ only in how they rank what comes out, so the filtering
   * (sport family, the as-of bound, the inner join) lives in one place and cannot drift between them.
   */
  function rankedEfforts(
    userId: string,
    q: { sports?: readonly string[] | undefined; until?: string | undefined } = {}
  ): RankedBestEffort[] {
    const allowed = q.sports ? new Set(q.sports) : null;
    ...
    for (const [k, entry] of bestEffortsByActivity) {
      ...
      if (q.until && entry.day > q.until) continue;
  ...
  return {
    ...
    async putActivities(userId, list) {
      ...
      for (const a of list) {
        const prev = m.get(a.activityId);
        // Parity with the pg adapter, which keeps `raw` and `garmin_workout_id` on an upsert that
        // carries neither: `listActivities` omits `raw` on purpose, so a row round-tripped through a
        // lite read (the streams phase does exactly that to set hasGps) would otherwise erase the
        // payload — and the workout id read off it — that the write never had in hand.
        m.set(a.activityId, {
          ...a,
          raw: a.raw ?? prev?.raw ?? null,
          garminWorkoutId: a.garminWorkoutId ?? prev?.garminWorkoutId ?? null
        });
      }
    },
    ...
    async listTopBestEfforts(userId, query) {
      const rows = rankedEfforts(userId, { sports: query.sports, until: query.until });
    ...
    async listBestEffortProgression(userId, query = {}) {
      // Mirrors the pg window function (spec 087) row for row: walk each distance in CHRONOLOGICAL
      // order and keep only the efforts that beat everything before them. The comparison is strict,
      // so an equal time set later is not a record — same rule as the leaderboard's tiebreak.
      const rows = rankedEfforts(userId, { sports: query.sports });
      const byKey = new Map<string, RankedBestEffort[]>();
      for (const r of rows) {
        const bucket = byKey.get(r.key);
        if (bucket) bucket.push(r);
      ...
      const kept: RankedBestEffort[] = [];
      for (const bucket of byKey.values()) {
        // The window's ORDER BY, not the ranking: day ascending, activity id as the tiebreak.
        bucket.sort((a, b) => a.day.localeCompare(b.day) || a.activityId.localeCompare(b.activityId));
        let best = Infinity;
        for (const r of bucket) {
          if (!(r.durationS < best)) continue;
          best = r.durationS;
          kept.push(r);
        }
      }
      // Same output order as the pg query.
      kept.sort(
        (a, b) =>
          a.metres - b.metres ||
    ...
    async updateWorkout(userId, id, patch: AuthoredWorkoutPatch) {
      ...
      const next: AuthoredWorkout = {
        ...
        ...(patch.contentPushed !== undefined ? { contentPushed: patch.contentPushed } : {}),
    ...
    async coverage(userId): Promise<CoverageSnapshot> {
      ...
      return {
        ...
        activities: {
          ...
          withWorkoutId: acts.filter((a) => a.garminWorkoutId !== null).length,
