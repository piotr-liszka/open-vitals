  ...
  switch (name) {
    ...
    case 'training_readiness': {
      ...
      */
      ...
      /*
        Garmin computes readiness in the morning and stamps it (spec 075) — without this the dev
        card can only repeat a frozen figure, so the countdown would look "broken" in dev alone.
        Deliberately zoneless with a `.0` fraction, exactly as the live API serves it, so dev
        exercises the parser's UTC handling rather than a tidied-up shape.
      */
      const stamp = `${rawDate}T05:12:33.0`;
      return {
