 ...
 *
 * Deliberately carries no `label`: this table is imported into request handlers that have no
 * `Translator` (and never should — `$lib/server/**` stays i18n-agnostic). A metric's display name
 * is looked up separately, at render time, via `metricLabel(t, spec.key)` from `$lib/metric-labels`
 * (spec: this i18n audit) — the one place that decides Polish vs. English.
 */
