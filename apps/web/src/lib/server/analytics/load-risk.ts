...
export interface LoadRisk {
  ...
  /** Sentence naming what to do about it, in the reader's locale. */
...
function adviceFor(t: Translator): Record<LoadRiskBand, string> {
  return {
    detraining: t('loadRisk.advice.detraining'),
    steady: t('loadRisk.advice.steady'),
    building: t('loadRisk.advice.building'),
    overreaching: t('loadRisk.advice.overreaching'),
    spike: t('loadRisk.advice.spike')
  };
}
 ...
 */
export function loadRisk(series: readonly DailyLoadPoint[], t: Translator = DEFAULT_TRANSLATOR): LoadRisk {
  ...
  if (!last || historyDays < MIN_HISTORY_DAYS || last.ctl <= 0) {
    return {
      ...
      band: 'steady',
      advice: t('loadRisk.advice.notEnoughHistory'),
      historyDays
    };
  }
  ...
  return { acwr, rampRatePerWeek, band, advice: adviceFor(t)[band], historyDays };
}
