...
import { createBcryptHasher, createFixedPasswordHasher, type PasswordHasher } from './auth/password';
...
export interface AppContainer {
  ...
  /** Password hashing port (spec 094): bcrypt in prod, a fixed deterministic double in tests. */
...
export function createContainer(overrides: ContainerOverrides = {}): AppContainer {
  ...
  // Password hashing (spec 094): real bcrypt in prod, injectable for tests.
  const passwordHasher = overrides.passwordHasher ?? createBcryptHasher();
...
export function createTestContainer(overrides: ContainerOverrides = {}): AppContainer {
  ...
  return createContainer({
    ...
    passwordHasher: overrides.passwordHasher ?? createFixedPasswordHasher(),
