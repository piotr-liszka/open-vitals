...
/**
 * `ip_?address`/`user_?agent` (spec 094) are word-bounded, not a bare `ip`, to avoid false positives
 * on unrelated keys that merely contain the letters "ip". Defense-in-depth per AGENTS.md §10: the
 * auth/session/onboarding handlers pass IP/UA straight from the route layer to the repo and never
 * through `logger.*()` at all — this regex is the backstop for a FUTURE change that does, not the
 * primary control.
 */
const REDACT_KEYS =
  /pass(word)?|secret|token|cookie|authorization|mfa|email|credential|session|ip_?address|user_?agent/i;
