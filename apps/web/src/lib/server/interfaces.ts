...
export interface GarminWorkoutInput {
  ...
  /**
   * The workout-level description (spec 082) — already composed by the caller from the athlete's
   * note and the provenance line. Null sends no description at all, which is what Garmin saw for
   * every workout before 082.
   */
...
export interface GarminWorkoutScheduleResult {
  ...
  /**
   * Short machine reason when unsupported. `workout_not_found` (spec 092) means Garmin answered 404
   * for THIS workout id specifically — evidence the upstream copy is gone (deleted externally, or
   * stale after this app deleted it), not that the account cannot schedule workouts at all. Distinct
   * from `unsupported_endpoint`, which is a permanent, account-wide answer. The HTTP adapter forwards
   * whichever the sidecar returns verbatim; no adapter-side mapping is needed for the new value.
   */
  readonly reason: string | null;
}
...
export interface SessionService {
   ...
   *
   * `meta` (spec 094) carries the raw request User-Agent + client address at issue time, captured at
   * the ROUTE layer (never inside a pure handler) and stored for the user's own "My Account" active
   * sessions list. Never logged.
   */
