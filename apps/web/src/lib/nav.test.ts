...
describe('primary navigation', () => {
  ...
  it('holds no dashboard entry (spec 064/2026-08)', () => {
...
describe('admin nav entry (spec 094)', () => {
  it('is absent by default', () => {
    expect(navGroups(t).flatMap((g) => g.items.map((i) => i.href))).not.toContain('/admin/users');
  });

  it('appears in the System group only when isAdmin is true', () => {
    const groups = navGroups(t, true);
    const system = groups.find((g) => g.group === 'System');
    expect(system?.items.map((i) => i.href)).toContain('/admin/users');
  });

  it('uses its own icon, distinct from every other item', () => {
    const withAdmin = navItems(t, true);
    const icons = withAdmin.map((i) => i.icon);
