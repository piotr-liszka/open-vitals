...
describe('MCP workout tools', () => {
  ...
  it('update_workout re-queues the session and re-validates the whole thing', async () => {
    ...
    // Re-queued for the push. The Garmin id is KEPT (spec 092 needs it to delete the stale upstream
    // copy), but contentPushed flips back to false so the next push deletes it and creates a fresh
    // one rather than re-scheduling the old content under the same id.
    expect(after?.pushState).toBe('pending');
    expect(after?.garminWorkoutId).toBe('g-1');
    expect(after?.contentPushed).toBe(false);
