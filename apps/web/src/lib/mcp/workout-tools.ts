 ...
 * feature switch. Read tools still take only a `GarminService`.
 *
...
import type { FeatureService } from '../server/features/types';
...
/** Feature gate. The write tools are dark while the Garmin workout-write switch is off (spec 071). */
async function requireWriteEnabled(deps: WorkoutToolDeps): Promise<ToolResult | null> {
  const enabled = await deps.features.isEnabled(WORKOUT_WRITE_FEATURE);
  if (enabled) return null;
  return errorText(
    'Writing workouts to Garmin is not enabled for this account. Open the web app → Ustawienia → ' +
      'Integracje → Garmin and switch "Automatyczny zapis treningów do Garmina" on. Reading data is ' +
      'unaffected.'
  );
}

/**
 * MCP tool output is an ENGLISH machine surface (every tool name, description and error string here
 * already is), so its sport names are English too (spec 076) — not the reader's UI language, which
 * an MCP request does not carry, and not Polish, which would be the odd word out in an English
 * payload.
 */
...
function view(workout: AuthoredWorkout): Record<string, unknown> {
  return {
    ...
    time: workout.time,
    sport: workout.sport,
    sportLabel: sportLabel(MCP_TRANSLATOR, workout.sport),
    title: workout.title,
...
const createWorkoutTool: WorkoutTool = {
  ...
  async handler(deps, args) {
    const gate = await requireWriteEnabled(deps);
    if (gate) return gate;
    try {
  ...
  async handler(deps) {
    ...
    return text({
      ...
      templates: templates.map((t) => ({
        title: t.title,
        sport: t.sport,
        sportLabel: sportLabel(MCP_TRANSLATOR, t.sport),
  ...
  async handler(deps, args) {
    ...
    try {
      ...
      const updated = await deps.store.updateWorkout(deps.userId, id, {
        ...
        // garminWorkoutId is KEPT here (spec 092 needs it to delete the stale upstream copy), but
        // `contentPushed: false` means the next push deletes it and creates a fresh one rather than
        // re-scheduling the old, now-outdated content under the same id.
        pushState: 'pending',
        pushError: null,
        contentPushed: false,
