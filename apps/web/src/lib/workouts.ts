...
export const WORKOUT_LIMITS = {
  maxTitle: 80,
  maxNote: 512,
  /** The composed Garmin description (spec 082). Mirrors the sidecar's own `Field(max_length=1024)`. */
  maxDescription: 1024,
  maxSteps: 50,
  maxChildSteps: 40,
  maxRepeats: 50,
...
/** Which unit a plan is actually written in — the axis its adherence should be judged on. */
export type WorkoutAxis = 'distance' | 'time';

/**
 * The axis a session was PRESCRIBED in, weighted by which one accounts for more of it.
 *
 * Adherence has to be judged on the axis the athlete was asked to hold. "Easy 5 km + kadencja 166"
 * is a 2.4 km block between two paced time blocks: judged on distance, running the time blocks
 * slower than the band reads as a SHORTER session, and the review calls it `shortened` — the wrong
 * word for running the full prescribed time. Judged on time, it is what it is.
 *
 * So neither "has a distance step" nor "has a time step" decides it; the larger share does. Seconds
 * are the common currency because every step can be expressed in them. A distance step whose pace is
 * unknown cannot be converted, and rather than drop it the answer falls back to `distance` — it was
 * still written in metres, which is the one thing we do know about it.
 *
 * `null` only when the plan prescribes nothing measurable at all (every step ends on the lap button).
 */
export function workoutPrescribedAxis(steps: readonly WorkoutStep[]): WorkoutAxis | null {
  let distanceSeconds = 0;
  let timeSeconds = 0;
  let unconvertedDistanceStep = false;

  const walk = (list: readonly WorkoutStep[], factor: number): void => {
    for (const step of list) {
      if (step.kind === 'repeat') {
        walk(step.steps ?? [], factor * (step.repeats ?? 1));
        continue;
      }
      if (!step.durationValue) continue;
      if (step.durationType === 'time') {
        timeSeconds += step.durationValue * factor;
        continue;
      }
      if (step.durationType === 'distance') {
        const speed = step.target ? targetSpeedMs(step.target) : null;
        if (speed) distanceSeconds += (step.durationValue / speed) * factor;
        else unconvertedDistanceStep = true;
      }
    }
  };
  walk(steps, 1);

  if (unconvertedDistanceStep) return 'distance';
  if (distanceSeconds === 0 && timeSeconds === 0) return null;
  // Ties go to distance: a plan that says "5 km" means the 5 km.
  return distanceSeconds >= timeSeconds ? 'distance' : 'time';
}

/**
 * Planned distance in metres, as far as it can be known — the mirror image of
 * {@link estimateWorkoutDurationS}: distance steps count directly, TIME steps count when a
 * pace/speed target pins their speed down (`durationValue × speed`), `lap` and `calories` steps
 * cannot be known at all. Returns null when nothing is knowable, so a caller never shows a
 * confidently wrong number.
 *
 * Counting only distance steps — as this did until the two estimators were made symmetric — reports
 * a 10-minute warm-up at 6:00/km as zero metres, so an "Easy 5 km" written as jog + 3 km + jog came
 * out at 2.4 km and any plan-versus-actual card read the session as double its target.
 *
 * The number can still UNDERSTATE the session, and is a floor rather than a promise: a time step
 * with no usable target (an HR or cadence band, or no target at all) adds nothing rather than a
 * guess, even though the athlete covers ground during it.
 */
export function estimateWorkoutDistanceM(steps: readonly WorkoutStep[]): number | null {
  ...
  for (const step of steps) {
    ...
    if (step.durationType === 'time' && step.durationValue && step.target) {
      const speed = targetSpeedMs(step.target);
      if (speed) {
        total += step.durationValue * speed;
        known = true;
...
/**
 * The line appended to every workout pushed to Garmin (spec 082), answering "did this come from
 * OpenVitals or from Garmin's own plan?" for someone reading Connect.
 *
 * Deliberately NOT localised and deliberately not templated with a date. It is a mark to recognise
 * at a glance, and a sentence that changes with the UI language — or with the day — stops being one.
 */
export const WORKOUT_PROVENANCE_LINE = 'via OpenVitals';

/**
 * The athlete's note plus the provenance line, as the workout-level `description` Garmin stores.
 *
 * Truncation drops the NOTE, never the provenance line: the line is a handful of characters and the
 * one part of this string that has to survive for the field to do its job. A workout with no note
 * sends the line alone rather than nothing, so an empty-note session is still recognisably ours.
 */
export function composeWorkoutDescription(note: string | null | undefined): string {
  const body = typeof note === 'string' ? note.trim() : '';
  if (!body) return WORKOUT_PROVENANCE_LINE;

  const suffix = `\n\n${WORKOUT_PROVENANCE_LINE}`;
  const room = WORKOUT_LIMITS.maxDescription - suffix.length;
  const kept = body.length <= room ? body : `${body.slice(0, Math.max(0, room - 1)).trimEnd()}…`;
  return `${kept}${suffix}`;
}
