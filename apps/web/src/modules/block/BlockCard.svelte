<script lang="ts">
  ...
  const PACE_LABEL_KEYS: Record<string, MessageKey> = {
    easy: 'block.pace.easy',
    long: 'block.pace.long',
    threshold: 'block.pace.threshold',
    interval: 'block.pace.interval',
    goal: 'block.pace.goal'
  };
  ...
  const paceRows = $derived(
    Object.entries(week?.paces ?? {})
      .filter(([, range]) => range)
      .map(([key, range]) => ({
        label: PACE_LABEL_KEYS[key] ? i18n.t(PACE_LABEL_KEYS[key]!) : key,
        value: `${mmss(range!.lowS)}–${mmss(range!.highS)}/km`
      }))
  );

  const subtitle = $derived(
    week ? `${formatDay(i18n.locale, week.weekStart)} – ${formatDay(i18n.locale, week.weekEnd)}` : undefined
  );
  ...
  const PUSH_LABEL_KEYS: Record<string, MessageKey> = {
    pushed: 'block.push.pushed',
    pending: 'block.push.pending',
    failed: 'block.push.failed',
    unsupported: 'block.push.unsupported'
  };
  const pushLabel = (state: string): string =>
    PUSH_LABEL_KEYS[state] ? i18n.t(PUSH_LABEL_KEYS[state]!) : state;
  ...
  const km = (m: number | null): string | null => (m === null ? null : `${nf1(m / 1000)} km`);
...
<Card title={i18n.t('block.title')} {...subtitle ? { subtitle } : {}}>
  {#if !week}
    <p class="empty">
      {i18n.t('block.emptyState')}
    </p>
  {:else}
    <div class="head">
      <div class="who">
        ...
        <span class="week-of"
          >{i18n.t('block.weekOf', { weekNumber: week.weekNumber, weeks: week.weeks })}</span
        >
      </div>
    ...
    {#if week.soreness}
      <div class="banner-slot">
        <Banner tone="warning" title={i18n.t('block.sorenessBannerTitle')}>
          {i18n.t('block.sorenessBody', {
            soreness: week.soreness.soreness,
            location: week.soreness.location ? ` — ${week.soreness.location}` : '',
            day: formatDay(i18n.locale, week.soreness.day)
          })}
        </Banner>
      </div>
    {/if}
    ...
    <div class="tiles">
      <StatTile
        label={i18n.t('block.tile.actual')}
        value={nf1(week.volumeActualKm)}
        unit="km"
        accent="orange"
      />
      <StatTile
        label={i18n.t('block.tile.target')}
        value={week.volumeTargetKm === null ? '—' : nf1(week.volumeTargetKm)}
        {...week.volumeTargetKm === null ? {} : { unit: 'km' }}
        muted={week.volumeTargetKm === null}
      />
      <StatTile
        label={i18n.t('block.tile.remaining')}
        value={remainingKm === null ? '—' : nf1(Math.max(remainingKm, 0))}
    ...
    {#if progress !== null}
      <ProgressBar value={progress} label={i18n.t('block.progressLabel')} />
    {/if}

    <h4 class="section">{i18n.t('block.sessionsTitle')}</h4>
    {#if week.sessions.length === 0}
      <p class="empty">{i18n.t('block.noSessions')}</p>
    {:else}
      <ul class="sessions">
        {#each week.sessions as session (session.id)}
          <li>
            <div class="session-main">
              <span class="session-day">{formatDay(i18n.locale, session.day)}</span>
    ...
    {#if paceRows.length > 0}
      <h4 class="section">{i18n.t('block.pacesTitle')}</h4>
      <dl class="paces">
    ...
    {#if week.constraints.length > 0}
      <h4 class="section">{i18n.t('block.constraintsTitle')}</h4>
    ...
    {#if week.goal}
      <p class="goal">
        {i18n.t('block.goalLabel')} <strong>{week.goal.title}</strong>
        · {formatDay(i18n.locale, week.goal.day)}
        · {week.goal.daysOut > 0
          ? i18n.t('block.goalDaysOut', { days: week.goal.daysOut })
          : i18n.t('block.goalPast')}
      </p>
    {/if}
  {/if}
</Card>
