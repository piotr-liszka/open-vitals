 ...
 */
import { DEFAULT_LOCALE, createTranslator, type Locale, type Translator } from '$lib/i18n';
...
function sessionView(t: Translator, w: AuthoredWorkout): WeekSession {
  ...
  return {
    ...
    time: w.time,
    sport: w.sport,
    sportLabel: sportLabel(t, w.sport),
    title: w.title,
...
async function resolveWeek(
  ...
  return {
    ...
    sessions: workouts.map((w) => sessionView(t, w)),
    paces: block.paces,
...
export async function loadCurrentWeek(
  ...
  return {
    today: day,
    block,
    week: await resolveWeek(t, deps, userId, block, day, targets, goal)
  };
}
