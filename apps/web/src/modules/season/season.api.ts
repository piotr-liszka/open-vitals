 ...
 */
import { createTranslator, type MessageKey, type Translator } from '$lib/i18n';
...
const PHASE_KEYS: Readonly<Record<string, MessageKey>> = {
  done: 'season.phase.done',
  'race-week': 'season.phase.raceWeek',
  taper: 'season.phase.taper',
  peak: 'season.phase.peak',
  build: 'season.phase.build',
  base: 'season.phase.base',
  far: 'season.phase.far'
};

function phaseLabel(t: Translator, phase: string): string {
...
/**
 * The verdict in one sentence, in the reader's own language.
 *
...
function verdictNote(
  ...
  if (daysOut < 0) return t('season.verdict.past');

  // Inside the taper the trajectory is settled; the only live question is whether load is falling.
  if (taper) {
    return t(taper.tapering ? 'season.verdict.taperGood' : 'season.verdict.taperBad', {
      pct: Math.round(taper.ratio * 100)
    });
  }

  switch (status) {
    case 'at-risk':
      return t('season.verdict.atRisk');
    case 'behind':
      return requiredRampPerWeek === null
        ? t('season.verdict.behindNoRoom')
        : t('season.verdict.behindRamp', { ramp: requiredRampPerWeek });
    case 'ahead':
      return t('season.verdict.ahead');
    case 'on-track':
      return t('season.verdict.onTrack');
    default:
      return t('season.verdict.unknown');
  }
}
...
function buildPrediction(
   ...
   */
  const target = RACE_TARGETS.find((rt) => Math.abs(rt.metres - goal.distanceM!) < 1) ?? {
    key: 'goal',
    label: goal.title,
...
export async function loadSeason(deps: SeasonDeps, req: SeasonRequest): Promise<SeasonData> {
  ...
  if (ftpWatts == null) {
    ...
    if (powerActs.length > 0) ftpWatts = buildPowerProfile(powerActs, { weightKg: null, t }).ftpWatts;
  }

  const loadOpts = { ftpWatts, endDay: today, t };
  ...
  const statuses: GoalStatus[] = goals.map((goal) => {
    ...
    const risk = hasFamilyData ? loadRisk(pmc.series, t) : null;
    ...
    return {
      ...
      sportLabel: sportGroupLabel(t, goal.sport),
      ...
      requiredRampPerWeek,
      taper,
      prediction: past ? null : buildPrediction(goal, runs, speedCurve, t),
      status,
      note: verdictNote(t, status, daysOut, requiredRampPerWeek, taper)
    };
  });
  ...
  return {
    today,
    goals: [...future, ...past],
    suggestions: await loadSuggestions(t, deps, req.userId, today, goals),
  ...
  return events
    ...
    .map((e) => {
      ...
      return {
        ...
        title: e.title,
        sport: group,
        sportLabel: sportGroupLabel(t, group),
