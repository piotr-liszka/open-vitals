...
export interface HrBlock {
  readonly avg: number | null;
  /** This session's own peak — what the heart actually did today. */
  readonly max: number | null;
  /**
   * The reference the %-of-max split was taken against (spec 090): the athlete's configured maximum
   * when they have saved one, otherwise this session's peak. Two separate fields because they answer
   * different questions, and conflating them is what made the top zone meaningless.
   */
  readonly zoneMax: number | null;
  /** True when `zoneMax` came from the athlete's profile rather than from this session. */
  readonly zoneMaxConfigured: boolean;
  readonly zones: ZoneBucket[];
}
