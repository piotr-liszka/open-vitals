...
/**
 * Spec 081 — the activity's own `workoutId`. Everything in `matchPlanned` above is a tie-break
 * between plausible plans; this is the case where there is nothing left to break.
 */
describe('matchPlanned — workout id (spec 081)', () => {
  it('takes the plan pushed under the activity’s own workout id', () => {
    const other = plan({ id: 'other', origin: 'authored', steps: [step({})] });
    const linked = plan({ id: 'linked', garminWorkoutId: '1668504046' });

    expect(matchPlanned([other, linked], 'cycling', '1668504046')?.id).toBe('linked');
    // Order must not matter.
    expect(matchPlanned([linked, other], 'cycling', '1668504046')?.id).toBe('linked');
  });

  it('beats the authored-over-garmin rule the ranking would otherwise apply', () => {
    const authored = plan({ id: 'a', origin: 'authored', steps: [step({})] });
    const linked = plan({ id: 'g', origin: 'garmin', garminWorkoutId: 'g-1' });
    expect(matchPlanned([authored, linked], 'cycling', 'g-1')?.id).toBe('g');
  });

  it('matches across sport families — the watch knows what was started', () => {
    const linked = plan({ id: 'swim', sport: 'lap_swimming', garminWorkoutId: 'g-1' });
    expect(matchPlanned([linked], 'cycling', 'g-1')?.id).toBe('swim');
  });

  it('matches nothing on a null on either side', () => {
    // Null is not a value: two sessions without an id are not "the same workout".
    expect(matchPlanned([plan({ id: 'p1' })], 'cycling', null)?.id).toBe('p1'); // heuristic, as before
    expect(matchPlanned([plan({ id: 'p1', sport: 'running' })], 'cycling', 'g-1')).toBeNull();
  });

  it('falls back to the ranking when no candidate carries the id', () => {
    const race = plan({ id: 'race', kind: 'race' });
    const workout = plan({ id: 'workout', kind: 'workout' });
    expect(matchPlanned([race, workout], 'cycling', 'g-unknown')?.id).toBe('workout');
  });
});
