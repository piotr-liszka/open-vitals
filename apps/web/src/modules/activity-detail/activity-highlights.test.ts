...
describe('buildHighlights', () => {
  ...
  it('offers speed to a ride and pace to a run, never both', () => {
    ...
    const asRun = buildHighlights(t, { ...args, sport: 'run', current: run('2026-08-01') });
    const asRide = buildHighlights(t, { ...args, sport: 'ride', current: run('2026-08-01') });
  ...
  it('is anchored on the activity day: a later session cannot change the standing', () => {
    ...
    const withoutFuture = buildHighlights(t, {
      ...base,
      current: run('2026-08-01', { distanceM: 30_000 }),
      history: boringHistory(12)
    });
...
describe('buildSuspects', () => {
  ...
  it('is silent on a clean activity', () => {
    expect(buildSuspects(t, clean)).toEqual([]);
  });
  ...
  it('flags a max speed past the ceiling for the sport as a warning', () => {
    const flags = buildSuspects(t, { ...clean, maxSpeedMps: 12 }); // 43.2 km/h in a run
  ...
  it('accepts on a bike what it rejects in a run', () => {
    const asRun = buildSuspects(t, { ...clean, sport: 'run', maxSpeedMps: 12 });
    const asRide = buildSuspects(t, {
      ...clean,
      sport: 'ride',
      avgSpeedMps: 8,
      maxSpeedMps: 12,
  ...
  it('flags a maximum far above its own average even when it clears the ceiling', () => {
    const flags = buildSuspects(t, { ...clean, maxSpeedMps: 8.4 }); // 30.2 km/h, under 32
  ...
  it('tolerates a few seconds of rounding between moving and elapsed', () => {
    const flags = buildSuspects(t, { ...clean, movingS: 3103, durationS: 3100 });
  ...
  it('flags a long cadence dropout', () => {
    ...
    expect(find(buildSuspects(t, { ...clean, cadence }), 'cadenceGap')).toMatchObject({
      severity: 'info'
    });
  });
  ...
  it('ignores cadence on a sport that has none', () => {
    ...
    expect(find(buildSuspects(t, { ...clean, sport: 'swim', cadence }), 'cadenceGap')).toBeUndefined();
  });

  it('puts warnings before notes', () => {
    const flags = buildSuspects(t, { ...clean, maxHr: 231, avgSpeedMps: 2 });
