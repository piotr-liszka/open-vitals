...
import { formatDecimals, DEFAULT_LOCALE, type Locale } from '$lib/i18n';
...
const SECTION_TITLE_KEYS: Readonly<Record<ChartGroupKey, MessageKey>> = {
  effort: 'chart.section.effort',
  terrain: 'chart.section.terrain',
  physiology: 'chart.section.physiology',
  dynamics: 'chart.section.dynamics'
};

export function chartGroupTitle(t: Translator, group: ChartGroupKey): string {
  return t(SECTION_TITLE_KEYS[group]);
}
...
export function buildActivityCharts(
  ...
  const inputs: ChartInput[] = [
    ...
    pace
      ? {
          key: 'pace',
          title: t('chart.pace.title'),
          unit: 'min/km',
          note: t('chart.pace.note'),
      ...
      ? [
          {
            ...
            title: t('chart.gradeAdjustedPace.title'),
            unit: 'min/km',
            note: t('chart.gradeAdjustedPace.note'),
            color: 'var(--lane-lime)',
    ...
    {
      key: 'cadence',
      title: t('chart.cadence.title'),
      unit: pace ? t('chart.unit.stepsPerMin') : t('chart.unit.rpm'),
    ...
    {
      key: 'elevation',
      title: t('chart.elevation.title'),
      unit: t('chart.unit.metresAsl'),
    ...
    {
      key: 'temperature',
      title: t('chart.temperature.title'),
      unit: '°C',
      color: 'var(--lane-sky)',
      group: 'terrain',
      kind: 'decimal',
    ...
    },
    {
      ...
      key: 'cardiacCost',
      title: t('chart.cardiacCost.title'),
      unit: t('chart.unit.beatsPerKm'),
      note: t('chart.cardiacCost.note'),
      color: 'var(--lane-red)',
      group: 'physiology',
      kind: 'int',
    ...
    {
      key: 'respirationRate',
      title: t('chart.respirationRate.title'),
      unit: t('chart.unit.breathsPerMin'),
      color: 'var(--lane-teal)',
      group: 'physiology',
      kind: 'decimal',
    ...
    },
    {
      ...
      title: t('chart.performanceCondition.title'),
      unit: t('chart.unit.points'),
      color: 'var(--lane-cyan)',
      group: 'physiology',
      kind: 'int',
    ...
    {
      key: 'verticalRatio',
      title: t('chart.verticalRatio.title'),
      unit: '%',
    ...
    },
    {
      ...
      title: t('chart.verticalOscillation.title'),
      unit: 'cm',
      color: 'var(--lane-violet)',
      group: 'dynamics',
      kind: 'decimal',
    ...
    },
    {
      ...
      title: t('chart.groundContactTime.title'),
      unit: 'ms',
      color: 'var(--lane-amber)',
      group: 'dynamics',
      kind: 'int',
    ...
    },
    {
      ...
      title: t('chart.groundContactBalance.title'),
      unit: '% L',
      note: t('chart.groundContactBalance.note'),
      color: 'var(--lane-cyan)',
      group: 'dynamics',
      kind: 'decimal',
    ...
    {
      key: 'strideLength',
      title: t('chart.strideLength.title'),
      unit: 'cm',
      color: 'var(--lane-green)',
      group: 'dynamics',
      kind: 'int',
  ...
  const stamina = staminaChart(t, streams, indices);
...
function staminaChart(
  t: Translator,
  streams: ActivityStreams,
  indices: readonly number[]
): ActivityChartSpec | null {
  ...
  if (definedCount(current) >= 2)
    series.push({ name: t('chart.stamina.current'), values: current, color: 'var(--lane-lime)' });
  if (definedCount(potential) >= 2)
    series.push({ name: t('chart.stamina.potential'), values: potential, color: 'var(--lane-teal)' });
  if (series.length === 0) return null;
  return {
    key: 'stamina',
    title: t('chart.stamina.title'),
    unit: '%',
...
export function buildChartSet(
  ...
  return {
    axis: effectiveAxis,
    indices,
    labels: axisLabels(effectiveAxis, elapsed, distance, indices, locale),
    elapsedS: indices.map((i) => elapsed[i] ?? 0),
    distanceM: distance ? indices.map((i) => distance[i] ?? 0) : null,
    canUseDistance,
    charts: buildActivityCharts(t, streams, sport, indices)
  };
}

/**
 * X tick labels: `mm:ss`/`h:mm:ss` on a time axis, `x.x km` (locale-aware separator) on a distance
 * axis. `locale` defaults to Polish so a caller that has not been given one yet keeps its exact
 * prior output.
 */
export function axisLabels(
  ...
  if (axis === 'distance' && distance) {
    return indices.map((i) => `${formatDecimals(locale, (distance[i] ?? 0) / 1000, 1)} km`);
  }
