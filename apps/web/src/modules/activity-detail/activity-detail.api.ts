...
export interface ActivityDetailDeps {
  ...
  /**
   * The caller's translator (spec: this i18n audit). Optional so every existing caller — MCP tools,
   * the wide sweep of tests below — keeps compiling and keeps its Polish-only behaviour unchanged;
   * only the web route passes the request's real locale. Defaults to the Polish catalog, same as
   * `createTranslator`'s own fallback.
   */
  t?: Translator;
}

/** The translator this handler actually uses — the caller's, or Polish when none was given. */
function resolveTranslator(deps: ActivityDetailDeps): Translator {
  return deps.t ?? createTranslator(DEFAULT_LOCALE);
}
...
const fromPlannedEvent = (e: PlannedEvent): PlannedCandidate => ({
  ...
  steps: null,
  // Garmin's calendar mirror does not carry the workout id (spec 024 stores the event, not the
  // workout behind it), so this half of the plan never wins on identity — only the authored half,
  // which is the half whose id we set ourselves when we pushed it.
...
const fromAuthoredWorkout = (w: AuthoredWorkout): PlannedCandidate => ({
  ...
  targetLoad: null,
  steps: w.steps,
  // Set by the push phase (spec 050). Equal to the activity's own `workoutId` = the watch says this
  // is the session that was executed (spec 081).
...
async function loadPlanned(
  ...
  if (!isDayKey(day)) {
    return {
      ...
      garminWorkoutId: activity.garminWorkoutId
    };
  }
  ...
  /*
   * Same day — plus, since spec 081, any authored session ANYWHERE in the probe window that the
   * watch says this activity was: an id match is not subject to a day tolerance, so a session run
   * three days late must still find its plan. Deduped by id so a session that is both is offered
   * once.
   */
  const linkedId = activity.garminWorkoutId;
  const sameDayAuthored = authored.filter((w) => w.day === day);
  const linkedElsewhere =
    linkedId === null
      ? []
      : authored.filter((w) => w.day !== day && w.garminWorkoutId !== null && w.garminWorkoutId === linkedId);

  return {
    sameDay: [
      ...events.filter((e) => e.day === day).map(fromPlannedEvent),
      ...[...sameDayAuthored, ...linkedElsewhere].map(fromAuthoredWorkout)
    ],
...
export async function loadActivityDetail(
  ...
  const t = resolveTranslator(deps);
  ...
  /**
   * The athlete's own maximum, when they have told us one (spec 090). Read once here because two
   * different consumers need it: the heart-rate zone split below and the TRIMP load model further
   * down. Absent until spec 090 gave it a screen — before that this whole file bucketed against
   * whatever the session itself happened to peak at.
   */
  const configuredMaxHr = positiveNumber(settings.maxHrBpm);
  ...
  if (hr && hr.length > 0) {
    // What the athlete's heart actually did today — a fact about this session, always reported.
    const sessionMaxHr = activity.maxHr ?? maxOf(hr);
    /*
     * What the %-of-max split is taken against (spec 090). The configured maximum wins when there
     * is one; without it we fall back to this session's own peak, which is byte-for-byte the
     * pre-090 behaviour — and also what made the split meaningless: dividing a session by its own
     * maximum hands out Z5 for free, so even a recovery ride came back with time in the top zone.
     */
    const zoneMax = configuredMaxHr ?? sessionMaxHr;
    hrBlock = {
      avg: activity.avgHr ?? meanOf(hr),
      max: sessionMaxHr,
      zoneMax,
      zoneMaxConfigured: configuredMaxHr !== null,
      zones: hrZones(hr, zoneMax, time)
    };
  }
  ...
  const matchedPlan = trainingComparison?.plannedWorkout
    ? matchPlanned(planned.sameDay, planned.sport, planned.garminWorkoutId)
    : null;
  ...
  const suspects = buildSuspects(t, {
    sport: family,
    distanceM: activity.distanceM,
