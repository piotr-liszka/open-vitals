<script lang="ts">
  ...
  import { DASH, SIMILAR_METRICS, fmtDuration, fmtKm, fmtPace, similarDeltaBadge } from './activity-format';
  ...
  const options = $derived([
    { value: 'effort', label: i18n.t('similar.tab.effort') },
    { value: 'route', label: i18n.t('similar.tab.route') }
  ]);
  ...
  const badgesFor = (
    ...
    SIMILAR_METRICS.flatMap((m) => {
      const badge = similarDeltaBadge(i18n.t, e[m.key], m);
      return badge ? [{ key: m.key, badge }] : [];
    });

  const plural = (
    key: 'similar.matches' | 'similar.comparedSessions' | 'similar.comparedSessionsShort',
    count: number
  ): string => i18n.t(key, { count });
</script>

<Card>
  <div class="head">
    <div>
      <h2 class="title">{i18n.t('similar.title')}</h2>
      <p class="sub">{i18n.t('similar.subtitle')}</p>
    </div>
    <SegmentedControl {options} bind:value={tab} ariaLabel={i18n.t('similar.tabsAriaLabel')} size="sm" />
  </div>
  ...
  {#if tab === 'effort'}
    {#if similar === null}
      ...
      <p class="empty">
        {i18n.t('similar.emptyNoAxis', { routeTab: i18n.t('similar.tab.route') })}
      </p>
    {:else if similar.entries.length === 0}
      <p class="empty">
        {i18n.t('similar.emptyNoMatch', {
          tolerance: similar.tolerancePct,
          compared: plural('similar.comparedSessions', similar.comparedCount)
        })}
      </p>
    {:else}
      <p class="scope">
        {i18n.t('similar.scope', {
          matches: plural('similar.matches', similar.entries.length),
          tolerance: similar.tolerancePct,
          compared: plural('similar.comparedSessionsShort', similar.comparedCount),
          recent: similar.coversAllHistory ? '' : i18n.t('similar.recentOnly')
        })}
      </p>
      ...
      <div class="scroller">
        <table class="tbl">
          <thead>
            <tr>
              <th scope="col">{i18n.t('similar.col.date')}</th>
              ...
              <th scope="col" class="num">{i18n.t('similar.col.distance')}<span class="unit">km</span></th>
              <th scope="col" class="num">{i18n.t('similar.col.time')}</th>
              <th scope="col" class="num">{i18n.t('similar.col.pace')}<span class="unit">/km</span></th>
              <th scope="col">{i18n.t('similar.col.comparison')}</th>
            </tr>
          </thead>
          <tbody>
            {#each similar.entries as e (e.activityId)}
              ...
              <tr>
                ...
                <td class="num">{fmtKm(e.distanceM, 1, i18n.locale)}</td>
