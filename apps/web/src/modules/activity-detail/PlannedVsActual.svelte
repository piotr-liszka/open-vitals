<script lang="ts">
  ...
  function fmtValue(key: PlannedStepKey, value: number | null): string {
    ...
    if (key === 'distance') return `${fmtKm(value, 2, i18n.locale)} km`;
    if (key === 'pace') return `${fmtPace(value)} min/km`;
    if (key === 'power') return `${fmtNum(value, 0, i18n.locale)} W`;
    if (key === 'hr') return `${fmtNum(value, 0, i18n.locale)} bpm`;
    return fmtNum(value, 0, i18n.locale);
  }
