...
import { hrZones } from '$lib/server/analytics/activity-power';
...
describe('loadActivityDetail', () => {
  ...
  describe('heart-rate zones and the maximum they are taken against (spec 090)', () => {
    /** A session whose own peak (160 bpm) sits well below a trained athlete's real maximum. */
    async function seedHr(samples: readonly number[]) {
      const store = createMemoryStore();
      await store.putActivities('u', [act({ avgHr: 140, maxHr: 160 })]);
      await store.putStreams('u', 'a', {
        heartRate: [...samples],
    ...
    const steady = new Array(600).fill(128);

    it('buckets against the configured maximum when the athlete has saved one', async () => {
      const store = await seedHr(steady);

      const data = await loadActivityDetail({ store, settings: stubSettings({ maxHrBpm: 175 }) }, 'u', 'a');

      // 128 bpm is 73% of 175 → Z3. Against this session's own 160 peak it would be 80% → Z4.
      expect(data!.hr?.zones.find((z) => z.zone === 3)?.pct).toBe(100);
      expect(data!.hr?.zones.find((z) => z.zone === 4)?.pct).toBe(0);
      expect(data!.hr?.zoneMax).toBe(175);
      expect(data!.hr?.zoneMaxConfigured).toBe(true);
      // The session's own peak is still reported — it is a fact about the ride, not a reference.
      expect(data!.hr?.max).toBe(160);
    });

    it('is byte-identical to the pre-090 split when no maximum is saved', async () => {
      const store = await seedHr(steady);

      const data = await loadActivityDetail({ store, settings: stubSettings({}) }, 'u', 'a');

      // Exactly what `hrZones(hr, activity.maxHr ?? maxOf(hr), time)` produced before this spec.
      expect(data!.hr?.zones).toEqual(
        hrZones(
          steady,
          160,
          steady.map((_, i) => i)
      ...
      expect(data!.hr?.zones.find((z) => z.zone === 4)?.pct).toBe(100);
      expect(data!.hr?.zoneMax).toBe(160);
      expect(data!.hr?.zoneMaxConfigured).toBe(false);
    });

    it('falls back to the stream peak when the summary carries no maximum, as before', async () => {
      const store = createMemoryStore();
      await store.putActivities('u', [act({ avgHr: null, maxHr: null })]);
      const ramp = Array.from({ length: 600 }, (_, i) => 100 + Math.floor(i / 10));
      await store.putStreams('u', 'a', { heartRate: ramp, time: ramp.map((_, i) => i) });

      const data = await loadActivityDetail({ store, settings: stubSettings({}) }, 'u', 'a');

      expect(data!.hr?.zoneMax).toBe(159); // the highest sample in the stream
      expect(data!.hr?.zones).toEqual(
        hrZones(
          ramp,
          159,
    ...
    it('stops handing the top zone out for free — that was the bug', async () => {
      // A ramp that finishes at the session's own peak: dividing by that peak guarantees Z5.
      const ramp = Array.from({ length: 600 }, (_, i) => 100 + Math.floor(i / 10));
      const store = await seedHr(ramp);

      const unset = await loadActivityDetail({ store, settings: stubSettings({}) }, 'u', 'a');
      expect(unset!.hr?.zones.find((z) => z.zone === 5)?.seconds).toBeGreaterThan(0);

      const configured = await loadActivityDetail(
        { store, settings: stubSettings({ maxHrBpm: 190 }) },
        'u',
        'a'
      );
      // 90% of 190 is 171 bpm, which this session never reached. Z5 is now something to earn.
      expect(configured!.hr?.zones.find((z) => z.zone === 5)?.seconds).toBe(0);
    });

    it('ignores a stored maximum that is not a usable number', async () => {
      const store = await seedHr(steady);

      for (const bad of [null, '175', 0, -175, Number.NaN]) {
        const data = await loadActivityDetail({ store, settings: stubSettings({ maxHrBpm: bad }) }, 'u', 'a');
        expect(data!.hr?.zoneMaxConfigured, String(bad)).toBe(false);
        expect(data!.hr?.zoneMax, String(bad)).toBe(160);
      }
    });
  });
...
/* ------------------------------------------------------------------------------------ *
 * The hard link (spec 081)
 * ------------------------------------------------------------------------------------ */

describe('loadActivityDetail — workout id (spec 081)', () => {
  ...
  it('picks the session the watch linked over the other plan on the same day', async () => {
    const store = createMemoryStore();
    await store.putActivities('u', [act({ garminWorkoutId: '1668504046' })]);
    // Ids chosen so the alphabetical tie-break of spec 085's ranking would pick the WRONG one:
    // without pass 0 this test fails, which is the whole point of it.
    await authored(store, 'aw-a-other', '2026-05-01', 'Nie ten');
    await authored(store, 'aw-z-linked', '2026-05-01', 'Ten właśnie');
    await store.updateWorkout('u', 'aw-z-linked', {
    ...
    expect(data!.trainingComparison!.plannedWorkout?.name).toBe('Ten właśnie');
    expect(data!.trainingComparison!.plannedWorkoutStatus).toBe('linked');
  });

  it('finds the linked session even when it was scheduled on another day', async () => {
    // An id match is not subject to a day tolerance: the session slipped, it is still that session.
    const store = createMemoryStore();
    await store.putActivities('u', [act({ garminWorkoutId: 'g-1' })]);
    await authored(store, 'aw-late', '2026-04-27', 'Przesunięty');
    await store.updateWorkout('u', 'aw-late', {
      garminWorkoutId: 'g-1',
      pushState: 'pushed',
      updatedAt: '2026-09-01T10:00:00.000Z'
    });
    ...
    expect(data!.trainingComparison!.plannedWorkout?.name).toBe('Przesunięty');
  });

  it('changes nothing when the activity carries no id — the day rules again', async () => {
    // The degradation case: with no id anywhere, spec 085's ranking is what decides, untouched.
    const store = createMemoryStore();
    await store.putActivities('u', [act({})]);
    await authored(store, 'aw-same-day', '2026-05-01', 'Ten dnia');
    await authored(store, 'aw-elsewhere', '2026-04-27', 'Inny dzień');
    await store.updateWorkout('u', 'aw-elsewhere', {
      garminWorkoutId: 'g-1',
      pushState: 'pushed',
      updatedAt: '2026-09-01T10:00:00.000Z'
    });
    ...
    expect(data!.trainingComparison!.plannedWorkout?.name).toBe('Ten dnia');
  });
});
