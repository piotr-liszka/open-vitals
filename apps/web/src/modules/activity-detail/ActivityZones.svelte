<script lang="ts">
   ...
   *
   * Spec 090 gives both references a second state. FTP and max HR can now be SAVED, so the panel
   * branches per value: "taken from your profile" when there is one, "estimated, and here is how"
   * plus where to fix it when there is not. `hr.zoneMaxConfigured` is what carries that for heart
   * rate — the split is against the profile maximum whenever it exists.
   */
  ...
  const zones = $derived(buildHrZones(i18n.t, stats.hr.timeInZoneS, hr?.zones ?? []));

  /** True only when OUR %-of-max split is on screen AND it was taken from the saved maximum. */
  const hrMaxConfigured = $derived(zones?.source === 'estimated' && (hr?.zoneMaxConfigured ?? false));

  const subtitleKey = $derived<MessageKey>(
    zones?.source !== 'estimated'
      ? 'zones.subtitleDefault'
      : hrMaxConfigured
        ? 'zones.subtitleConfigured'
        : 'zones.subtitleEstimated'
  );
...
{#if zones || powerArcs.length > 0}
  <Card title={i18n.t('zones.title')} subtitle={i18n.t(subtitleKey)} overflowVisible>
    {#snippet actions()}
      <InfoPopover label={i18n.t('zones.explainLabel')} title={i18n.t('zones.explainTitle')} align="end">
        ...
        {:else if zones}
          ...
          {#if hrMaxConfigured && hr?.zoneMax != null}
            <p>{i18n.t('zones.hrConfiguredMax', { maxHr: formatInteger(i18n.locale, hr.zoneMax) })}</p>
          {:else}
            <p>{i18n.t('zones.hrEstimatedMax')}</p>
            <p>{i18n.t('zones.hrSetInSettings')}</p>
          {/if}
        {/if}
        ...
        {#if showPower}
          ...
          {/if}
          {#if ftpEstimated}
            <p>{i18n.t('zones.ftpSetInSettings')}</p>
          {/if}
        {/if}
      </InfoPopover>
    {/snippet}
    ...
    <div class="cols">
      {#if zones}
        <div class="block">
          <h4 class="block-title">{i18n.t('zones.hrBlockTitle')}</h4>
          <ul class="bars">
      ...
      {#if power && powerArcs.length > 0}
        <div class="block">
          <h4 class="block-title">{i18n.t('zones.powerBlockTitle')}</h4>
          ...
          <div class="donut-wrap" onpointerleave={() => (activeZone = null)}>
            <svg class="donut" viewBox="0 0 160 160" role="img" aria-label={i18n.t('zones.donutAriaLabel')}>
...
{#if power && power.curve.length > 0}
  <Card title={i18n.t('zones.powerTableTitle')} subtitle={i18n.t('zones.powerTableSubtitle')}>
    <Table zebra>
      {#snippet head()}
        <th>{i18n.t('plan.step.duration')}</th>
        <th class="num">{i18n.t('plan.step.power')}</th>
        {#if weightKg}<th class="num">W/kg</th>{/if}
      {/snippet}
      {#each power.curve as p (p.durationS)}
        <tr>
          <td>{durLabel(p.durationS)}</td>
          <td class="num">{fmtNum(p.watts, 0, i18n.locale)} W</td>
          {#if weightKg}<td class="num">{fmtNum(p.watts / weightKg, 1, i18n.locale)}</td>{/if}
        </tr>
      {/each}
    </Table>
  </Card>
