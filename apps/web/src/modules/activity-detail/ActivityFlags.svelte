...
{#if has}
  <Card title={i18n.t('flags.title')} subtitle={i18n.t('flags.subtitle')}>
    <div class="flags">
      {#if highlights.length > 0}
        <ul class="list" aria-label={i18n.t('flags.highlightsAriaLabel')}>
          {#each highlights as h (h.key)}
            <li class="row" class:record={h.kind === 'record'}>
              <div class="head">
                <span class="label">{h.label}</span>
                <Badge tone={h.kind === 'record' ? 'success' : 'info'}>
                  {h.kind === 'record' ? i18n.t('flags.record') : i18n.t('flags.notable')}
                </Badge>
              </div>
              <p class="value">
              ...
              <p class="rank">{i18n.t('flags.rankOf', { rank: h.rank, outOf: h.outOf })}</p>
            </li>
          {/each}
        </ul>
      {/if}
      ...
      {#if suspects.length > 0}
        <ul class="list suspects" aria-label={i18n.t('flags.suspectsAriaLabel')}>
          {#each suspects as s (s.key)}
            <li class="row" class:warn={s.severity === 'warn'}>
              <div class="head">
                <span class="label">{s.label}</span>
                <Badge tone={s.severity === 'warn' ? 'danger' : 'warning'}>
                  {s.severity === 'warn' ? i18n.t('flags.suspectSevere') : i18n.t('flags.suspectMild')}
                </Badge>
              </div>
