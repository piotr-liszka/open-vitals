...
export function buildLapTable(
  ...
  const fields: LapField[] = [
    ...
    {
      key: 'distance',
      label: t('laps.col.distance'),
      numeric: true,
      cell: (l) => (isNum(l.distanceM) ? `${fmtNum(l.distanceM / 1000, 2, t.locale)} km` : null),
      has: (l) => isNum(l.distanceM)
    },
    {
      key: 'duration',
      label: t('laps.col.duration'),
      numeric: true,
    ...
    paceSport
      ...
      : {
          key: 'speed',
          label: t('laps.col.speed'),
          numeric: true,
          cell: (l) => {
            const kmh = lapSpeedKmh(l);
            return kmh === null ? null : `${fmtNum(kmh, 1, t.locale)} km/h`;
          },
          has: (l) => lapSpeedKmh(l) !== null
        },
    {
      key: 'avgHr',
      label: t('laps.col.avgHr'),
      numeric: true,
      cell: (l) => (isNum(l.avgHr) ? `${fmtNum(l.avgHr, 0, t.locale)} bpm` : null),
      has: (l) => isNum(l.avgHr)
    },
    {
      key: 'maxHr',
      label: t('laps.col.maxHr'),
      numeric: true,
      cell: (l) => (isNum(l.maxHr) ? `${fmtNum(l.maxHr, 0, t.locale)} bpm` : null),
      has: (l) => isNum(l.maxHr)
    },
    {
      key: 'avgPower',
      label: t('laps.col.avgPower'),
      numeric: true,
      cell: (l) => (isNum(l.avgPower) ? `${fmtNum(l.avgPower, 0, t.locale)} W` : null),
      has: (l) => isNum(l.avgPower)
    },
    {
      key: 'cadence',
      label: t('laps.col.cadence'),
      numeric: true,
      cell: (l) => (isNum(l.avgRunCadenceSpm) ? fmtNum(l.avgRunCadenceSpm, 0, t.locale) : null),
      has: (l) => isNum(l.avgRunCadenceSpm)
    },
    {
      key: 'elevation',
      label: t('laps.col.elevation'),
      numeric: true,
      cell: (l) => (isNum(l.elevationGainM) ? `${fmtNum(l.elevationGainM, 0, t.locale)} m` : null),
      has: (l) => isNum(l.elevationGainM)
    },
    {
      key: 'calories',
      label: t('laps.col.calories'),
      numeric: true,
      cell: (l) => (isNum(l.calories) ? `${fmtNum(l.calories, 0, t.locale)} kcal` : null),
 ...
 */
export function buildSplitSummary(t: Translator, splits: readonly ActivityLap[]): SplitSummary[] {
