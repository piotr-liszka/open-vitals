...
import { alignPlannedStructure, flattenWorkoutSteps } from './activity-plan';
...
describe('ActivityStreamsPanel', () => {
  ...
  describe('the planned structure laid over the same axis (spec 085)', () => {
    ...
    it('draws the EXECUTED extent under the plan once the laps aligned (spec 091)', () => {
      // 20 s per step planned; the athlete actually ran 25 / 15 / 19 s.
      const aligned = alignPlannedStructure(structure, [
        { index: 1, durationS: 25, elapsedDurationS: 25 },
        { index: 2, durationS: 15, elapsedDurationS: 15 },
        { index: 3, durationS: 19, elapsedDurationS: 19 }
      ]);
      expect(aligned.status).toBe('aligned');

      const { container } = render(ActivityStreamsPanel, {
        props: { streams: runStreams, sport: 'run', plannedStructure: aligned.steps }
      });
      ...
      expect(Number.parseFloat(executed[1]?.style.left ?? '')).toBeCloseTo((25 / 59) * 100, 4);
    });

    it('draws no executed row at all when the laps could not be reconciled', () => {
      const { container } = render(ActivityStreamsPanel, {
        props: { streams: runStreams, sport: 'run', plannedStructure: structure }
      });
      expect(container.querySelectorAll('.strip')).toHaveLength(1);
      expect(container.textContent).not.toContain('Wykonanie');
    });
