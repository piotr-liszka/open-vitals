...
describe('buildLapTable', () => {
  it('is null when a table would say nothing the summary does not', () => {
    expect(buildLapTable(t, [], 'run')).toBeNull();
    expect(buildLapTable(t, [lap({ index: 1, distanceM: 5000 })], 'run')).toBeNull();
  });
...
describe('buildSplitSummary', () => {
  it('aggregates per class, longest first, keeping Garmin’s stretch count', () => {
    const summary = buildSplitSummary(t, [
  ...
  it('ignores rows with no usable duration', () => {
    expect(buildSplitSummary(t, [{ index: 1, type: 'RWD_RUN' }])).toEqual([]);
    expect(buildSplitSummary(t, [])).toEqual([]);
  });
});
