...
const plan = (over: Partial<PlannedWorkoutComparison>): PlannedWorkoutComparison => ({
  ...
  steps: [step({})],
  compliancePct: 100,
  intensitySource: 'session-average',
  ...over
});
...
describe('PlannedVsActual', () => {
  ...
  const rep = (over: Partial<PlannedStepActual>): PlannedStepActual => ({
    stepIndex: 0,
    repeatIndex: 1,
    repeatTotal: 5,
    targetLow: 240,
  ...
  const paceRow = (perStep: PlannedStepActual[] | null): PlannedStepComparison =>
    step({ key: 'pace', target: 245, targetLow: 240, targetHigh: 250, actual: 245, met: true, perStep });

  it('shows each rep under its metric when the laps aligned to the plan', () => {
    const { container } = render(PlannedVsActual, {
      props: {
        comparison: comparison({
          plannedWorkout: plan({
            intensitySource: 'per-step',
            steps: [
              paceRow([
                rep({ stepIndex: 0, repeatIndex: 1 }),
                rep({ stepIndex: 2, repeatIndex: 2, actual: 262, met: false })
              ])
            ]
          })
        })
    ...
    expect(cells).toHaveLength(3);
    expect(cells[0]).toEqual(['Tempo', '4:00 min/km – 4:10 min/km', '4:05 min/km', 'Zgodnie z planem']);
    expect(cells[1]).toEqual(['Praca 1/5', '4:00 min/km – 4:10 min/km', '4:05 min/km', 'Zgodnie z planem']);
    // A rep outside its own band is named, so the figure above can be checked rather than trusted.
    expect(cells[2]).toEqual(['Praca 2/5', '4:00 min/km – 4:10 min/km', '4:22 min/km', 'Poza planem']);
  });

  it('says which of the two comparisons produced the number on screen', () => {
    const perStep = render(PlannedVsActual, {
      props: {
        comparison: comparison({
          plannedWorkout: plan({ intensitySource: 'per-step', steps: [paceRow([rep({})])] })
        })
      }
    });
    expect(perStep.container.querySelector('.source')?.textContent).toContain(
      'okrążeń dopasowanych do kolejnych kroków planu'
    );

    cleanup();
    const average = render(PlannedVsActual, {
      props: {
        comparison: comparison({
          plannedWorkout: plan({ intensitySource: 'session-average', steps: [paceRow(null)] })
        })
      }
    });
    expect(average.container.querySelector('.source')?.textContent).toContain(
      'Nie udało się dopasować okrążeń'
    );
    expect(average.container.querySelectorAll('.per-step')).toHaveLength(0);
  });

  it('says nothing about alignment when the plan set no intensity band to align', () => {
    const { container } = render(PlannedVsActual, {
      props: { comparison: comparison({ plannedWorkout: plan({ steps: [step({})] }) }) }
    });
