...
export async function logEntry(
  ...
  const parsed = parseJournalEntry(body, today(deps), t);
  if (!parsed.ok) return { ok: false, status: 400, error: parsed.error };
  const input = parsed.value;

  if (input.activityId !== null) {
    const activity = await deps.store.getActivity(userId, input.activityId);
    if (!activity) return { ok: false, status: 400, error: t('journal.error.noSuchActivity') };
    const activityDay = activity.startTimeLocal.slice(0, 10);
    if (activityDay !== input.day) {
      return {
        ok: false,
        status: 400,
        error: t('journal.error.activityDayMismatch', { activityDay, entryDay: input.day })
      };
    }
  }
...
export async function removeEntry(
  ...
  if (!deleted) return { ok: false, status: 404, error: t('journal.error.noSuchEntry') };
