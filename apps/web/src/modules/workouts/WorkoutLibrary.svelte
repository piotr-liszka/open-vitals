<script lang="ts">
  ...
  const summary = (t: WorkoutTemplateView): string => {
    ...
    if (t.estimatedDistanceM !== null) bits.push(fmtDistance(i18n.t, t.estimatedDistanceM));
    return bits.join(' · ');
  };
</script>
...
<section class="library">
  <div class="head">
    <h3 class="title">{i18n.t('workout.library.heading')}</h3>
    {#if canWrite}
      <Button size="sm" variant="secondary" onclick={oncreate}>{i18n.t('workout.library.newButton')}</Button>
    {/if}
  </div>

  {#if templates.length === 0}
    <p class="empty">
      {i18n.t('workout.library.emptyBody')}{canWrite ? '' : i18n.t('workout.library.emptyWriteHint')}
    </p>
  {:else}
    <p class="hint">{i18n.t('workout.library.dragHint')}</p>
    <ul class="list">
      {#each templates as t (t.id)}
        ...
        >
          <div class="row">
            <div class="who">
              <Badge tone="neutral">{sportLabel(i18n.t, t.sport)}</Badge>
          ...
          <p class="steps">{describeSteps(i18n.t, t.steps)}</p>

          {#if canWrite}
            <div class="actions">
              <Button size="sm" variant="ghost" onclick={() => onschedule(t, selectedDay)}>
                {i18n.t('workout.library.scheduleFor', {
                  date: formatDay(i18n.locale, selectedDay, 'short')
                })}
              </Button>
              <Button size="sm" variant="ghost" onclick={() => onedit(t)}>{i18n.t('common.edit')}</Button>
              <Button size="sm" variant="ghost" onclick={() => ondelete(t)}>{i18n.t('common.delete')}</Button>
            </div>
          {/if}
        </li>
      {/each}
