...
import type { Adherence, MatchedBy } from '$lib/session-match';

export type { WorkoutStep, WorkoutPushState, Adherence, MatchedBy };

/**
 * What actually happened on a planned session (spec 081).
 *
 * DERIVED ON READ, from the same `matchWeek` the week review runs — never a second stored copy.
 * A stored flag would survive the activity being deleted or re-synced and leave a "done" behind
 * that nothing backs up.
 */
export interface WorkoutCompletion {
  readonly activityId: string;
  /** `done` at or above 90 % of what was planned, `shortened` below it. */
  readonly adherence: Adherence;
  /** Actual over planned on the plan's own axis; null when the plan had no distance and no time. */
  readonly adherenceRatio: number | null;
  /** Days between the planned day and the day it was done; 0 when on time. */
  readonly dayShift: number;
  /** `workout-id` = the watch linked it. `heuristic` = we inferred it. The UI must say which. */
...
export interface AuthoredWorkoutView {
  ...
  /** What was actually done against this session (spec 081), or null when nothing matched it. */
  readonly completion: WorkoutCompletion | null;
  /**
   * True when a same-day synced planned event was folded into this row (spec 093) — Garmin has
   * echoed this exact push back onto its own calendar. DERIVED ON READ by `matchPlannedEcho`, like
   * `completion` and `onGarmin`: no stored flag, so an echo that later disappears (an external
   * delete, a re-sync that drops the window) loses the badge on the very next load rather than
   * lying about a state that no longer holds.
   */
  readonly syncedBack: boolean;
}
