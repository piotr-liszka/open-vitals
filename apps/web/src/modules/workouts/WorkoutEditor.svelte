...
<form class="editor" onsubmit={submit}>
  {#if error}
    <!-- The server's own sentence, verbatim. `normalizeWorkout` already explains what is wrong; a
         ...
         eventually disagree. -->
    <Banner tone="danger" title={i18n.t('workout.editor.notSaved')}>{error}</Banner>
  {/if}

  <div class="meta">
    <Field label={i18n.t('workout.editor.nameLabel')}>
      {#snippet children(control)}
        <Input
          id={control.id}
          bind:value={title}
          maxlength={WORKOUT_LIMITS.maxTitle}
          placeholder={i18n.t('workout.editor.namePlaceholder')}
          autocomplete="off"
        />
      {/snippet}
    </Field>

    <Field label={i18n.t('workout.editor.sportLabel')}>
      {#snippet children(control)}
    ...
    {#if !isLibrary}
      <Field label={i18n.t('workout.editor.timeLabel')} help={i18n.t('workout.editor.timeHelp')}>
        {#snippet children(control)}
  ...
  {#if isLibrary && workout}
    ...
    <p class="scope-note">
      {i18n.t('workout.editor.libraryScopeNote')}
    </p>
  {/if}

  <div class="steps">
    <div class="steps-head">
      <h4>{i18n.t('workout.editor.stepsHeading')}</h4>
    ...
    {#snippet stepRow(s: DraftStep, list: DraftStep[], i: number, child: boolean)}
      <div class="step-bar" class:child>
        <select
          class="select kind"
          bind:value={s.kind}
          aria-label={i18n.t('workout.editor.stepKindAriaLabel')}
        >
          {#each kindsFor(child) as k (k)}
            <option value={k}>{stepKindLabel(i18n.t, k)}</option>
          {/each}
        </select>

        {#if s.kind === 'repeat'}
          <label class="inline">
            <span class="lbl">{i18n.t('workout.editor.repeatsLabel')}</span>
            <input class="num" type="number" min="1" max={WORKOUT_LIMITS.maxRepeats} bind:value={s.repeats} />
          </label>
        {:else}
          <select
            class="select"
            bind:value={s.durationType}
            aria-label={i18n.t('workout.editor.durationTypeAriaLabel')}
          >
            {#each WORKOUT_DURATION_TYPES as t (t)}
              <option value={t}>{durationTypeLabel(i18n.t, t)}</option>
            {/each}
          </select>
          ...
          {#if s.durationType !== 'lap'}
            <input
              ...
              aria-label={durationValueLabel(i18n.t, s.durationType)}
            />
          ...
          <select
            class="select"
            bind:value={s.targetType}
            aria-label={i18n.t('workout.editor.targetAriaLabel')}
          >
            {#each allowedTargets as t (t)}
              <option value={t}>{targetTypeLabel(i18n.t, t)}</option>
            {/each}
          </select>
          ...
          {#if s.targetType !== 'none'}
            <input
              class="num"
              type="number"
              bind:value={s.targetLow}
              aria-label={i18n.t('workout.editor.targetLowAriaLabel')}
            />
            <span class="dash">–</span>
            <input
              class="num"
              type="number"
              bind:value={s.targetHigh}
              aria-label={i18n.t('workout.editor.targetHighAriaLabel')}
            />
      ...
      <div class="step" class:block={step.kind === 'repeat'}>
        ...
        {#if step.kind === 'repeat'}
          <div class="children">
            ...
            {/each}
            <Button size="sm" variant="ghost" onclick={() => addStep(step.steps)}
              >{i18n.t('workout.editor.addStepInBlock')}</Button
            >
          </div>
        {/if}
      </div>
    ...
    <div class="adders">
      <Button size="sm" variant="secondary" onclick={() => addStep(steps)}
        >{i18n.t('workout.editor.addStep')}</Button
      >
      <!-- Top level only: `normalizeWorkout` refuses nested repeats, so no control may produce one. -->
      <Button size="sm" variant="secondary" onclick={addRepeat}>{i18n.t('workout.editor.addRepeat')}</Button>
    </div>
  </div>
  ...
  <Field label={i18n.t('workout.editor.noteLabel')}>
    {#snippet children(control)}
      <textarea
        ...
        bind:value={note}
        placeholder={i18n.t('workout.editor.notePlaceholder')}
      ...
      <p class="hint" id="{control.id}-sent">
        {i18n.t('workout.editor.noteHint')}
      </p>
    {/snippet}
  </Field>

  <div class="actions">
    <Button type="button" size="sm" variant="ghost" onclick={oncancel}>{i18n.t('common.cancel')}</Button>
    <Button type="submit" size="sm" variant="primary" disabled={!canSubmit}>
      {saving
        ? i18n.t('workout.editor.saving')
        : workout
          ? i18n.t('workout.editor.saveChanges')
          : i18n.t('workout.editor.addWorkout')}
    </Button>
  </div>
</form>
