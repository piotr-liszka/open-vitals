...
import { ALL_SPORTS, WEEKLY_SUMMARY_WEEKS } from './weekly-summary.types';
...
describe('loadWeeklySummary', () => {
  it('returns the documented contract for the fixed 12-week window', async () => {
    ...
    const data = await loadWeeklySummary(deps(store), { userId: 'u', locale: 'pl' });
    ...
    // One family only: no "Wszystko" to open on, so the card still opens on the sport (spec 089).
  ...
  it('labels the x axis where the month changes and leaves the rest blank', async () => {
    const data = await loadWeeklySummary(deps(await seeded([])), { userId: 'u', locale: 'pl' });
  ...
  it('orders families by training time in the window and defaults to the busiest', async () => {
    ...
    // Chip ORDER is still busiest-first; only the thing the card OPENS on moved to "all" (spec 089).
    expect(data.defaultGroup).toBe(ALL_SPORTS);
  ...
  it('resolves the week boundary in the configured timezone, not UTC', async () => {
    ...
    const warsaw = await loadWeeklySummary(deps(store, mondayNight, 'Europe/Warsaw'), {
      userId: 'u',
      locale: 'pl'
    });
    ...
    const utc = await loadWeeklySummary(deps(store, mondayNight, 'UTC'), { userId: 'u', locale: 'pl' });
  ...
  it('never leaks another user’s training (AGENTS.md §2 rule 2)', async () => {
    ...
    const mine = await loadWeeklySummary(deps(store), { userId: 'a', locale: 'pl' });
    expect(mine.sports.map((s) => s.group)).toEqual(['run']);
    expect(mine.sports[0]?.thisWeek.distanceM).toBe(5_000);

    const theirs = await loadWeeklySummary(deps(store), { userId: 'b', locale: 'pl' });
  ...
  it('scopes the store read to the window and to the user', async () => {
    ...
    await loadWeeklySummary(deps(store as unknown as LocalStore), { userId: 'u-42', locale: 'pl' });
  ...
  it('adds every family into `combined`, one entry per week in the lattice (spec 089)', async () => {
    const store = await seeded([
      act({ startTimeLocal: '2026-08-11 19:00:00', distanceM: 10_000, movingS: 3_000, elevationGainM: 80 }),
      act({
        sport: 'cycling',
        startTimeLocal: '2026-08-11 17:00:00',
        distanceM: 60_000,
        movingS: 5_400,
        elevationGainM: 640
      }),
      act({
        sport: 'cycling',
        startTimeLocal: '2026-07-28 17:00:00',
        distanceM: 30_000,
        movingS: 3_600,
        elevationGainM: 400
      })
    ...
    expect(data.defaultGroup).toBe(ALL_SPORTS);
    expect(combined?.weekly).toHaveLength(WEEKLY_SUMMARY_WEEKS);
    expect(combined?.weekly.map((w) => w.week)).toEqual(data.weekStarts);
    expect(combined?.weekly.filter((w) => w.partial).map((w) => w.week)).toEqual([THIS_MONDAY]);

    // This week is the ride plus the run — time, climb and sessions.
    expect(combined?.thisWeek).toEqual({
      activities: 2,
      distanceM: 70_000,
      durationS: 8_400,
      elevationGainM: 720
    });
    // …and it agrees, field for field, with the per-sport chips it was folded from.
    const sum = (pick: (t: { durationS: number; activities: number }) => number): number =>
      data.sports.reduce((acc, sp) => acc + pick(sp.window), 0);
    expect(combined?.window.durationS).toBe(sum((t) => t.durationS));
    expect(combined?.window.activities).toBe(sum((t) => t.activities));

    // JSON-serialisable, like the rest of the payload — this crosses the SvelteKit load boundary.
    const roundTripped = JSON.parse(JSON.stringify(data)) as typeof data;
    expect(roundTripped.combined?.weekly).toHaveLength(WEEKLY_SUMMARY_WEEKS);
    expect(roundTripped.defaultGroup).toBe('all');
  });

  it('reads the store once for every family and the combined series alike', async () => {
    const store = await seeded([
      act({ startTimeLocal: '2026-08-11 19:00:00' }),
      act({ sport: 'cycling', startTimeLocal: '2026-08-11 17:00:00' })
    ]);
    let reads = 0;
    const counted = {
      listActivities: async (userId: string, query?: unknown) => {
        reads += 1;
        return store.listActivities(userId, query as never);
      }
    };

    const data = await loadWeeklySummary(deps(counted as unknown as LocalStore), {
      userId: 'u',
      locale: 'pl'
    });

    expect(reads).toBe(1);
    expect(data.combined).not.toBeNull();
  });

  it('leaves `combined` null for a one-sport athlete, whatever the volume', async () => {
    const store = await seeded([
      act({ startTimeLocal: '2026-08-11 07:00:00' }),
      act({ startTimeLocal: '2026-08-12 07:00:00' }),
      act({ startTimeLocal: '2026-07-15 07:00:00' })
    ]);
