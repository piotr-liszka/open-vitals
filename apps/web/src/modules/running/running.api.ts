...
import { dayRange, lastMonths, monthKeyOf, toDayKey, todayKey } from '$lib/date';
...
export async function loadRunning(deps: RunningDeps, req: RunningRequest): Promise<RunningData> {
  const t = createTranslator(req.locale ?? DEFAULT_LOCALE);
  ...
  const [effortsNow, effortsThen, progression] = await Promise.all([
    ...
    // The record progression (spec 087): every effort that WAS a record when it was set. Unbounded
    // in time on purpose — bounding it would lose the standing record the window starts from — and
    // a handful of rows per distance either way, so it costs less than the two reads above.
    deps.store.listBestEffortProgression(req.userId, { sports: runSports })
  ]);
  ...
  /*
   * The shape BETWEEN those two endpoints (spec 087). No extra store read and no stored history: the
   * fastest-effort-so-far is a step function whose steps are `progression`, and the projection side
   * decomposes the same way — `personalBests` is a per-distance minimum over independent runs, so
   * per-run projections merged forward reproduce it exactly on every day.
   *
   * Deliberately without a critical speed, exactly like `previousPredictions` above: reconstructing
   * the past speed–duration curve would mean re-reading up to SPEED_STREAM_CAP streams PER DAY.
   */
  const history = predictionHistory({
    days: dayRange(range.start, range.end),
    efforts: progression,
    projections: runs.flatMap((r) => personalBests([r], t))
  });
