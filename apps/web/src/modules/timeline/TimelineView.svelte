<script lang="ts">
  ...
  let { data, connected = true }: Props = $props();
  ...
  const ORIENTATION_OPTIONS = $derived([
    { value: 'vertical', label: i18n.t('timelineView.vertical') },
    { value: 'horizontal', label: i18n.t('timelineView.horizontal') }
  ]);
  ...
  function dayLabel(day: DayKey, today: DayKey): string {
    ...
    if (delta === 0) return i18n.t('timelineView.today');
    if (delta === 1) return i18n.t('timelineView.yesterday');
    return formatDay(i18n.locale, day, 'weekday');
  }
   ...
   * watch by definition.
   */
  const PUSH_BADGE = $derived<
    Record<string, { tone: 'neutral' | 'success' | 'warning' | 'danger'; label: string }>
  >({
    pending: { tone: 'neutral', label: i18n.t('timelineView.push.pending') },
    pushed: { tone: 'success', label: i18n.t('timelineView.push.pushed') },
    failed: { tone: 'warning', label: i18n.t('timelineView.push.failed') },
    unsupported: { tone: 'danger', label: i18n.t('timelineView.push.unsupported') }
  });
...
<Card
  title={i18n.t('timelineView.title')}
  subtitle={i18n.t('timelineView.subtitle', { past: windowDays, future: plannedDays })}
>
  {#snippet actions()}
    {#if connected && data !== null}
      <SegmentedControl
        ...
        ariaLabel={i18n.t('timelineView.orientation')}
        size="sm"
  ...
  {#if !connected}
    <p class="note">{i18n.t('timelineView.notConnected')}</p>
    <a class="link" href="/settings">{i18n.t('timelineView.connectCta')}</a>
  {:else if data === null}
    <p class="note">{i18n.t('timelineView.notEnoughData')}</p>
  {:else if orientation === 'horizontal'}
    ...
    <div class="timeline" data-orientation="horizontal">
      {#if axisGroups.length === 0 && data.planned.status !== 'ok'}
        <p class="note">
          {i18n.t('timelineView.emptyPast', { days: windowDays })}
        </p>
      {:else}
        <!--
        ...
        <div
          ...
          role="group"
          tabindex="0"
          aria-label={i18n.t('timelineView.axisAriaLabel', { days: windowDays })}
          bind:this={track}
          ...
          <ol class="track">
            {#each axisGroups as group (group.day)}
              <li class="col" class:is-today={group.day === data.today}>
                <div class="col-head">
                  ...
                  <span class="col-date">{formatDay(i18n.locale, group.day, 'numeric')}</span>
                </div>
            ...
            {#if data.planned.status === 'ok'}
              {#each data.planned.events as event (event.id)}
                <li class="col ahead">
                  <div class="col-head">
                    <span class="col-label">{formatDay(i18n.locale, event.day, 'weekday')}</span>
                    <span class="col-date">{event.time ?? i18n.t('timelineView.planFallback')}</span>
                  </div>
                  <div class="rule" aria-hidden="true"><span class="tick"></span></div>
            ...
            {:else}
              <li class="col ahead">
                <div class="col-head">
                  <span class="col-label">{i18n.t('timelineView.nextHeading')}</span>
                  <span class="col-date">{plannedDays} {i18n.t('timeline.unit.days')}</span>
                </div>
                <div class="rule" aria-hidden="true"><span class="tick"></span></div>
                <div class="col-events">
                  {#if data.planned.status === 'empty'}
                    <p class="empty-title">{i18n.t('timelineView.noPlanned')}</p>
                    <p class="note">{i18n.t('timelineView.noPlannedBody', { days: plannedDays })}</p>
                  {:else}
                    <p class="empty-title">{i18n.t('timelineView.plannedNotSynced')}</p>
                    <p class="note">
                      {i18n.t('timelineView.plannedNotSyncedBodyShort')}
                    </p>
                  {/if}
                </div>
              </li>
        ...
        {#if hiddenCount > 0}
          <div class="expander">
            <Button size="sm" variant="ghost" aria-expanded={expanded} onclick={toggleExpanded}>
              {expanded
                ? i18n.t('timelineView.showPrimary')
                : i18n.t('timelineView.showAll', { count: data.past.totalCount })}
            </Button>
            {#if !expanded}
              <span class="expander-note">{i18n.t('timelineView.hiddenCount', { count: hiddenCount })}</span>
            {/if}
          </div>
        {/if}
      {/if}
  ...
  {:else}
    ...
    <div class="timeline" data-orientation="vertical">
      <section class="half" aria-labelledby="timeline-past">
        <h4 class="half-title" id="timeline-past">{i18n.t('timelineView.pastHeading')}</h4>
        ...
        {:else}
            ...
            {#each groups as group (group.day)}
              <li class="group">
                <div class="day">
                  ...
                  <span class="day-date">{formatDay(i18n.locale, group.day, 'numeric')}</span>
                </div>
                <ul class="events">
                  {#each group.events as event (event.id)}
          ...
          {#if hiddenCount > 0}
            <div class="expander">
              ...
              {#if !expanded}
                <span class="expander-note">
                  {i18n.t('timelineView.hiddenCount', { count: hiddenCount })}
                </span>
              {/if}
            </div>
          {/if}
      ...
      <section class="half planned" aria-labelledby="timeline-next">
        <h4 class="half-title" id="timeline-next">{i18n.t('timelineView.nextHeading')}</h4>
          ...
          <ol class="rail">
            {#each data.planned.events as event (event.id)}
              <li class="group">
                <div class="day">
                  <span class="day-node" aria-hidden="true"></span>
                  <span class="day-label">{formatDay(i18n.locale, event.day, 'weekday')}</span>
          ...
          <div class="empty">
            ...
            <div class="empty-body">
              {#if data.planned.status === 'empty'}
                <p class="empty-title">{i18n.t('timelineView.noPlanned')}</p>
                <p class="note">
                  {i18n.t('timelineView.noPlannedBody', { days: plannedDays })}
                </p>
              {:else}
                <p class="empty-title">{i18n.t('timelineView.plannedNotSynced')}</p>
                <p class="note">
                  {i18n.t('timelineView.plannedNotSyncedBody')}
                </p>
              {/if}
            </div>
          </div>
