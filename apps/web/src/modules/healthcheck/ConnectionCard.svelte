...
<Card title="Garmin" subtitle={i18n.t('connection.fullSubtitle')}>
  {#snippet actions()}
    {#if onRefresh}
      <Button size="sm" variant="ghost" loading={refreshing} onclick={onRefresh}
        >{i18n.t('common.refresh')}</Button
      >
    {/if}
  {/snippet}
  ...
  <div class="body">
    ...
    <dl class="meta">
      {#if status.expiresAt}
        <div>
          <dt>{i18n.t('connection.sessionValidUntil')}</dt>
          <dd>{formatInstant(i18n.locale, status.expiresAt, 'dateTime')}</dd>
        </div>
      {/if}
      {#if !status.reachable}
        <div>
          <dt>{i18n.t('connection.detailsLabel')}</dt>
          <dd>{i18n.t('connection.unreachableDetail')}</dd>
        </div>
      {/if}
    </dl>
    ...
    {#if status.connected}
      {#if onDisconnect}
        <div class="row">
          {#if confirming}
            <span class="confirm-q">{i18n.t('connection.disconnectConfirm')}</span>
          ...
          {:else}
            <Button size="sm" variant="secondary" onclick={() => (confirming = true)}
              >{i18n.t('connection.disconnectGarmin')}</Button
            >
          {/if}
        </div>
      {/if}
