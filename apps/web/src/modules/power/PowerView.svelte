<script lang="ts">
  ...
  function fmtDur(s: number): string {
    ...
    return Number.isInteger(h)
      ? `${h}h`
      : `${formatNumber(i18n.locale, h, { minimumFractionDigits: 1, maximumFractionDigits: 1 })}h`;
  }
  const fmtWkg = (n: number): string =>
    formatNumber(i18n.locale, n, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  const wkg = (n: number | null): string => (n == null ? '—' : `${fmtWkg(n)} W/kg`);
  ...
  const curveSeries = $derived<ChartSeries[]>([
    {
      name: i18n.t('power.allTimeSeriesName'),
...
<div class="power">
  ...
  {:else}
    <div class="grid-2">
      <Card
        title={i18n.t('power.riderTypeTitle')}
        subtitle={data.weightKg
          ? i18n.t('power.riderTypeSubtitleWeight', { weight: data.weightKg })
          : i18n.t('power.riderTypeSubtitleUnknown')}
      >
        <div class="radar-wrap">
          <RadarChart axes={radarAxes} ariaLabel={i18n.t('power.riderRadarAriaLabel')} />
        </div>
        <dl class="axis-list">
      ...
      <Card title={i18n.t('power.ftpZonesTitle')}>
        <div class="ftp">
          <div class="ftp-main">
            ...
            <span class="ftp-sub"
              ...
              {#if data.ftpSource === 'settings'}<Badge tone="info">{i18n.t('power.ftpFromSettings')}</Badge
                >{:else if data.ftpSource === 'estimated'}<Badge tone="neutral"
                  >{i18n.t('power.ftpEstimated')}</Badge
                >{/if}
            </span>
          </div>
          <div class="ftp-side">
            <div>
              <span class="k">{i18n.t('power.best20MinLabel')}</span><span class="v"
                >{data.best20MinWatts ?? '—'} W</span
              >
            </div>
            <div>
              <span class="k">{i18n.t('power.best60MinLabel')}</span><span class="v"
                >{data.best60MinWatts ?? '—'} W</span
              >
            </div>
          </div>
        </div>
      ...
      <div class="tiles">
        {#each data.bests as b (b.durationS)}
          <div class="tile">
            ...
            <span class="t-meta"
              >{b.wattsPerKg != null ? `${fmtWkg(b.wattsPerKg)} W/kg · ` : ''}{fmtDate(b.day)}</span
            >
          </div>
        {/each}
      </div>
    ...
    {#if data.yearCurves.length > 0}
      <Card title={i18n.t('power.compareTitle')} subtitle={i18n.t('power.compareSubtitle')}>
        <TrendChart
          ...
          height={320}
          unit="W"
          label={i18n.t('power.curveLabel')}
          formatValue={(n) => `${Math.round(n)} W`}
        />
      </Card>

      {#if tableCols.length > 0}
        <Card title={i18n.t('power.yearlyBestsTitle')}>
          <Table zebra caption={i18n.t('power.tableCaption')}>
            {#snippet head()}
              <th scope="col">{i18n.t('power.yearColumnHeader')}</th>
