<script lang="ts">
  ...
  import { formatNumber, formatInteger, getI18n } from '$lib/i18n';
  ...
  const nf = (n: number): string => formatInteger(i18n.locale, n);
...
>
  ...
  {:else}
    <p class="headline">
      <span class="big">{nf1(mix.easyPct)}<small>%</small></span>
    ...
    {#if hasMinutes}
      <section class="minutes">
        ...
        <p class="hint">
          ...
          {#if weeksOnTarget > 0}
            Cel osiągnięty w {nf(weeksOnTarget)} z {nf(weeks.length)} tygodni.
          {:else}
        ...
        <BarChart
          ...
          formatValue={(v) => `${nf(v)} min`}
        />
      </section>
    {/if}

    <p class="note">
      Podział z tętna średniego wobec maksymalnego {nf(mix.maxHr ?? 0)} bpm: poniżej 80% spokojnie, do 87% średnio,
      wyżej mocno. Udziały liczymy z czasu, który dało się zaklasyfikować — jednostka bez pomiaru tętna nie jest
      wliczana jako spokojna.
      {#if mix.unclassifiedSessions > 0}
        Pominięto {nf(mix.unclassifiedSessions)}
