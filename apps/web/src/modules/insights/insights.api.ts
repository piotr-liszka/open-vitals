...
import {
  ...
  type GarminMetricDay,
  type GarminService
} from '$lib/server/interfaces';
import { METRICS, extractMetricValue, inner, pick } from '$lib/server/garmin/metric-specs';
...
import { computeReadiness, DEFAULT_READINESS_CONFIG, type ReadinessInputs } from './insights.readiness';
...
import { DEFAULT_LOCALE, createTranslator, type Locale } from '$lib/i18n';
...
export interface InsightsDeps {
  ...
  /**
   * Narrowest clock that works: this loader only ever asks for `now()`. Taking the full `Clock` (with
   * its JWT-oriented `nowSeconds`) shut the MCP surface out of reusing it, whose `ToolContext` carries
   * a `NowSource` — and reusing it is the point of spec 084.
   */
  clock: NowSource;
...
export interface LoadInsightsOptions {
  ...
  window?: number;
  /**
   * Language the assembled SENTENCES render in (spec 076) — the condition summary and the Garmin
   * readiness verdict are prose, so they follow the reader, not the process. Defaults to Polish for
   * callers that produce no prose (tests, the MCP surface).
   */
  locale?: Locale;
}
...
/**
 * Nightly Garmin sleep SCORES (spec 084) — the sleep and sleep-history fallbacks.
 *
 * Not the same series as the `sleep` metric, whose extracted value is a duration. Seven hours of poor
 * sleep and seven hours of good sleep are the same number there and 40 points apart here, which is why
 * the fallback reads the score rather than reusing what is already extracted.
 */
function nightlySleepScores(raw: readonly GarminMetricDay[]): DayPoint[] {
  return raw.map((day) => ({
    date: day.date,
    value: pick(inner(day.data), ['dailySleepDTO.sleepScores.overall.value', 'sleepScores.overall.value'])
  }));
}
...
export async function loadInsights(
  ...
  // One translator for the whole load: metric labels, trend/anomaly/correlation phrasing and the
  // condition narrative must all agree on the caller's language (spec 076 follow-up).
  const t = createTranslator(opts.locale ?? DEFAULT_LOCALE);
  ...
  const charts: MetricChart[] = seriesList.map(({ spec, days }) => {
    ...
    const stats = summarizeMetric(spec, days, t);
  ...
  /*
   * Readiness is assembled here, from Garmin's own factors plus the LIVE recovery countdown (spec
   * 084). Nothing below is a new store read: the Training Readiness and HRV payloads were already
   * fetched above for the charts, and spec 084's whole point is that the answer was in them all along.
   *
   * The timer is fed as live remaining minutes, never the frozen `recoveryTimeFactorPercent`: it runs
   * down whether or not we sync, so a channel stuck at the morning's value would keep suppressing the
   * score for hours after Garmin had stopped suppressing its own (spec 075).
   */
  ...
  const recovery = markSuperseded(parsedGarmin?.recovery ?? null, latestActivityStartMs);
  const recoveryMinutes = recovery ? remainingMinutes(recovery, nowMs) : null;

  const hrvSeries = seriesList.find((s) => s.spec.key === 'hrv');
  const sleepSeries = seriesList.find((s) => s.spec.key === 'sleep');
  const stressSeries = seriesList.find((s) => s.spec.key === 'stress');

  const readinessInputs: ReadinessInputs = {
    garmin: parsedGarmin,
    hrv: hrvSeries ? latestHrvStatus(hrvSeries.raw) : null,
    recoveryMinutes,
    // The `hrv` metric's own extracted value IS `hrvSummary.lastNightAvg` (metric-specs.ts), so the
    // series the charts drew is exactly the nightly series the HRV projection needs.
    hrvNights: hrvSeries?.days,
    sleepScores: sleepSeries ? nightlySleepScores(sleepSeries.raw) : undefined,
    stressDays: stressSeries?.days,
    today,
    nowMs,
    recoverySuperseded: recovery?.superseded ?? false,
    recoveryEndsAt: recovery?.endsAt ?? null
  };
  const readiness = computeReadiness(readinessInputs, DEFAULT_READINESS_CONFIG);
