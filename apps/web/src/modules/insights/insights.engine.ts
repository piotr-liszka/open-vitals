...
function phraseCorrelation(t: Translator, aLabel: string, bLabel: string, r: number): string {
  return t(r >= 0 ? 'insights.correlationHigher' : 'insights.correlationLower', { a: aLabel, b: bLabel });
}
...
export function computeCorrelations(
  ...
  for (const pair of config.correlationPairs) {
    ...
    const aLabel = metricLabel(t, aEntry.spec.key);
    const bLabel = metricLabel(t, bEntry.spec.key);
    out.push({
      a: pair.a,
      b: pair.b,
      aLabel,
      ...
      phrasing: phraseCorrelation(t, aLabel, bLabel, r)
    });
  }
...
export function computeInsights(
  ...
  return {
    readiness,
    trends: computeTrends(series, config, t),
    anomalies: computeAnomalies(series, config, t),
    correlations: computeCorrelations(series, config, t)
  };
}
