 ...
 *
 * `t` is optional so every existing caller/test keeps compiling with the Polish label it always
 * got; the one real caller (`insights.api.ts`) passes the request's own translator.
 */
