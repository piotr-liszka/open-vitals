...
import { createTranslator, DEFAULT_LOCALE, formatInteger, type MessageKey, type Translator } from '$lib/i18n';
import type {
  ConditionMetric,
  ConditionSnapshot,
  DayPoint,
 ...
 */
export function buildConditionMetric(
  spec: MetricSpec,
...
const STATE_HEAD: Readonly<Record<RecoveryState, MessageKey>> = {
  rested: 'condition.head.rested',
  steady: 'condition.head.steady',
  strained: 'condition.head.strained',
  unknown: 'condition.head.unknown'
};

function channelClause(t: Translator, c: ConditionMetric): string {
  const where = t(c.direction === 'up' ? 'condition.aboveBaseline' : 'condition.belowBaseline');
  const value =
    c.format === 'duration'
      ? (fmtSleepDuration(c.latest) ?? '—')
      : formatInteger(t.locale, Math.round(c.latest));
 ...
 */
export function summaryClauses(
  t: Translator,
  channels: readonly ConditionMetric[],
  ...
  const movers = channels
    ...
    .slice(0, 2)
    .map((c) => channelClause(t, c));

  const sleepClause =
    sleep === null ? null : t('condition.sleepClause', { duration: fmtSleepDuration(sleep.totalS) ?? '—' });
...
export function conditionSummary(
  ...
  const head = t(STATE_HEAD[state]);
  if (state === 'unknown') return `${head} — ${t('condition.unknownTail')}`;

  const parts = summaryClauses(t, channels, sleep);
  ...
  if (stale !== null && stale.days > 0 && isDayKey(stale.day)) {
    parts.unshift(t('condition.dataFrom', { day: formatDay(t.locale, stale.day) }));
  }
...
export function computeCondition(
  ...
  for (const key of CONDITION_KEYS) {
    ...
    const metric = buildConditionMetric(spec, entry.days, t);
    if (metric) channels.push(metric);
  }

  const sleepSpec = sleepSeries?.spec ?? SPEC_BY_KEY.get('sleep');
  const sleepChannel = sleepSeries && sleepSpec ? buildConditionMetric(sleepSpec, sleepSeries.days, t) : null;
  ...
  const clauses = summaryClauses(t, interpreted, sleep);
