...
<script lang="ts">
  ...
  interface Props {
    ...
    now?: number | null;
    /** The "where this number comes from" explainer, rendered right beside the score itself. */
    popover?: Snippet;
  }

  let { readiness, size = 'md', garminScore = null, now = null, popover }: Props = $props();
    ...
    <ul class="limits" aria-label={i18n.t('readiness.limitsAriaLabel')}>
      {#each limits as l (l.key)}
        <li>
          ...
          {#if l.minutes !== undefined}<span class="limit-meta">{fmtRecovery(i18n.t, l.minutes)}</span>{/if}
