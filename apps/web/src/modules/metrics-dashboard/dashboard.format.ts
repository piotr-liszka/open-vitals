 ...
 */
import { formatInteger, formatDecimals, DEFAULT_LOCALE, type Locale } from '$lib/i18n';
...
/**
 * Render a metric value, or `null` when there is nothing to show (the tile prints "—"). `locale`
 * defaults to Polish so every existing caller (none of which pass one yet) keeps its exact prior
 * output.
 */
export function formatMetricValue(
  ...
  switch (format) {
    case 'int':
      return formatInteger(locale, Math.round(n));
    case 'decimal':
      return formatDecimals(locale, n, 1);
    case 'duration': {
...
export function formatMetricText(n: number, format: MetricFormat, locale: Locale = DEFAULT_LOCALE): string {
  return String(formatMetricValue(n, format, locale) ?? '—');
}
