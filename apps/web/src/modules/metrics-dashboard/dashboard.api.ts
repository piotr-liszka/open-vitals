...
import { metricLabelKey } from '$lib/metric-labels';
import {
...
export interface DashboardOptions {
   ...
   */
  range?: ResolvedRange;
  /** Locale the headline value is rendered in (spec 076). Defaults to Polish. */
  locale?: Locale;
}
...
async function buildTile(
  ...
  const base: MetricTile = {
    key: spec.key,
    // The NAME is not carried in the payload (spec 076): it is `metric.<key>` in the catalog, and the
    // component renders it in the reader's language rather than the one the loader happened to run in.
    labelKey: metricLabelKey(spec.key) ?? 'metric.steps',
    accent: spec.accent,
    value: null,
    unit: spec.unit,
    delta: null,
  ...
  try {
    ...
    return {
      ...base,
      value: formatMetricValue(headlineOf(daily), spec.format, locale),
      series,
...
export async function loadDashboard(
  ...
  const tiles = await Promise.all(SNAPSHOT.map((spec) => buildTile(garmin, spec, range, dayLattice, locale)));
