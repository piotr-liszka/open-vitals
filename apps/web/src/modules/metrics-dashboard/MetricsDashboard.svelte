<script lang="ts">
  import { StatTile, RangeBadge, TrendChart } from '$lib/ui';
  ...
  let { data, updatedLabel, busy = false }: Props = $props();

  // `data.date` is a calendar day key — formatted as a day, never re-parsed as an instant (spec 018).
  const dateLabel = $derived(formatDay(i18n.locale, data.date, 'long'));
  ...
  const dayLabels = $derived(data.days.map((d) => bucketAxisLabel(i18n.locale, d, data.range.bucket)));
...
<section class="dash" aria-label={i18n.t('dash.ariaLabel')}>
  <header class="head">
    <div>
      <h2 class="h">{i18n.t('dash.today')}</h2>
      <p class="sub">{i18n.t('dash.snapshotOf', { date: dateLabel })}</p>
    </div>
    <div class="head-meta">
      {#if updatedLabel}
          ...
          {#snippet sparkline()}
            {#if definedCount(tile.series) > 1}
              ...
              <TrendChart
                ...
                legend={false}
                showArea
                formatValue={(n) => formatMetricText(n, tile.format, i18n.locale)}
              />
            {/if}
          {/snippet}
        </StatTile>
