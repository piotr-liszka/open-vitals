...
import { matchWeek, type CompletedActivity, type MatchResult, type PlannedSession } from '$lib/session-match';
...
export async function loadWeekReview(
  ...
  const planned: PlannedSession[] = workouts.map((w) => {
    ...
    return {
      ...
      prescribedAxis: workoutPrescribedAxis(steps),
      // Set once the push phase has created the session on Garmin (spec 050); null until then, and
      // forever for a session that never reaches Garmin. Pass 0 of the matcher joins on it.
