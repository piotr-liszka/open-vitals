        ...
        <div class="head">
          ...
          {#if p.trend && delta !== null}
            <DeltaBadge
              ...
              label={delta === 0
                ? `${p.label}: bez zmian od ${formatDay(i18n.locale, p.trend.sinceDay, 'shortYear')}`
                : `${p.label}: ${delta > 0 ? 'szybciej' : 'wolniej'} o ${fmtDur(Math.abs(delta))} niż ${formatDay(i18n.locale, p.trend.sinceDay, 'shortYear')}`}
            />
          {/if}
        </div>
