<script lang="ts">
  ...
  async function schedule(template: WorkoutTemplateView, onDay: DayKey): Promise<void> {
    ...
    toasts.success(`Zaplanowano „${template.title}” na ${formatDay(i18n.locale, onDay, 'short')}`);
  }
  ...
  async function pushNow(w: AuthoredWorkoutView): Promise<void> {
    ...
    try {
      ...
      if (!res.ok) {
        toasts.error(body?.error ?? 'Nie udało się wysłać treningu');
        return;
      }
      await invalidateAll();
      if (body?.pushState === 'pushed') toasts.success(`Wysłano „${w.title}” na Garmina`);
      ...
      else toasts.error(body?.pushError ?? 'Garmin nie przyjął tego treningu');
    } finally {
      pushing = null;
    }
  }
  ...
  <Card>
    ...
    {#if !data.canWrite}
      ...
      <Banner tone="info" title="Tryb tylko do odczytu">
        Zapis treningów jest wyłączony. Włącz „Zapis treningów do Garmina” na karcie Garmin w <a
          href="/settings">Ustawieniach</a
        >, aby dodawać i edytować sesje tutaj.
      </Banner>
    {/if}
    ...
    {:else}
        ...
        <article class="session">
          ...
          {#if data.canWrite}
            <div class="session-actions">
              ...
              <!-- Spec 083. A session already in Garmin's library AND on its calendar has nothing to
                   push, so it says so rather than offering a button that would be a no-op. -->
              {#if w.pushState === 'pushed'}
                <span class="on-garmin">Na Garminie</span>
              {:else}
                <Button size="sm" variant="secondary" disabled={pushing !== null} onclick={() => pushNow(w)}>
                  {pushing === w.id ? 'Wysyłam…' : 'Wyślij na Garmina'}
                </Button>
              {/if}
              {#if w.onGarmin && w.pushState !== 'pushed'}
                <!-- Known limitation, said plainly rather than discovered on the watch: the push is
                     ...
                     that was first sent. -->
                <span class="stale">Na Garminie może być starsza wersja</span>
              {/if}
            </div>
          {/if}
        </article>
      ...
      {#each dayPlanned as p (p.id)}
        ...
        <article class="session synced">
          <div class="session-head">
            <div class="who">
              <span class="sport">{p.sport ? sportLabel(i18n.t, p.sport) : 'Z kalendarza'}</span>
