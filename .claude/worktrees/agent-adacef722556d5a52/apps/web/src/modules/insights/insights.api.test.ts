...
interface FakeOpts {
  ...
  /**
   * Newest day Garmin holds anything for (spec 072). Days after it come back as gaps — a watch that
   * stopped uploading, which is indistinguishable upstream from an account that simply has no data.
   */
  dataThrough?: string;
}
  ...
  const svc: GarminService & { rangeCalls: number } = {
    ...
    async getMetricRange(name: GarminMetricName, start: string, end: string): Promise<GarminMetricRange> {
      ...
      const days = eachDate(start, end).map((date, i) => ({
        date,
        data: missing || (opts.dataThrough != null && date > opts.dataThrough) ? null : payload(name, i)
      }));
...
function deps(garmin: GarminService) {
  ...
  return { c, garmin: c.garminFor(USER), clock };
}
...
describe('loadInsights', () => {
  ...
  it('returns a full populated payload', async () => {
    ...
    /*
     * Readiness end to end (spec 084): five of Garmin's factors, sleep history derived from the
     * nightly sleep scores because this fixture omits that factor, and recovery computed from the
     * timer rather than read from `recoveryTimeFactorPercent`.
     *
     * 70·.25 + 53·.25 + 40·.20 + 65·.15 + 80·.10 + 55·.05 = 59.25 → composite 59, capped to the
     * recovery channel's 53 by the 2040-minute timer.
     */
    expect(data.readiness).not.toBeNull();
    expect(data.readiness!.drivers.length).toBe(6);
    expect(data.readiness!.composite).toBe(59);
    expect(data.readiness!.score).toBe(53);
    expect(data.readiness!.band).toBe('moderate');
    expect(data.readiness!.limitedBy.map((l) => l.key)).toEqual(['recovery']);
    expect(data.readiness!.drivers.find((d) => d.key === 'sleep_history')!.source).toBe('derived');
    /*
     * This fixture's payload carries no `timestamp`, so there is no instant to count down from — the
     * forecast refuses a date rather than inventing one (spec 075's rule, spec 084's consequence).
     */
    expect(data.readiness!.forecast.recoveredAt).toBeNull();
    expect(data.readiness!.forecast.fullyReadyAt).toBeNull();
    ...
    expect(data.condition!.recovery).toMatchObject({
      minutes: 2040,
  ...
  /*
   * Spec 075 — the countdown reaches the wire. The clock is fixed at 2026-08-07T10:00Z and the
   * fixture is captured at 06:00Z the same day, so "now" sits four hours into a 2040-minute timer.
   */
  describe('the recovery timer as an instant', () => {
    /** Garmin's zoneless UTC stamp, on the newest day the fake serves. */
    const CAPTURED_AT = '2026-08-07T06:00:00.0';
    const CAPTURED_MS = Date.UTC(2026, 7, 7, 6, 0, 0);

    /** The fake, with a capture instant on readiness and an optional activity to supersede it. */
    function anchoredGarmin(activityStartGmt?: string): GarminService & { rangeCalls: number } {
      const base = fakeGarmin({ authenticated: true });
      const inner = base.getMetricRange.bind(base);
      base.getMetricRange = async (name, start, end) => {
        const range = await inner(name, start, end);
        if (name === 'training_readiness') {
          return {
            ...range,
            days: range.days.map((d) => ({
              ...d,
              data: d.data === null ? null : { ...(d.data as object), timestamp: CAPTURED_AT }
            }))
          };
        }
        if (name === 'activities') {
          return {
            ...
            days: range.days.map((d, i) => ({
              ...d,
              // One activity, on the last day only — the case that can supersede the reading.
              data:
                activityStartGmt !== undefined && i === range.days.length - 1
                  ? [{ activityId: 1, startTimeGMT: activityStartGmt }]
                  : []
            }))
          };
        }
    ...
    it('serves the capture instant and the derived end moment', async () => {
      const data = await loadInsights(deps(anchoredGarmin()), { window: 30 });

      expect(data.condition!.recovery).toMatchObject({
        minutes: 2040, // still exactly what Garmin said — the MCP contract is untouched
        capturedAt: CAPTURED_MS,
        endsAt: CAPTURED_MS + 2040 * 60_000,
        superseded: false
      });
    });

    it('flags a reading a later session has already invalidated', async () => {
      // 08:00Z, two hours after Garmin computed the reading it could not have known about.
      const data = await loadInsights(deps(anchoredGarmin('2026-08-07 08:00:00')), { window: 30 });
      expect(data.condition!.recovery!.superseded).toBe(true);
    });

    it('keeps counting when the only activity predates the reading', async () => {
      const data = await loadInsights(deps(anchoredGarmin('2026-08-07 04:30:00')), { window: 30 });
      expect(data.condition!.recovery!.superseded).toBe(false);
    });

    it('nulls the instants for a payload without a timestamp, rather than inventing one', async () => {
      const data = await loadInsights(deps(fakeGarmin({ authenticated: true })), { window: 30 });
      expect(data.condition!.recovery).toMatchObject({
        minutes: 2040,
        capturedAt: null,
        endsAt: null,
    ...
    it('quotes the counted-down figure in the sentence, not the captured one', async () => {
      const data = await loadInsights(deps(anchoredGarmin()), { window: 30 });
      // Four hours in: 2040 − 240 = 1800 min = 30 h → "1 dzień 6 h". Captured, it reads "1 dzień 10 h".
      expect(data.condition!.garmin!.summary).toContain('do pełnej regeneracji 1 dzień 6 h');
      expect(data.condition!.garmin!.summary).not.toContain('1 dzień 10 h');
    });
  });
  ...
  it('honours the window and computes the span (90 days = 3 chunks per metric)', async () => {
    ...
    /*
     * One chunk per 31 days, per metric — plus exactly ONE more, whatever the window: the 4-day
     * activities read that decides whether Garmin's recovery countdown has been superseded by a
     * session it has not scored yet (spec 075). Asserted rather than waved through, so the tail
     * cannot quietly start scaling with the window.
     */
    expect(garmin.rangeCalls).toBe(METRICS.length * 3 + 1);
  });
  ...
  it('anchors the window on the local day, not the UTC day (spec 018)', async () => {
    ...
    const d = { garmin: c.garminFor(USER), clock: c.clock };
  ...
  /*
   * Spec 072. `loadInsights` resolves "today" from the injected clock; the condition snapshot is
   * built from the newest day the SOURCE holds. Those two coincided in every test until now, which
   * is exactly why a two-day-old readiness could reach the start page dressed as this morning's.
   */
  it('reports a current snapshot as not stale', async () => {
    const d = deps(fakeGarmin({ authenticated: true }));
    const data = await loadInsights(d, { window: 30 });

    expect(data.condition?.day).toBe('2026-08-07'); // = today, per the fixed clock
    expect(data.condition?.staleDays).toBe(0);
    expect(data.condition?.garmin?.staleDays).toBe(0);
  });

  it('measures how old the snapshot is when Garmin stopped a few days back', async () => {
    const d = deps(fakeGarmin({ authenticated: true, dataThrough: '2026-08-05' }));
    const data = await loadInsights(d, { window: 30 });

    expect(data.condition?.day).toBe('2026-08-05');
    expect(data.condition?.staleDays).toBe(2);
    // Garmin's own verdict carries its own age: the card renders the two side by side.
    expect(data.condition?.garmin?.day).toBe('2026-08-05');
    expect(data.condition?.garmin?.staleDays).toBe(2);
    expect(data.condition?.garmin?.summary).toContain('dane z 5 sie');
  });
