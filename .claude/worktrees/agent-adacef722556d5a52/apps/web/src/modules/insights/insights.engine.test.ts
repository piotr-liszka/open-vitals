...
describe('computeInsights', () => {
  it('assembles the three window-based kinds and passes readiness through', () => {
    const result = computeInsights(
      [series('body_battery', [40, 40, 60, 50, 60]), series('hrv', [60, 60, 40, 50, 40])],
      DEFAULT_INSIGHTS_CONFIG
    );
    // Readiness is no longer derived here (spec 084) — the caller computes it and hands it over.
  ...
  it('returns the readiness object it was given, untouched', () => {
    const readiness = { score: 57, band: 'moderate' } as never;
    const result = computeInsights([series('hrv', [1, 2, 3])], DEFAULT_INSIGHTS_CONFIG, readiness);
