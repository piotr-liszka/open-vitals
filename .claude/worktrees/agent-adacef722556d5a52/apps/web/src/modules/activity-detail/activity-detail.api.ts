...
import { flattenWorkoutSteps, matchPlanned, type PlannedCandidate } from './activity-plan';
...
export async function loadActivityDetail(
  ...
  /*
   * The plan's own step sequence, for the Przebieg strip (spec 085). `matchPlanned` is pure and
   * deterministic, so asking it again costs a sort over at most a handful of candidates and saves
   * shipping the same array twice inside `trainingComparison`.
   */
  ...
  const plannedStructure = flatSteps.length > 0 ? flatSteps : null;
