...
import { combineSports } from './weekly-summary.combine';
import {
  ALL_SPORTS,
...
/** A two-sport athlete: the payload folds the families and opens on "Wszystko" (spec 089). */
const multi = (): WeeklySummaryData => {
  const sports = [ride, sport()];
  return data({ sports, combined: combineSports(sports), defaultGroup: ALL_SPORTS });
};

/** Tile labels, in render order. */
function tileLabels(container: HTMLElement): string[] {
  return [...container.querySelectorAll('.tile .label-text')].map((e) => e.textContent?.trim() ?? '');
}

/** The readout of the tile with this label, or `null` when there is no such tile. */
function tileValue(container: HTMLElement, label: string): string | null {
  const tile = [...container.querySelectorAll('.tile')].find(
    (t) => t.querySelector('.label-text')?.textContent?.trim() === label
  );
  return tile?.querySelector('.value')?.textContent?.trim() ?? null;
}

function chipLabels(group: HTMLElement): string[] {
  return [...group.querySelectorAll('button')].map((c) => c.textContent?.trim() ?? '');
}

describe('WeeklySportSummary', () => {
  it('leads with Wszystko, then the families busiest-first, and opens on Wszystko', () => {
    const { getByRole } = render(WeeklySportSummary, { props: { data: multi() } });

    const group = getByRole('group', { name: 'Sport' });
    expect(chipLabels(group)).toEqual(['Wszystko', 'Rower', 'Bieg']);
    expect([...group.querySelectorAll('button')].map((c) => c.getAttribute('aria-pressed'))).toEqual([
      'true',
      'false',
      'false'
    ]);
  ...
  it('re-renders the whole card for the family a chip selects', async () => {
    const { getByRole, getByText, container } = render(WeeklySportSummary, { props: { data: multi() } });

    await fireEvent.click(getByRole('button', { name: 'Rower' }));

    // The ride: 62 km this week, in the ride lane colour.
  ...
  it('shows the combined week as time, climb and sessions — and no distance tile', () => {
    const { container, getByText, queryByText } = render(WeeklySportSummary, { props: { data: multi() } });

    expect(tileLabels(container)).toEqual(['Czas', 'Przewyższenie', 'Liczba sesji']);
    expect(queryByText('Dystans')).toBeNull();
    // 9 000 s of riding + 7 200 s of running, 640 m + 145 m climbed, one ride + two runs.
    expect(tileValue(container, 'Czas')).toBe('4 h 30 min');
    expect(tileValue(container, 'Przewyższenie')).toBe('785');
    expect(tileValue(container, 'Liczba sesji')).toBe('3');
    // …and it says why the kilometres are missing, rather than leaving a hole.
    expect(getByText(/kilometra na rowerze nie dodajemy/)).toBeTruthy();
  });

  it('plots combined weeks in hours, and per-sport weeks in kilometres', async () => {
    const { container, getByRole } = render(WeeklySportSummary, { props: { data: multi() } });

    // Ride 62 000 m / 3 s per m + run 21 500 m / 3 = 27 833 s ≈ 7,7 h in the week in progress.
    const summary = () => container.querySelector('svg')?.getAttribute('aria-label') ?? '';
    expect(summary()).toContain('Czas trend');
    expect(summary()).toContain('bieżący tydzień (w toku): 7,7 godz.');
    expect(container.querySelector('.axis-unit')?.textContent).toBe('godz.');

    await fireEvent.click(getByRole('button', { name: 'Bieg' }));

    expect(summary()).toContain('Dystans trend');
    expect(summary()).toContain('bieżący tydzień (w toku): 21,5 km');
    expect(container.querySelector('.axis-unit')?.textContent).toBe('km');
  });

  it('drops the note and the distance tile back in when a sport is picked', async () => {
    const { container, getByRole, queryByText } = render(WeeklySportSummary, { props: { data: multi() } });

    await fireEvent.click(getByRole('button', { name: 'Bieg' }));

    expect(tileLabels(container)).toEqual(['Dystans', 'Przewyższenie', 'Czas']);
    expect(tileValue(container, 'Dystans')).toBe('21,5');
    expect(queryByText(/kilometra na rowerze nie dodajemy/)).toBeNull();
  });

  it('offers no Wszystko chip to a single-sport athlete — it would name the only tab there is', () => {
    const { getByRole, queryByText, container } = render(WeeklySportSummary, { props: { data: data() } });

    expect(chipLabels(getByRole('group', { name: 'Sport' }))).toEqual(['Bieg']);
    expect(queryByText(/kilometra na rowerze nie dodajemy/)).toBeNull();
    expect(tileLabels(container)).toEqual(['Dystans', 'Przewyższenie', 'Czas']);
  });
