...
import { WORKOUT_PROVENANCE_LINE } from '$lib/workouts';
...
describe('sync engine — authored workout push (spec 050)', () => {
  ...
  it('sends the note and the provenance line as the workout description (spec 082)', async () => {
    const source = writeSource();
    const { store, engine } = engineWith(source);
    await seed(store, { note: 'Trzymaj moc, nie tętno' });

    await engine.syncUser('u1', { kind: 'incremental' });

    expect(source.log.created[0]?.note).toBe(`Trzymaj moc, nie tętno\n\n${WORKOUT_PROVENANCE_LINE}`);
  });

  it('sends the provenance line alone when the session has no note (spec 082)', async () => {
    const source = writeSource();
    const { store, engine } = engineWith(source);
    await seed(store);
    ...
    expect(source.log.created[0]?.note).toBe(WORKOUT_PROVENANCE_LINE);
  });
});
