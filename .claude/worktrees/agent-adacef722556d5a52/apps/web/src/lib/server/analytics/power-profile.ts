...
const RIDER_AXES: ReadonlyArray<{ key: RiderAxisKey; label: string; durationS: number }> = [
...
function zonesFromFtp(ftp: number): PowerZone[] {
...
export function buildPowerProfile(activities: PowerActivity[], opts: PowerProfileOptions): PowerProfile {
  ...
  const radar: RiderAxis[] = RIDER_AXES.map((axis) => {
