...
import { canPush, pushWorkout } from './workout-push';
...
import { extractMetricValue, hasMetricValue } from '../garmin/metric-specs';
...
export interface SyncEngineDeps {
  ...
  timeZone?: string;
  /**
   * Whether the authored-workout push phase may run FOR THIS USER (spec 050, re-homed in spec 071).
   * It used to be one deployment-wide boolean (`GARMIN_WORKOUT_PUSH`); it is now the user's own
   * Settings switch, so it has to be asked per user, per run. Defaults to FALSE when not injected —
   * this is the only phase that writes to a Garmin account, and a test that forgot to wire it should
   * not silently start writing.
   */
  workoutPushEnabledFor?: (userId: string) => Promise<boolean>;
}
...
export function createSyncEngine(deps: SyncEngineDeps): SyncEngine {
  ...
  const workoutPushEnabledFor = deps.workoutPushEnabledFor ?? (async () => false);

  async function syncUser(userId: string, opts: SyncOptions): Promise<SyncRun> {
    const source = sourceFor(userId);
    const workoutPushEnabled = await workoutPushEnabledFor(userId);
    ...
    try {
       ...
       * gated twice — by the user's own switch (spec 071) and by the source's ability to write.
       */
      if (workoutPushEnabled && canPush(source)) {
        try {
          ...
          for (const w of batch) {
            // One shared implementation of create-if-missing → schedule (spec 083); the row's push
            // state is written there. This loop only counts and decides when to give up.
            const result = await pushWorkout({ store, source, clock, classify: phaseFailure }, userId, w);
            ...
            if (
              f &&
              (f.code === 'sidecar_unreachable' || f.code === 'timeout' || f.code === 'rate_limited')
            ) {
              stopped = f;
              break;
            }
      ...
      try {
        ...
        /** Newest day this run found a real watch reading for (spec 072). Same reason for the box. */
        const fresh: { lastDataDay: string | null } = { lastDataDay: null };
        ...
        const pullChunk = async (chunkStart: DayKey, chunkEnd: DayKey): Promise<boolean> => {
          ...
          for (const metric of DAILY_METRICS) {
            try {
              ...
              /*
               * `data != null` is NOT presence (spec 072). Garmin answers a day it holds nothing for
               * with a present daily-summary object whose every field is null, and counting those is
               * how a run reported "106 dni z danymi / historia kompletna" on a morning when the
               * watch had not uploaded for two days and the start page was showing Thursday's
               * readiness as today's.
               */
              const days = range.days.map((d) => ({
                day: d.date,
                data: d.data,
                hasValue: hasMetricValue(metric, d.data)
              }));
              await store.putMetricDays(userId, metric, days);
              const present = days.filter((d) => d.hasValue).length;
              if (present > 0) {
                hadData = true;
                daysWithData += present;
                if (FRESHNESS_METRICS.includes(metric)) {
                  const newest = days.filter((d) => d.hasValue).at(-1)!.day;
                  if (fresh.lastDataDay === null || newest > fresh.lastDataDay) {
                    fresh.lastDataDay = newest;
                  }
                }
              }
            } catch (callErr) {
        ...
        /*
         * The freshness verdict (spec 072). This is the line that was missing on 2026-08-16: the run
         * said `ok`, the panel said "historia kompletna", and the newest thing Garmin actually held
         * was two days old because the watch had never uploaded. Our sync cannot fix that and must
         * not imply it did — so say plainly what Garmin has, and whose move it is.
         */
        if (fresh.lastDataDay !== null && fresh.lastDataDay < today) {
          const behind = daysBetween(fresh.lastDataDay, today);
          log(
            'warn',
            `Garmin nie ma danych nowszych niż ${fresh.lastDataDay} (${behind} dni wstecz) — ` +
              'zegarek prawdopodobnie nie zsynchronizował się z Garmin Connect.',
            { phase: 'metrics', day: fresh.lastDataDay }
          );
        }
