...
describe('ActivityBestEfforts', () => {
  ...
  it('shows the distance actually measured, since the window overshoots the target', () => {
    const { container, getByText } = render(ActivityBestEfforts, {
      props: { efforts: [effort({ actualM: 1050 })] }
    });
    expect(container.querySelectorAll('tbody td')[3]?.textContent).toContain('1050');
    expect(getByText(/tempo liczymy z tej wartości/)).toBeTruthy();
  });

  it('warns about a coarse sample interval only when the windows actually overshoot', () => {
