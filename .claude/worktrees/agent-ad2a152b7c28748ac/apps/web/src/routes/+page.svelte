<script lang="ts">
  ...
  $effect(() => {
    if (!data.authed || !data.health.connected) return;
...
<svelte:head>
  <title>{data.authed ? 'Pulpit · OpenVitals' : 'OpenVitals — Twoje dane z Garmina, połączone z AI'}</title>
</svelte:head>

{#if !data.authed}
  <Landing />
{:else}
  <AppShell title="Pulpit">
    {#snippet footer()}
      <SyncFooter />
    {/snippet}
