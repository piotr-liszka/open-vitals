...
describe('buildStatSections', () => {
  it('renders nothing at all for an activity with no stats', () => {
    expect(buildStatSections({ stats: emptyStats(), sport: 'run', hasTypedSplits: false })).toEqual([]);
  });

  it('keeps only the groups that actually measured something', () => {
    const sections = buildStatSections({
  ...
  it('says why there is no run/walk split when Garmin sent no typed splits', () => {
    const withoutSplits = buildStatSections({
    ...
    const withSplits = buildStatSections({
  ...
  it('is sport-aware: full running dynamics on a run, cadence only on a ride', () => {
    ...
    const run = buildStatSections({ stats: emptyStats(dynamics), sport: 'run', hasTypedSplits: true });
    expect(find(run, 'runningDynamics')?.items.map((i) => i.key)).toContain('gct');

    const ride = buildStatSections({ stats: emptyStats(dynamics), sport: 'ride', hasTypedSplits: true });
  ...
  it('offers minutes per kilometre only where they mean something', () => {
    ...
    const run = buildStatSections({ stats, sport: 'run', hasTypedSplits: true });
    expect(item(find(run, 'pace'), 'avgPace')?.value).toBe('2:05');

    const ride = buildStatSections({ stats, sport: 'ride', hasTypedSplits: true });
...
describe('buildHrZones', () => {
  ...
  it('prefers Garmin’s own zone times over our estimate', () => {
    const zones = buildHrZones([300, 600, 100, 0, 0], estimated);
  ...
  it('falls back to the estimate and says so', () => {
    const zones = buildHrZones(undefined, estimated);
  ...
  it('is null when neither source has any time in it', () => {
    expect(buildHrZones([0, 0, 0, 0, 0], [])).toBeNull();
    expect(buildHrZones(undefined, [])).toBeNull();
  });
});
