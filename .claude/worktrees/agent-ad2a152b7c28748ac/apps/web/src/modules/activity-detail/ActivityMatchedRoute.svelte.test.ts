...
describe('ActivityMatchedRoute', () => {
  ...
  it('shows the overlap per row and says the match is probable, not proven', () => {
    const { container, getByText } = render(ActivityMatchedRoute, { props: { route: route() } });
    expect(container.textContent).toContain('94%');
    expect(getByText(/nie dowód/)).toBeTruthy();
    expect(getByText(/Porównano 40 zapisanych tras/)).toBeTruthy();
  });

  it('says direction does not matter, since the engine ignores it', () => {
    const { getByText } = render(ActivityMatchedRoute, { props: { route: route() } });
    expect(getByText(/na odwrót też się dopasuje/)).toBeTruthy();
  });
