  ...
  {:else}
    ...
    <div class="panel">
      <div class="hero" class:with-chart={hasBatteryChart}>
        <div class="lead">
          <div class="lead-head">
            <span class="source-name">{i18n.t('readiness.subtitle')}</span>

            <!--
              `align="start"`: this trigger sits at the LEFT edge of the lead column (and wraps onto
              its own line when the column is narrow), so an end-aligned panel would open leftwards,
              across the card edge and over the sidebar. It opens rightwards, where the room is.
            -->
            <InfoPopover
          ...
          {#if condition.readiness}
            <ReadinessGauge readiness={condition.readiness} size="lg" {garminScore} now={nowMs} />
          {:else}
            <p class="note">Za mało danych, żeby policzyć wynik.</p>
          {/if}
          ...
          -->
          {#if garmin === null}
            <p class="source-note">Garmin nie przysłał dla tego konta swojego wyniku gotowości.</p>
          {/if}
        </div>
