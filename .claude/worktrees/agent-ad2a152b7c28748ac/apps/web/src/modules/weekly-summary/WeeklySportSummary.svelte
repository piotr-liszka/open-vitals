<script lang="ts">
  ...
  const chartLabel = $derived(showingAll ? i18n.t('timeline.stat.time') : 'Dystans');
  const point = (n: number): string => `${nf1.format(n)} ${chartUnit}`;
  ...
  {#if view}
    ...
    <section class="block" aria-labelledby="weekly-summary-now">
      <div class="head">
        ...
        <p class="caption">
          od poniedziałku {weekStartLabel} · {data.currentWeekDays} z 7 dni · {view.label}
        </p>
      </div>
      <div class="tiles">
        {#if showingAll}
          ...
          <StatTile
            ...
            value={nf0.format(view.thisWeek.elevationGainM)}
            unit="m"
            accent="violet"
          />
          <StatTile
            label={i18n.t('weeklySummary.all.sessions')}
            value={nf0.format(view.thisWeek.activities)}
            accent="orange"
          />
        {:else}
          <StatTile label="Dystans" value={km(view.thisWeek.distanceM)} unit="km" accent="green" />
          <StatTile
            label="Przewyższenie"
            value={nf0.format(view.thisWeek.elevationGainM)}
            unit="m"
            accent="violet"
          />
          <StatTile label="Czas" value={hours(view.thisWeek.durationS)} accent="cyan" />
        {/if}
      </div>
      {#if showingAll}
    ...
    <section class="block" aria-labelledby="weekly-summary-trend">
      ...
      <p class="caption">
        ...
        Ostatni punkt to bieżący, niepełny tydzień: {point(currentValue)} po {data.currentWeekDays}
        {data.currentWeekDays === 1 ? 'dniu' : 'dniach'}.
      </p>
    </section>

    <p class="more">
      <a href="/training/volume">Pełny widok objętości →</a>
    </p>
  {:else}
    <p class="empty">
