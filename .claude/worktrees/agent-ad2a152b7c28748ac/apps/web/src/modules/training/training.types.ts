...
import type { TrainingTab } from './training-nav';
...
/**
 * Which tab bar `/training/**` renders (spec 088). The pathname is part of the request because the
 * two sections under `/training` have different tab sets — and only one of them reads the store.
 */
export interface TrainingTabsRequest {
  readonly userId: string;
  readonly locale: Locale;
  /** Pathname being rendered, e.g. `/training/plan`. Unknown paths fall back to the analysis set. */
  readonly pathname: string;
}

/** Just the store: the tab bar needs per-sport counts and nothing else. */
export interface TrainingTabsDeps {
  readonly store: Pick<LocalStore, 'listSports'>;
}

export interface TrainingTabsResult {
  readonly tabs: TrainingTab[];
}
