...
describe('BestEffortsCard', () => {
  ...
  it('states how many results a distance can show, from the data rather than a hardcoded 3', () => {
    const { getByText } = render(BestEffortsCard, { props: { data: data({ topN: 5 }) } });
    expect(getByText(/do\s*5\s*najlepszych/)).toBeTruthy();
  });
});
