...
describe('GarminHttpAdapter — planned events + diagnostics (specs 019/024)', () => {
  ...
  it('posts a workout and normalises a numeric Garmin id to text', async () => {
    ...
    // The workout-level description rides the same POST (spec 082).
    expect(body.note).toBe('Progi\n\nvia OpenVitals');
  });
  ...
  it('keeps an unsupported create unsupported, with no id', async () => {
    ...
    expect(await garmin.createWorkout!({ sport: 'running', title: 'x', steps: [], note: null })).toEqual({
      supported: false,
      workoutId: null,
  ...
  it('surfaces a sidecar failure on a write instead of pretending it landed', async () => {
    ...
    await expect(
      garmin.createWorkout!({ sport: 'running', title: 'x', steps: [], note: null })
    ).rejects.toThrow(GarminUnavailableError);
  });
