...
/**
 * Spec 072 — Garmin answers a day it holds nothing for with a PRESENT daily-summary object whose
 * every field is null (`includesWellnessData: false`, `lastSyncTimestampGMT: null`). Counting those
 * as data is how a run on 2026-08-16 reported "106 dni z danymi" and a complete history while the
 * newest thing Garmin actually held was two days old, because the watch had never uploaded.
 */
describe('sync engine — hollow days (spec 072)', () => {
  /** Real data up to `lastRealDay`, then Garmin's all-null object for every day after it. */
  function watchStoppedUploading(lastRealDay: string): GarminSyncSource {
    return fakeSource({
          ...
          days.push({
            ...
            data:
              day < '2026-07-01'
                ? null
                : day <= lastRealDay
                  ? { totalSteps: 1000, restingHeartRate: 50, score: 60 }
                  : {
                      calendarDate: day,
                      totalSteps: null,
  ...
  it('does not advertise a hollow day as the newest day it holds', async () => {
    const { store, engine } = engineWith(watchStoppedUploading('2026-08-06'));
    await engine.syncUser('u1', { kind: 'incremental' });

    const cov = await store.coverage('u1');
    const steps = cov.metrics.find((m) => m.metric === 'steps')!;

    // The run WROTE rows for the 7th and 8th — it must remember it checked them — but neither is
    // data, so neither may be reported as coverage.
    expect(steps.lastDay).toBe('2026-08-06');
    expect(cov.freshness.lastDataDay).toBe('2026-08-06');
    expect(cov.storage.rows.metricDays).toBeGreaterThan(steps.presentDays);
  });

  it('warns that Garmin is behind instead of reporting a clean run', async () => {
    const { engine } = engineWith(watchStoppedUploading('2026-08-06'));
    const run = await engine.syncUser('u1', { kind: 'incremental' });

    expect(run.status).toBe('succeeded'); // our side genuinely worked — that was never the problem
    const warning = (run.detail?.log ?? []).find(
      (e) => e.level === 'warn' && e.msg.includes('Garmin nie ma danych nowszych')
    );
    expect(warning?.msg).toContain('2026-08-06');
    // The line has to name whose move it is; "sync failed" would send the user to the wrong lever.
    expect(warning?.msg).toContain('Garmin Connect');
  });

  it('says nothing when the watch is current', async () => {
    const { store, engine } = engineWith(watchStoppedUploading(TODAY));
    const run = await engine.syncUser('u1', { kind: 'incremental' });

    expect((await store.coverage('u1')).freshness.lastDataDay).toBe(TODAY);
    expect((run.detail?.log ?? []).some((e) => e.msg.includes('Garmin nie ma danych nowszych'))).toBe(false);
  });
});
