...
/**
 * Enough for a readiness score under spec 084: Garmin's own factors and a recovery timer. Body Battery
 * and resting heart rate are no longer channels — the score reads the six inputs Garmin publishes, not
 * four metrics z-scored against a rolling window — but they stay here so the fixture still exercises
 * the charts and trends the same call returns.
 */
const READINESS_METRICS = {
  body_battery: { bodyBatteryValuesArray: [[0, 'MEASURED', 45, 0]] },
  sleep: { dailySleepDTO: { sleepTimeSeconds: 25_200, sleepScores: { overall: { value: 80 } } } },
  hrv: {
    hrvSummary: {
      status: 'BALANCED',
      weeklyAvg: 110,
      lastNightAvg: 112,
      baseline: { balancedLow: 102, balancedUpper: 133 }
    }
  },
  ...
  training_readiness: {
    score: 44,
    level: 'LOW',
    timestamp: '2026-08-06T05:05:19.0',
...
describe('garmin MCP tools', () => {
  ...
  it('get_readiness returns a score and drivers over a window', async () => {
    ...
    // No `window` argument: since spec 084 the score is a snapshot of today and has none.
    const res = await tool('get_readiness').handler(garmin, {});
    expect(res.isError).toBeFalsy();
    const payload = JSON.parse(res.content[0]!.text) as {
      score: number;
      composite: number;
      drivers: Array<{ key: string; percent: number }>;
      limitedBy: unknown[];
      forecast: { fullyReadyAt: string | null };
    };
    expect(typeof payload.score).toBe('number');
    expect(payload.drivers.map((d) => d.key).sort()).toEqual([
      'hrv',
      'load',
      'recovery',
      'sleep',
    ...
    // Balanced HRV and a 10-hour timer: nothing crushing, so nothing capped.
    expect(payload.limitedBy).toEqual([]);
    expect(payload.score).toBe(payload.composite);
  });

  it('get_readiness takes no window argument at all', () => {
    expect(Object.keys(tool('get_readiness').inputShape)).toEqual([]);
  });
