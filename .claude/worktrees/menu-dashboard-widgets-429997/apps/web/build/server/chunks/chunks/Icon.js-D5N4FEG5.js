import { ab as attr, a7 as ensure_array_like, Q as derived } from './index.js-D7taQuDv.js';
...
function Icon($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    let { name, size = 20, label, strokeWidth = 1.6 } = $$props;
    const glyph = derived(() => ICONS[name]);
    $$renderer2.push(`<svg class="icon svelte-jf3o41" viewBox="0 0 24 24"${attr("width", size)}${attr("height", size)} fill="none" stroke="currentColor"${attr("stroke-width", strokeWidth)} stroke-linecap="round" stroke-linejoin="round"${attr("role", label ? "img" : void 0)}${attr("aria-label", label)}${attr("aria-hidden", label ? void 0 : "true")} focusable="false"${attr("data-icon", name)}><!--[-->`);
    const each_array = ensure_array_like(glyph().rects ?? []);
    for (let i = 0, $$length = each_array.length; i < $$length; i++) {
      let r = each_array[i];
      $$renderer2.push(`<rect${attr("x", r[0])}${attr("y", r[1])}${attr("width", r[2])}${attr("height", r[3])}${attr("rx", r[4])}></rect>`);
    }
    $$renderer2.push(`<!--]--><!--[-->`);
    const each_array_1 = ensure_array_like(glyph().circles ?? []);
    for (let i = 0, $$length = each_array_1.length; i < $$length; i++) {
      let c = each_array_1[i];
      $$renderer2.push(`<circle${attr("cx", c[0])}${attr("cy", c[1])}${attr("r", c[2])}></circle>`);
    }
    $$renderer2.push(`<!--]--><!--[-->`);
    const each_array_2 = ensure_array_like(glyph().paths);
...
export { Icon as I };
//# sourceMappingURL=Icon.js-D5N4FEG5.js.map
