 ...
 * while /settings still does. Loader-level test with stub locals (same style as tier-gating.test.ts).
 */
...
function consentStub(advanced: boolean): ConsentService {
  return {
    isEnabled: async () => advanced,
    listFeatures: async () => [],
    accept: async () => {
...
function homeLocals(advanced: boolean) {
  return {
    user: { id: 'user-1' },
    consent: consentStub(advanced),
    garmin: createGarminMock({ status: { authenticated: true, displayName: 'Ada' } }),
    container: containerStub()
  } as never;
}

/** The page load reads `?trend=` for the dashboard window (spec 028). */
function homeEvent(advanced: boolean, url = 'http://localhost/') {
  return { locals: homeLocals(advanced), url: new URL(url) } as never;
}

describe('start page payload', () => {
  it('base tier: no mcpUrl in the payload and no token service touched', async () => {
    const data = (await loadHome(homeEvent(false))) as Record<string, unknown>;
  ...
  it('advanced tier: no mcpUrl in the payload either', async () => {
    const data = (await loadHome(homeEvent(true))) as Record<string, unknown>;

    expect(data.tier).toBe('advanced');
    expect('mcpUrl' in data).toBe(false);
  ...
  it('carries the global range into the dashboard, falling back on junk (spec 028/035)', async () => {
    const dashboardOf = async (url: string) => {
      const data = (await loadHome(homeEvent(true, url))) as {
  ...
  it('carries the global range into the timeline too (spec 047)', async () => {
    const timelineOf = async (url: string) => {
      const data = (await loadHome(homeEvent(true, url))) as { timeline: { past: { from: string } } };
  ...
  it('logged out: nothing per-user is loaded at all', async () => {
    const data = (await loadHome({ locals: { user: null } } as never)) as Record<string, unknown>;
    expect(data).toEqual({ authed: false });
  });
});
