...
describe('RANGE_OPTIONS', () => {
  it('covers every key exactly once, in order, with both labels', () => {
    expect(RANGE_OPTIONS.map((o) => o.value)).toEqual([...RANGE_KEYS]);
    for (const option of RANGE_OPTIONS) {
      expect(option.label.length).toBeGreaterThan(0);
      expect(option.short.length).toBeGreaterThan(0);
    }
  });

  it('labels the extended ranges in Polish', () => {
    expect(rangeKeyLabel('365')).toBe('1 rok');
    expect(rangeKeyLabel('all')).toBe('cały czas');
  });
});
...
describe('resolveRange', () => {
  ...
  it('anchors "all" on the earliest synced day', () => {
    ...
    expect(range.label).toContain('2021-03-04');
  });
  ...
  it('degrades "all" to the default window when nothing is synced', () => {
    ...
    expect(range.label).toBe('cały czas');
  });
...
describe('routeSupportsRange', () => {
  ...
  it('covers the windowed training subpages', () => {
    expect(routeSupportsRange('/training/bieg')).toBe(true);
    expect(routeSupportsRange('/training/marsz')).toBe(true);
  });
  ...
  it('is off where nothing would react to it', () => {
    ...
    expect(routeSupportsRange('/training/rower')).toBe(false);
...
describe('withRange', () => {
  const at = (search: string): URL => new URL(`http://vagus.test/insights${search}`);
  ...
  it('never invents a range on a page that ignores it', () => {
    ...
    expect(withRange('/training/rower', at('?range=365'))).toBe('/training/rower');
