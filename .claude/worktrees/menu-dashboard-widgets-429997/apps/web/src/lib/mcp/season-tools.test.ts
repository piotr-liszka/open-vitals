...
function deps(consent = fakeConsent()): SeasonToolDeps {
  return {
...
describe('season MCP tools', () => {
  ...
  it('every tool refuses without the data-processing consent', async () => {
    const d = deps(fakeConsent(false));
    for (const t of SEASON_TOOLS) {
      const result = await t.handler(d, { goalId: 'x', day: TODAY, sport: 'run', title: 'x' });
