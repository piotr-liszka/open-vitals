 ...
 * the consent gate, per-user isolation, and never silently leaving a session on the watch after a
...
import { createConsentService } from '../server/consent/service';
import { createMemoryConsentStore } from '../server/consent/store';
import { WORKOUT_WRITE_FEATURE } from '../server/consent/registry';
import { getFeature } from '../server/consent/registry';
import type { ConsentStore } from '../server/consent/types';
...
/** A harness with consent ALREADY accepted (the gate has its own tests below). */
async function harness(
  over: { accepted?: boolean; deleteResult?: GarminWorkoutDeleteResult | 'throw' | 'none' } = {}
): Promise<Harness> {
  const store = createMemoryStore();
  const consentStore = createMemoryConsentStore(clock);
  const userId = 'u1';
  if (over.accepted !== false) {
    await consentStore.set(userId, WORKOUT_WRITE_FEATURE, getFeature(WORKOUT_WRITE_FEATURE)!.termsVersion);
  }
  ...
  return {
    ...
    deps: {
      ...
      consent: createConsentService({ store: consentStore, userId }),
      garmin
    }
  };
}
...
describe('MCP workout tools', () => {
  ...
  it('create_workout stores a session locally and says it is only queued for Garmin', async () => {
    ...
    expect(out.sportLabel).toBe('Bieg');
  ...
  describe('consent gate', () => {
    ...
    it('still allows READING the list without the write consent', async () => {
