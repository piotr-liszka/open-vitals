 ...
 * pages used to each pass an identical snippet). Items flagged `advanced` only show for users who
 * enabled data processing (the Advanced tier, spec 014).
 *
...
export interface NavItem {
   ...
   */
  icon: IconName;
  /** Only shown to Advanced-tier users (data-processing consent). */
  advanced?: boolean;
  /**
...
/** One heading and the items under it, after tier filtering. */
...
/**
 * The nav a user of this tier actually sees, collapsed into runs of adjacent items sharing a group.
 * A group whose every item is Advanced-only disappears entirely for a Base user rather than leaving
 ...
 */
export function navGroups(advanced: boolean, dashboards: readonly NavItem[] = []): NavGroup[] {
