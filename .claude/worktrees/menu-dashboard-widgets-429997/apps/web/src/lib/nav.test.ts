...
describe('primary navigation', () => {
  it('groups adjacent items under one heading, leaving Start ungrouped', () => {
    expect(navGroups(true).map((g) => [g.group, g.items.map((i) => i.label)])).toEqual([
      [undefined, ['Start']],
  ...
  it('is six FIXED items for an Advanced user', () => {
    ...
    expect(navGroups(true).flatMap((g) => g.items)).toHaveLength(6);
  });
  ...
  it('splices caller-supplied items in directly after Start, before Trening', () => {
    const injected = [
      { href: '/dashboard/x', label: 'X', icon: 'grid' as const, advanced: true, group: 'Panele' }
    ];
    const groups = navGroups(true, injected);
    expect(groups.map((g) => g.group)).toEqual([undefined, 'Panele', 'Trening', 'Zdrowie', 'System']);
  });

  it('is unchanged when the caller supplies none', () => {
    expect(navGroups(true, [])).toEqual(navGroups(true));
  });

  it('hides every Advanced item from a Base user', () => {
    const items = navGroups(false).flatMap((g) => g.items);
    expect(items.map((i) => i.href)).toEqual(['/', '/data', '/settings']);
    expect(items.some((i) => i.advanced)).toBe(false);
  });

  it('never emits a heading with nothing under it', () => {
    for (const advanced of [true, false]) {
      for (const group of navGroups(advanced)) expect(group.items.length).toBeGreaterThan(0);
    }
  });
  ...
  it('keeps each group contiguous, so no heading is rendered twice', () => {
    ...
    const names = navGroups(true).map((g) => g.group);
