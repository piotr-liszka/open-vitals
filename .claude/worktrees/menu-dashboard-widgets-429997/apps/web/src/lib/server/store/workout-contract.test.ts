...
function newWorkout(overrides: Partial<NewAuthoredWorkout> = {}): NewAuthoredWorkout {
  return {
    id: 'w-1',
    day: '2026-08-20',
    time: '18:00',
...
export function workoutContract(name: string, make: () => Promise<Fixture>): void {
  describe(`${name} — authored workout contract`, () => {
    ...
    it('lists day then time ascending, filtered by window and push state', async () => {
      ...
      await store.updateWorkout(userId, 'b', {
        pushState: 'pushed',
      ...
      expect(window.map((w) => w.id)).toEqual(['a', 'b']);

      const pending = await store.listWorkouts(userId, { pushState: 'pending' });
      expect(pending.map((w) => w.id)).toEqual(['a', 'c']);
    });

    it('patches only the keys present, and can clear a nullable column', async () => {
      const { store, userId } = await make();
      await store.createWorkout(userId, newWorkout());
      await store.updateWorkout(userId, 'w-1', {
        pushState: 'failed',
        pushError: 'sidecar unavailable',
        updatedAt: '2026-08-13T10:00:00.000Z'
      });

      const failed = await store.getWorkout(userId, 'w-1');
      ...
      const cleared = await store.updateWorkout(userId, 'w-1', {
        pushState: 'pushed',
        pushError: null,
    ...
    it('returns the deleted row so the caller can still clean it up upstream', async () => {
      ...
      const deleted = await store.deleteWorkout(userId, 'w-1');

      expect(deleted?.garminWorkoutId).toBe('424242');
      expect(await store.getWorkout(userId, 'w-1')).toBeNull();
      expect(await store.deleteWorkout(userId, 'w-1')).toBeNull();
    });
    ...
    it('never leaks across users', async () => {
      ...
      expect(await store.getWorkout(otherUserId, 'w-1')).toBeNull();
      expect(await store.listWorkouts(otherUserId)).toEqual([]);
      expect(
        await store.updateWorkout(otherUserId, 'w-1', { updatedAt: '2026-08-13T11:00:00.000Z' })
      ).toBeNull();
      expect(await store.deleteWorkout(otherUserId, 'w-1')).toBeNull();
      // …and the owner's row is untouched by all of that.
      expect(await store.getWorkout(userId, 'w-1')).not.toBeNull();
    });
