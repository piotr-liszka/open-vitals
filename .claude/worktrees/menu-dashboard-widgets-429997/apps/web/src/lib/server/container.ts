...
import { createMemoryConsentStore, createPgConsentStore } from './consent/store';
...
import type { ConsentService, ConsentStore } from './consent/types';
...
export interface AppContainer {
  ...
  /** Build a ConsentService scoped to one user — records are keyed by their id. */
  consentFor(userId: string): ConsentService;
}
...
export interface ContainerOverrides {
  ...
  /** Inject a per-user ConsentService factory (tests). */
  consentFor?: (userId: string) => ConsentService;
...
export function createContainer(overrides: ContainerOverrides = {}): AppContainer {
  ...
  // Per-user consent: one shared store (pg in prod, in-memory in tests) + a per-user service view.
  const consentStore = overrides.consentStore ?? createPgConsentStore(db!, clock);
  const consentFor: (userId: string) => ConsentService =
    overrides.consentFor ?? ((userId: string) => createConsentService({ store: consentStore, userId }));
...
export function createTestContainer(overrides: ContainerOverrides = {}): AppContainer {
  ...
  return createContainer({
    ...
    consentStore: overrides.consentStore ?? createMemoryConsentStore(clock)
  });
}
