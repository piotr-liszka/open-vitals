...
describe('TimelineView', () => {
  ...
  it('has honest not-connected and consent-off states', () => {
    const off = render(TimelineView, { props: { data: null, connected: false } });
    expect(text(off.container)).toContain('Połącz konto Garmin');
    cleanup();

    const noConsent = render(TimelineView, { props: { data: null, connected: true, enabled: false } });
    expect(text(noConsent.container)).toContain('Włącz tryb zaawansowany');
  });
});
