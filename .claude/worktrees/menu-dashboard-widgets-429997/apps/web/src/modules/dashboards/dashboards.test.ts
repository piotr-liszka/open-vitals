...
describe('dashboard config', () => {
  ...
  it('falls back to the default when empty/garbage', () => {
    ...
    expect(sanitizeConfig({ dashboards: [] })).toEqual(defaultConfig());
  });

  it('round-trips through the settings repo', async () => {
    const settings = createMemorySettingsRepo();
    expect(await getConfig(settings, 'u')).toEqual(defaultConfig());
    const cfg = {
      dashboards: [{ id: 'main', name: 'Home', widgets: [{ id: 'w', type: 'coverage', span: 12 }] }]
    };
    const saved = await saveConfig(settings, 'u', cfg);
    expect(saved.dashboards[0]!.widgets[0]!.span).toBe(12);
    expect(await getConfig(settings, 'u')).toEqual(saved);
  });
  ...
  describe('ids are route segments now (spec 064)', () => {
    ...
    it('refuses `new`, which is the create page', () => {
      const clean = sanitizeConfig({ dashboards: [{ id: 'new', name: 'X', widgets: [] }] });
    ...
    it('never returns zero dashboards, so the nav and /dashboard always have a target', () => {
      for (const junk of [null, {}, { dashboards: [] }, { dashboards: [null, 7, 'x'] }]) {
        expect(sanitizeConfig(junk).dashboards.length).toBeGreaterThan(0);
      }
    });
  });
  ...
  describe('nav placement (spec 064)', () => {
    ...
    it('puts every entry in the Panele group and marks it Advanced-only', () => {
      for (const item of dashboardNavItems([{ id: 'a', name: 'A' }])) {
        expect(item.group).toBe(NAV_GROUP_DASHBOARDS);
        expect(item.advanced).toBe(true);
      }
    });
    ...
    it('sits directly below Start and above Trening in the sidebar', () => {
      const groups = navGroups(true, dashboardNavItems([{ id: 'main', name: 'Przegląd' }]));
    ...
    it('shows a Base user no Panele group at all', () => {
      const groups = navGroups(false, dashboardNavItems([{ id: 'main', name: 'Przegląd' }]));
      expect(groups.map((g) => g.group)).not.toContain(NAV_GROUP_DASHBOARDS);
    });
  });
