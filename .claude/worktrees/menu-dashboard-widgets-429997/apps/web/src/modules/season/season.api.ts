 ...
 *
 * Pure over injected deps (store + settings + clock + random + consent): no live Garmin, no
 * `Date.now()`, no env. Never throws on a consent/empty-history condition — it degrades to an
...
export async function loadSeason(deps: SeasonDeps, req: SeasonRequest): Promise<SeasonData> {
  ...
  // Not consented: an honest empty payload rather than a throw, so the caller renders a gate.
