...
function fakeConsent(enabled = true): ConsentService {
  return {
    async isEnabled() {
      return enabled;
    }
...
function deps(
  over: { store?: LocalStore; consent?: ConsentService; settings?: UserSettings } = {}
): SeasonDeps {
  return {
    store: over.store ?? createMemoryStore(),
    settings: fakeSettings(over.settings),
    consent: over.consent ?? fakeConsent(),
    clock,
...
describe('loadSeason', () => {
  it('degrades to an empty payload when data processing is not consented', async () => {
    const d = deps({ consent: fakeConsent(false) });
    await d.store.createGoal(USER, {
      id: 'g-1',
    ...
    const data = await loadSeason(d, { userId: USER });
  ...
  it('is empty but enabled for a consented athlete with no goals', async () => {
