<script lang="ts">
  ...
  interface Props {
    ...
    loading?: boolean;
    /** Called after consent changes so the parent can reload. */
    onConsentChange?: () => void;
  }

  let { data, range, analyticsFeature, loading = false, onConsentChange }: Props = $props();

  const bucketLabel = $derived(range.bucket === 'day' ? undefined : bucketNoun(range.bucket));
  ...
  const fmtDate = (key: string): string => formatDay(key, 'short');
...
<div class="insights">
  <header class="head">
    ...
    <RangeBadge label={range.label} bucketNoun={bucketLabel} />
  </header>
  ...
  {:else if !data.enabled}
    <Card title="Włącz tryb zaawansowany" subtitle="Wnioski powstają z Twoich wielodniowych metryk.">
      {#if analyticsFeature}
        <ConsentPanel feature={analyticsFeature} onUpdated={() => onConsentChange?.()} />
      {/if}
    </Card>
  {:else}
    <ReadinessCard readiness={data.readiness} connected={data.connected} enabled={data.enabled} />
    ...
    <section aria-label="Wykresy długiego okresu">
      <div class="section-head">
        <h3 class="section-title">Metryki — {range.label}</h3>
        ...
        <RangeBadge label={range.label} bucketNoun={bucketLabel} size="sm" />
      </div>
