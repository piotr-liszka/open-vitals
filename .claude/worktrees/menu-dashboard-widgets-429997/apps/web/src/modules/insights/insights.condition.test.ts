...
describe('computeCondition', () => {
  ...
  it('assembles sleep, the four recovery channels, readiness and a sentence', () => {
    const snapshot = computeCondition(series(), readiness())!;
  ...
  it('still reports when only some channels exist', () => {
    const snapshot = computeCondition([{ spec: spec('hrv'), days: days([50, 60]), raw: [] }], null)!;
  ...
  it('returns null when there is genuinely nothing to show', () => {
    expect(computeCondition([], null)).toBeNull();
    expect(computeCondition([{ spec: spec('hrv'), days: days([null, null]), raw: [] }], null)).toBeNull();
  });
});
