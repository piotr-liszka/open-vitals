...
describe('parseTrainingReadinessDay', () => {
  ...
  it('accepts the snake_case variant of every field', () => {
    ...
    expect(parsed!.recovery).toEqual({ day: '2026-08-14', hours: 0, change: 'bez zmian' });
  ...
  it('drops an unrecognised change code rather than rendering it as prose', () => {
    ...
    expect(parsed!.recovery).toMatchObject({ hours: 34, change: null });
  });
...
describe('fmtRecovery', () => {
  ...
  it('counts hours, then days', () => {
    expect(fmtRecovery(1)).toBe('1 h');
    expect(fmtRecovery(23)).toBe('23 h');
    expect(fmtRecovery(24)).toBe('1 dzień');
    expect(fmtRecovery(34)).toBe('1 dzień 10 h');
    expect(fmtRecovery(50)).toBe('2 dni 2 h');
  });
});

describe('garminSummary', () => {
  it('leads with the level, then the timer, then the shared clauses', () => {
    const text = garminSummary('low', { day: '2026-08-14', hours: 34, change: null }, [
  ...
  it('states that recovery is done at zero hours', () => {
    const text = garminSummary('high', { day: '2026-08-14', hours: 0, change: null }, []);
