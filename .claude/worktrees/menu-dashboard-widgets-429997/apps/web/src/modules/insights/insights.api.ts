 ...
 * the pure engine, and assembles `InsightsData`. Gated by `detailed_analytics` consent.
 *
 * Pure over injected deps (`GarminService`, `ConsentService`, `Clock`). Never throws on
 * connected/consent/sidecar issues — it degrades to flags with empty results. The only thrown
