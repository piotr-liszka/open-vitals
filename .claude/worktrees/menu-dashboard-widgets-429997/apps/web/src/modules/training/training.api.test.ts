...
import { loadTrainingOverview, type TrainingDeps } from './training.api';
...
describe('loadTrainingOverview', () => {
  it('returns an empty, safe payload for a user with no activities', async () => {
    const data = await loadTrainingOverview(deps(), { userId: USER, range: RANGE });
  ...
  it('builds a PMC series from Garmin training load and extends it to today', async () => {
    ...
    const data = await loadTrainingOverview(d, { userId: USER });
  ...
  it('splits the window by sport family, busiest first, and links the families with a subpage', async () => {
    ...
    const data = await loadTrainingOverview(d, { userId: USER, range: RANGE });
    ...
    expect(ride.href).toBe('/training/rower');

    expect(data.sports.find((s) => s.group === 'run')!.href).toBe('/training/bieg');
    // Walking finally has a home of its own — this is the regression that mattered.
    expect(data.sports.find((s) => s.group === 'walk')!.href).toBe('/training/marsz');
  ...
  it('resizes the split window to the global range and leaves the PMC alone (spec 047)', async () => {
    ...
    const week = await loadTrainingOverview(d, { userId: USER, range: resolveRange('7', TODAY) });
    const month = await loadTrainingOverview(d, { userId: USER, range: resolveRange('30', TODAY) });
  ...
  describe('intensity mix (spec 044)', () => {
    it('splits the window‘s time into easy / moderate / hard against the athlete‘s max HR', async () => {
      ...
      const { intensityMix } = await loadTrainingOverview(d, { userId: USER });
