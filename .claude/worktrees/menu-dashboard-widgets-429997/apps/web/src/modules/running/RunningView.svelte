<script lang="ts">
  ...
  const efficiencyLabels = $derived(
    data.efficiency.map((m) =>
      m.month.endsWith('-01') ? formatMonth(m.month, 'shortYear') : formatMonth(m.month)
    )
  );
  /**
...
{:else}
  <div class="stack">
    ...
    <div class="section-head">
      <h2 class="section-title">Zakres: {data.range.label}</h2>
      <RangeBadge label={data.range.label} size="sm" />
    </div>
