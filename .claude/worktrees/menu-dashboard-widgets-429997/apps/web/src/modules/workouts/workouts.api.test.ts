 ...
 * this slice REUSES that validator rather than re-implementing it, that the consent gate holds on
...
function consentStub(enabled: boolean): ConsentService {
  return {
    async listFeatures() {
      return [];
    },
...
describe('the workout_write consent gate', () => {
  it('refuses every write when the feature is off', async () => {
    const d = deps(false);
    await expect(createWorkout(d, USER, draft())).rejects.toBeInstanceOf(WorkoutConsentError);
    await expect(updateWorkout(d, USER, 'x', draft())).rejects.toBeInstanceOf(WorkoutConsentError);
    await expect(deleteWorkout(d, USER, 'x')).rejects.toBeInstanceOf(WorkoutConsentError);
  });
...
describe('workoutErrorStatus', () => {
  it('maps a consent refusal to 403 and a validation failure to 400', () => {
    expect(workoutErrorStatus(new WorkoutConsentError('nope'))).toEqual({ status: 403, error: 'nope' });
...
describe('workout library (spec 069)', () => {
  ...
  it('gates every library write on workout_write, but not the read', async () => {
    const d = deps(false);
    await expect(createTemplate(d, USER, template())).rejects.toBeInstanceOf(WorkoutConsentError);
    await expect(updateTemplate(d, USER, 'x', template())).rejects.toBeInstanceOf(WorkoutConsentError);
    await expect(deleteTemplate(d, USER, 'x')).rejects.toBeInstanceOf(WorkoutConsentError);
