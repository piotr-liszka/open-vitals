<script lang="ts">
  ...
  /** Monday-first, matching `dayOfWeek` and Polish convention. */
  const WEEKDAYS = [
...
<div class="cal">
  <div class="bar">
    ...
    >
    <h3 class="month">{formatMonth(month, 'longYear')}</h3>
    <button
      type="button"
      class="nav"
            ...
            >
              ...
              >
                ...
                <span class="dots" aria-hidden="true">
                  {#each { length: Math.min(authored, 3) } as _, i (i)}<span class="dot mine"></span>{/each}
                  {#each { length: Math.min(planned, 3) } as _, i (i)}<span class="dot theirs"></span>{/each}
                </span>
                <span class="sr">
                  {formatDay(cell.day, 'long')}{authored > 0
                    ? `, ${authored} zaplanowanych treningów`
                    : ''}{planned > 0 ? `, ${planned} z kalendarza Garmina` : ''}
                </span>
              </button>
            </td>
          {/each}
  ...
  <p class="legend">
    <span class="dot mine"></span> Twój plan
    <span class="dot theirs"></span> Z Garmina
  </p>
</div>
