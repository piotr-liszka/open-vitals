...
describe('loadVolume', () => {
  it('reports no data for an athlete with nothing synced', async () => {
    const store = createMemoryStore();
    const data = await loadVolume({ store, clock }, { userId: 'u' });
  ...
  it('marks the current month partial and keeps it out of the average', async () => {
    ...
    expect(data.bestMonth?.month).toBe('2026-06');
  });
  ...
  describe('consistency grid (spec 046)', () => {
    it('builds per-day totals for the current year only', async () => {
      const store = await seeded([
      ...
      const data = await loadVolume({ store, clock }, { userId: 'u' });
      expect(data.gridYear).toBe(2026);
      expect(data.gridDays.map((d) => d.day)).toEqual(['2026-03-01']);
      // Two activities on the day are summed.
      expect(data.gridDays[0]!.km).toBeCloseTo(17.4, 1);
      expect(data.gridDays[0]!.title).toContain('2 aktywności');
    });
    ...
    it('is empty for an athlete with nothing this year', async () => {
      const store = await seeded([act({ startTimeLocal: '2025-03-01 09:00:00' })]);
      const data = await loadVolume({ store, clock }, { userId: 'u' });
