...
import { WORKOUT_WRITE_FEATURE } from './features/registry';
...
export function createContainer(overrides: ContainerOverrides = {}): AppContainer {
  ...
  const syncEngine = createSyncEngine({
    ...
    // env var to the user's own switch, so it is resolved per user, per run.
    workoutPushEnabledFor: (userId: string) => featuresFor(userId).isEnabled(WORKOUT_WRITE_FEATURE)
  });
