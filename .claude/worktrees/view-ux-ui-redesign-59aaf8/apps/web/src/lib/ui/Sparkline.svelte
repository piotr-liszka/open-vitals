<script lang="ts">
  ...
  const summary = $derived.by(() => {
    const prefix = label ? i18n.t('sparkline.trendOf', { label }) : i18n.t('sparkline.trend');
    if (n === 0) return i18n.t('sparkline.summaryEmpty', { prefix });
    return i18n.t('sparkline.summary', {
      prefix,
      count: n,
      latest: formatNumber(i18n.locale, latest!)
    });
  });
</script>
