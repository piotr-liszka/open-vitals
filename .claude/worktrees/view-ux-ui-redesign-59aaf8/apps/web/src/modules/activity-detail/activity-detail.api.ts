...
import type { ActivityLap, ActivitySummary, ActivityStreams } from '$lib/server/store/types';
...
/**
 * How far either side of the activity we look for calendar data. Finding nothing in a two-month
 * window is what lets us say "nothing was scheduled" instead of "we never synced the calendar".
 */
const CALENDAR_PROBE_DAYS = 30;

/** The scheduled side: what was planned for this day, and whether we hold any calendar at all. */
...
export async function loadActivityDetail(
  ...
  const derivedSpeed =
    activity.distanceM !== null && movingS !== null && movingS > 0 ? activity.distanceM / movingS : null;
