...
<script lang="ts">
  ...
  /**
   * When the recovery timer capped the score, the chips no longer add up to the number above them —
   * so the card says why, in one line, rather than leaving the reader to check the arithmetic and
   * conclude the gauge is broken (spec 070).
   */
  const limit = $derived(readiness.limitedBy);
</script>
...
<div class="gauge" class:lg={size === 'lg'}>
  ...
  {#if limit}
    <p class="limit">
      Kanały same dają <strong>{limit.uncapped}</strong>, ale zegar regeneracji Garmina (<strong
        >{fmtRecovery(limit.minutes)}</strong
      >) obniża wynik do <strong>{readiness.score}</strong>.
    </p>
  {/if}
</div>
