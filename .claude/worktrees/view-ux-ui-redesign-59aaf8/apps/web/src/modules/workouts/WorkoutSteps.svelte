<script lang="ts">
  ...
  import { STEP_KIND_LABELS, describeDuration, describeTarget } from './workout-format';
...
<ol class="steps">
  {#each steps as step, i (i)}
    <li class="step" class:repeat={step.kind === 'repeat'}>
      {#if step.kind === 'repeat'}
        <div class="row">
          <span class="kind">{STEP_KIND_LABELS.repeat}</span>
        ...
        <ol class="children">
          {#each step.steps ?? [] as child, j (j)}
            <li class="step child">
              <div class="row">
                <span class="kind kind-{child.kind}">{STEP_KIND_LABELS[child.kind]}</span>
                <span class="dur">{describeDuration(child)}</span>
                {#if describeTarget(child.target)}
                  <span class="target">{describeTarget(child.target)}</span>
                {/if}
              </div>
      ...
      {:else}
        <div class="row">
          <span class="kind kind-{step.kind}">{STEP_KIND_LABELS[step.kind]}</span>
          <span class="dur">{describeDuration(step)}</span>
          {#if describeTarget(step.target)}<span class="target">{describeTarget(step.target)}</span>{/if}
        </div>
