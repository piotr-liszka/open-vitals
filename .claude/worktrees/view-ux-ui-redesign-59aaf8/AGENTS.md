...
A self-hosted, **multi-tenant** service to talk to your **own** Garmin Connect data from AI clients. Each user
signs in with **Google (OIDC, passwordless)** and connects their **own** Garmin account; all data is per-user.
(Pivoted from the original single-user/LAN model in **spec 012** — internet-connected so Google sign-in works.)

- **Web app + API + MCP server:** one Node/TypeScript service built with **SvelteKit (Svelte 5) + adapter-node**.
- **Data:** a **Postgres** service holds users, sessions, and per-user connections/feature switches/settings/MCP tokens.
...
- After signing in the web app shows, per user: a **health check**, a **metrics dashboard**, and a **personal MCP URL**.
