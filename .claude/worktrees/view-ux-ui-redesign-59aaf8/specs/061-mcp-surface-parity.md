...
- [ ] A `lib/mcp/analysis-tools.ts` exposing, each gated by `detailed_analytics` and scoped to the resolved
      user: `get_race_predictions`, `get_power_profile`, `get_volume` (monthly + year-over-year),
      `get_weekly_summary` (spec 056).
      ~~`get_training_load`, `get_best_efforts`~~ — **done, in spec 079**, as `get_load_series` and
      `get_personal_bests`. Coaching feedback pulled them forward ahead of the rest of this list.
      ~~`list_activities`, `get_activity`~~ — **done, in spec 077**: coaching feedback made the lap
      splits the single most valuable item on this list, so they shipped on their own rather than
      waiting for the other six. This spec keeps the rest.
...
Four of this spec's original eight tools have shipped elsewhere, each pulled forward by the coaching
feedback of August 2026: `list_activities` and `get_activity_detail` in **spec 077**, and
`get_training_load` (as `get_load_series`) and `get_best_efforts` (as `get_personal_bests`) in
**spec 079**. What remains open here is the four listed above.
