...
import { MCP_FEATURE } from '../server/features/registry';
...
/**
 * Decide how to answer one `/mcp` request: reject it (401/403/429) or serve it for a resolved user.
...
export function gateMcpRequest(input: McpGateInput): Promise<McpGateDecision> {
  return mcpGate(
    {
      resolveUser: (token) => container().repo.mcpTokens.resolve(token),
      authFailures,
      perUser,
      // Spec 071: the owner can switch their own MCP endpoint off in Settings.
      mcpEnabledFor: (userId) => container().featuresFor(userId).isEnabled(MCP_FEATURE)
    },
    input
  );
}
...
export function newMcpServerForUser(userId: string): McpServer {
  ...
  return createMcpServer(
    ...
    // Training blocks (spec 073): the plan's memory. Local store only, like the season tools.
    {
      store: c.store,
      userId,
      clock: c.clock,
    ...
    // Subjective journal (spec 062): the athlete's own reports. Local store only.
    {
      store: c.store,
      userId,
      clock: c.clock,
    ...
    // Activity detail (spec 077): reuses the web page's own handler, so there is one query behind
    // the laps rather than two that can drift.
    { store: c.store, settings: c.repo.settings, userId },
    // Load / bests / time trial (spec 079): local store plus settings for FTP.
    {
      store: c.store,
      settings: c.repo.settings,
      userId,
