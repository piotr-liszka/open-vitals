/**
 * Shared date helpers (spec 018) — the single source of truth for day keys and date formatting.
 *
...
/* ------------------------------------------------------------------ *
 * Formatting (via Intl — no hand-rolled month tables)
 * ------------------------------------------------------------------ */

/**
 * Every formatter below takes the LOCALE FIRST and takes it REQUIRED (spec 076).
 *
 * Required, because a default would be a silent one: a call site that forgot to pass the active
 * language would keep rendering Polish month names inside an English page, and nothing would fail.
 * Making it the first parameter is what turns that into a compile error — `DayKey` is a `string`,
 * so a locale added anywhere else would have slotted in beside it unnoticed.
 */
...
function formatter(
  ...
  const cacheKey = `${locale}|${timeZone}|${style}`;
  let fmt = formatters.get(cacheKey);
  if (!fmt) {
    fmt = new Intl.DateTimeFormat(intlLocale(locale), { timeZone, ...options });
...
/**
 * Render a calendar day in the given language. The key is pinned to UTC midnight and formatted in
 * UTC, so the output is identical on the server and in the browser regardless of either one's
 * timezone.
 */
export function formatDay(locale: Locale, key: DayKey, style: DayStyle = 'short'): string {
  const { year, month, day } = parseDayKey(key);
  if (style === 'iso') return key;
  const at = new Date(Date.UTC(year, month - 1, day));
  if (year >= 0 && year < 100) at.setUTCFullYear(year); // Date.UTC maps 0..99 into 1900+
  return formatter(locale, 'UTC', `day:${style}`, DAY_STYLE_OPTIONS[style]).format(at);
}
...
/**
 * Render a calendar month in the given language. Pinned to UTC midnight on the 1st and formatted in
 * UTC, so the output is identical on the server and in the browser — the same rule `formatDay`
 * follows.
 */
export function formatMonth(locale: Locale, month: MonthKey, style: MonthStyle = 'short'): string {
  const { year, month: m } = parseMonthKey(month);
  const at = new Date(Date.UTC(year, m - 1, 1));
  if (year >= 0 && year < 100) at.setUTCFullYear(year);
  return formatter(locale, 'UTC', `month:${style}`, MONTH_STYLE_OPTIONS[style]).format(at);
}

/**
 * Render a real instant in the given language, in an explicit timezone (default: the app timezone).
 * Returns `''` for input that is not a parseable instant, so a bad timestamp degrades to blank
 * instead of "Invalid Date". Never pass a bare `YYYY-MM-DD` here — use `formatDay`.
 */
export function formatInstant(
  ...
  return formatter(locale, timeZone, `instant:${style}`, INSTANT_STYLE_OPTIONS[style]).format(at);
}
