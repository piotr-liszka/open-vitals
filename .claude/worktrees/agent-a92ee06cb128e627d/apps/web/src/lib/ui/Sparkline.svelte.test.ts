...
describe('Sparkline', () => {
  ...
  it('summarises the trend in the aria-label', () => {
    ...
    expect(label).toBe('trend: steps, 3 punkty, ostatnio 9204');
  });
  ...
  it('handles an empty array with a placeholder and no-data label', () => {
    ...
    const svg = container.querySelector('svg');
    expect(svg).not.toBeNull();
    expect(svg?.getAttribute('aria-label')).toBe('trend: steps, brak danych');
  ...
  it('renders a single point without a connecting line', () => {
    ...
    expect(container.querySelector('svg')?.getAttribute('aria-label')).toBe('trend, 1 punkt, ostatnio 42');
  });
