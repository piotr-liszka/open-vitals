...
const state = { url: new URL('http://openvitals.test/') };
...
describe('RangeSwitch', () => {
  ...
  it('falls back to the default for a hand-typed range', () => {
    state.url = new URL('http://openvitals.test/?range=9999');
    render(RangeSwitch);
    expect(screen.getByRole('radio', { name: '7 dni' }).getAttribute('aria-checked')).toBe('true');
  });

  it('navigates with the new range and re-runs the loaders', async () => {
    state.url = new URL('http://openvitals.test/insights?range=7');
    render(RangeSwitch);
  ...
  it('keeps the page and its other query parameters', async () => {
    state.url = new URL('http://openvitals.test/activities?sport=running&sort=distance&range=7');
    render(RangeSwitch);
