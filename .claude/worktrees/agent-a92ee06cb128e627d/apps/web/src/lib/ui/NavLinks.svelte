<script lang="ts">
   ...
   * route via `$app/stores`, so `AppShell` just drops `<NavLinks />` into the sidebar.
   *
   ...
   * them from page data instead of every page passing them down. Absent for signed-out readers,
   * who have no `Panele` group.
   *
  ...
  const groups = $derived(navGroups(i18n.t, dashboards));

  /**
   * The single current destination, resolved across ALL rendered items by longest match (spec 088) —
   * `/training/plan` marks `Plan treningowy` and leaves `Analiza` alone. Computed once per render
   * rather than per item, because "is this one active" is not answerable by looking at one item.
   */
