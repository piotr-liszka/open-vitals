<script lang="ts">
  ...
  const fmtBucket = (start: string): string => bucketAxisLabel(i18n.locale, start, data.range.bucket);
</script>
...
<div class="bars">
  ...
  {#if hidden > 0}
    <p class="note">
      {i18n.t('widget.showingLastOf', { shown: shown.length, total: all.length })}
    </p>
  {/if}
</div>
