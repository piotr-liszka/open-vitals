...
export function defaultConfig(t: Translator): DashboardConfig {
  return {
    dashboards: [
      {
        id: 'main',
...
function sanitizeDashboard(t: Translator, raw: unknown, i: number): DashboardDef | null {
  if (!raw || typeof raw !== 'object') return null;
  const o = raw as Record<string, unknown>;
  const name =
 ...
 */
export function sanitizeConfig(t: Translator, raw: unknown): DashboardConfig {
  if (!raw || typeof raw !== 'object') return defaultConfig(t);
  const o = raw as Record<string, unknown>;
  const parsed = Array.isArray(o.dashboards)
    ? o.dashboards.map((d, i) => sanitizeDashboard(t, d, i)).filter((d): d is DashboardDef => d !== null)
    : [];
  if (parsed.length === 0) return defaultConfig(t);
...
/**
 * The names minted here (the starter dashboard, an unnamed panel's fallback) are PERSISTED, so they
 * are written in whatever language the user held when the panel first appeared — and stay that way
 * until renamed, which is right: a name someone may have edited must not change under them.
 */
export async function getConfig(
  ...
  return KEY in bag ? sanitizeConfig(t, bag[KEY]) : defaultConfig(t);
}
...
export async function saveConfig(
  ...
  const clean = sanitizeConfig(t, raw);
  const bag = await settings.get(userId);
