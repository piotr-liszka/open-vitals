...
const hrBlock: HrBlock = { avg: 140, max: 180, zones: buckets([300, 300, 200, 150, 50]) };
...
describe('ActivityZones — zone explanation', () => {
  ...
  it('says the FTP was estimated, and from what', async () => {
    ...
    // No screen writes an FTP today, so the estimate is what everyone sees — say so.
    expect(text).toContain(t('zones.ftpNoSettings'));
  ...
  it('says the FTP was configured when it came from settings', async () => {
    ...
    expect(text).not.toContain(t('zones.ftpNoSettings'));
  });
