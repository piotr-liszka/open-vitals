<script lang="ts">
  ...
  const sportChips = $derived<FilterChipOption[]>(
    data.sports.map((s) => ({ value: s.sport, label: sportLabel(i18n.t, s.sport) }))
  );
  ...
  const km = $derived(`${formatInteger(i18n.locale, data.totalDistanceM / 1000)} km`);
...
<div class="layout">
  <aside class="rail">
    <div class="tiles">
      <StatTile label={i18n.t('training.tile.activities')} value={String(data.count)} accent="orange" />
      <StatTile label="Dystans" value={km} accent="green" />
      <StatTile label={i18n.t('heatmap.withGpsTrack')} value={String(data.tracks.length)} accent="cyan" />
    </div>
    ...
    {#if data.tracks.length === 0}
      <p class="empty">
        {i18n.t('heatmap.empty')} <a href="/data">{i18n.t('nav.data')}</a>.
      </p>
    {/if}
  </aside>

  <div class="map">
    <LeafletMap {polylines} height="100%" ariaLabel={i18n.t('heatmap.mapLabel')} />
  </div>
</div>
