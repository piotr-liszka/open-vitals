...
export async function loadWeeklySummary(
  ...
  const sports: WeeklySummarySport[] = [...groups]
    .map((group) => {
      ...
      return {
        ...
        window: weekly.reduce<WeeklySummaryTotals>((acc, w) => addTotals(acc, w), emptyTotals()),
        weekly: weekly.map((w) => ({
          week: w.week,
  ...
  /*
   * Spec 089: the same weeks, added up. A fold over the arrays above — never a second read, and
   * never a second `weeklyVolume()` pass, so "Wszystko" and the chips behind it cannot disagree.
   * `null` for a one-sport athlete: see `combineSports`.
   */
  const combined = combineSports(sports);

  return {
    weeks: WEEKLY_SUMMARY_WEEKS,
    weekStarts,
    monthLabels: monthChangeLabels(req.locale, weekStarts),
    ...
    sports,
    combined,
    // Open on the whole week when there is a whole week to show; otherwise on the busiest family.
    defaultGroup: defaultSelection(combined !== null, sports),
    hasData: sports.length > 0
  };
}

/** What the card opens on. "All" only when there is more than one family to add up. */
function defaultSelection(
  hasCombined: boolean,
  sports: readonly WeeklySummarySport[]
): WeeklySummarySelection | null {
  if (hasCombined) return ALL_SPORTS;
  return sports[0]?.group ?? null;
}
 ...
 */
function monthChangeLabels(locale: Locale, weekStarts: readonly DayKey[]): string[] {
  let previous = '';
  return weekStarts.map((week) => {
    const month = monthKeyOf(week);
    if (month === previous) return '';
    previous = month;
    return formatMonth(locale, month, 'short');
  });
}
