<script lang="ts">
  ...
  const lastLabel = $derived(
    status?.lastSyncAt ? formatInstant(i18n.locale, status.lastSyncAt, 'dateTime') : i18n.t('sync.never')
  );
  ...
  const unchangedAt = $derived.by(() => {
    ...
    return formatInstant(i18n.locale, lastCheckAt, 'time');
  });
  ...
  const autoLabel = $derived.by(() => {
    ...
    if (nextInMin <= 1) return i18n.t('sync.autoImminent');
    return i18n.t('sync.autoIn', { minutes: nextInMin });
  });
...
{#if available}
  <div class="sync">
    <span class="label">{i18n.t('sync.lastSync')}</span>
    ...
    <div class="row">
      ...
      <IconButton
        ...
        loading={running}
        onclick={syncNow}
        label={i18n.t(running ? 'sync.inProgress' : 'sync.syncNow')}
      />
    </div>

    {#if running}
      <span class="note running" aria-live="polite">
        {i18n.t('sync.syncing')}{progressPct !== null ? ` · ${progressPct}%` : ''}
      </span>
    {:else if failed}
      <span class="note bad"
        >{i18n.t('sync.lastAttemptFailed')} — <a href="/data">{i18n.t('sync.details')}</a></span
      >
    {:else if autoLabel}
      <span class="note auto">
        ...
        {autoLabel}{unchangedAt ? ` · ${i18n.t('sync.unchangedAt', { at: unchangedAt })}` : ''}
      </span>
    {:else if unchangedAt}
      <span class="note">{i18n.t('sync.checkedUnchanged', { at: unchangedAt })}</span>
    {/if}
  </div>
{/if}
