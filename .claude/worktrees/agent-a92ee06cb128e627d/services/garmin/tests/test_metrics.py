...
def test_training_readiness_unwraps_single_item_list(client: TestClient) -> None:
    ...
    # spec 075: the web tier counts the recovery timer down from Garmin's own
    # capture instant, so this document must reach it with `timestamp` intact.
    assert data["timestamp"] == "2026-08-01T15:54:57.0"
