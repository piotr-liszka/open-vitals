...
export const ICONS: Readonly<Record<IconName, IconGlyph>> = {
  ...
  /* ---- status marks (spec 072) ----
     The three glyphs a notification badge needs. Drawn bare — no enclosing ring — because the badge
     that carries them already IS the circle; the "i in a circle" construction would double it.
     Deliberately wider than the type-scale glyphs above (they run 4→20 rather than 5→19) so they
     still read as a confident mark once shrunk into a 20px badge. */
  check: {
    paths: ['M4.6 12.4 9.5 17.4 19.4 6.9']
  },
  x: {
    paths: ['M6 6 18 18', 'M18 6 6 18']
  },
  /* A bare lowercase "i": stem plus the same round-cap dot the `alert`/`help` glyphs use. */
  info: {
    paths: ['M12 11v6.6', 'M12 6.8h.01']
  }
};
