...
export type McpGateDecision =
  ...
  | { action: 'reject'; status: 401 | 403 | 429; headers: Record<string, string>; body: string };
...
export interface McpGateDeps {
  ...
  perUser: RateLimiter;
  /**
   * Whether the resolved user has the MCP switch on (spec 071). Optional so existing callers and
   * tests keep working; absent means "on", which is the pre-071 behaviour.
   */
  mcpEnabledFor?: (userId: string) => Promise<boolean>;
}
...
export async function mcpGate(deps: McpGateDeps, input: McpGateInput): Promise<McpGateDecision> {
  ...
  const reject = (status: 401 | 403 | 429, error: string, retryAfter?: number): McpGateDecision => ({
    action: 'reject',
    status,
    headers: {
      ...headers,
  ...
  // The user's own switch (spec 071). Checked AFTER the token resolves and charged to nobody: this
  // is the owner turning their own endpoint off, not a failed authentication, so it must not eat the
  // per-IP failure budget. 403 rather than 401 — the credential was fine, the capability is not on.
  if (deps.mcpEnabledFor && !(await deps.mcpEnabledFor(userId))) {
    return reject(403, 'mcp_disabled');
  }
