...
describe('mcpGate', () => {
  ...
  // Spec 071: the owner can switch their own MCP endpoint off in Settings.
  it('rejects a resolved user whose MCP switch is off with 403, not 401', async () => {
    const deps = { ...makeDeps({ 'good-token': 'user-1' }), mcpEnabledFor: async () => false };
    const decision = await mcpGate(deps, { ...INPUT, token: 'good-token' });

    expect(decision.action).toBe('reject');
    if (decision.action !== 'reject') return;
    expect(decision.status).toBe(403);
    expect(JSON.parse(decision.body)).toEqual({ error: 'mcp_disabled' });
  });

  it('does not charge the auth-failure budget for a switched-off endpoint', async () => {
    // The credential was VALID. Spending the per-IP brute-force budget on it would let a user lock
    // themselves out of their own endpoint just by leaving a client polling while it is off.
    const deps = {
      ...makeDeps({ 'good-token': 'user-1' }, { failureLimit: 2 }),
      mcpEnabledFor: async () => false
    };
    for (let i = 0; i < 5; i++) {
      const d = await mcpGate(deps, { ...INPUT, token: 'good-token' });
      expect(d.action === 'reject' && d.status).toBe(403);
    }
  });

  it('serves normally when the switch is on', async () => {
    const deps = { ...makeDeps({ 'good-token': 'user-1' }), mcpEnabledFor: async () => true };
    const decision = await mcpGate(deps, { ...INPUT, token: 'good-token' });
    expect(decision.action).toBe('serve');
  });
