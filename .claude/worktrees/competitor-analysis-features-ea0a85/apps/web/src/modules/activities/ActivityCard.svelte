<script lang="ts">
  ...
  const meters = (m: number | null): string =>
    m == null ? '—' : `${Math.round(m).toLocaleString('pl-PL')} m`;
  ...
  function dateLabel(local: string): string {
    ...
    return isDayKey(head) ? formatDay(head, 'shortYear') : head;
  }
...
<a class="card {view}" href={`/activities/${item.id}`}>
  <div class="thumb">
    ...
    {/if}
    <span class="sport"><Badge tone="neutral" dot={false}>{sportLabel(item.sport)}</Badge></span>
  </div>

  <div class="body">
    <div class="head">
      <h3 class="name">{item.name ?? sportLabel(item.sport)}</h3>
