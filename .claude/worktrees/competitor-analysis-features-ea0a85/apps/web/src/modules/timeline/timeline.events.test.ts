...
describe('formatting', () => {
  it('renders distance in km above a kilometre and in metres below it', () => {
    expect(fmtDistance(18_400)).toBe('18,4 km');
...
describe('activityStats', () => {
  ...
  it('leads a ride with distance and average power', () => {
    const stats = activityStats(act({ activityId: 'c', sport: 'cycling', avgPower: 214 }));
  ...
  it('falls back to average speed when a ride has no power', () => {
    const stats = activityStats(act({ activityId: 'c2', sport: 'cycling', avgPower: null }));
  ...
  it('never renders more than three readouts', () => {
    const stats = activityStats(act({ activityId: 'x', sport: 'running', elevationGainM: 900 }));
...
describe('importance', () => {
  it('ranks an HRV anomaly above a routine easy walk', () => {
    ...
    const [hrv] = buildHealthEvents([signal()], FROM, TO);
  ...
  it('scores a strong outlier above a moderate one', () => {
    const [moderate] = buildHealthEvents([signal({ z: -2.1, severity: 'moderate' })], FROM, TO);
...
describe('buildHealthEvents', () => {
  ...
  it('marks a move in the healthy direction as favourable', () => {
    const [drop] = buildHealthEvents([signal({ key: 'hrv', direction: 'down' })], FROM, TO);
    const [rise] = buildHealthEvents([signal({ key: 'hrv', direction: 'up', date: '2026-08-02' })], FROM, TO);
  ...
  it('rejects structurally unsound signals instead of rendering junk', () => {
    ...
    expect(buildHealthEvents([signal({ date: 'not-a-day' })], FROM, TO)).toEqual([]);
  });

  it('drops signals outside the window', () => {
    expect(buildHealthEvents([signal({ date: '2026-01-01' })], FROM, TO)).toEqual([]);
  });
});
...
describe('milestones', () => {
  ...
  it('emits a streak milestone on the day the run reaches a full week', () => {
    ...
    const events = buildMilestoneEvents(week, FROM, TO);
