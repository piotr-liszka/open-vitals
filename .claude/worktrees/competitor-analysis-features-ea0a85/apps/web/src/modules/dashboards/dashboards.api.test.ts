...
describe('POST /api/dashboards contract', () => {
  it('returns the sanitized document, not the one it was handed', async () => {
    ...
    const saved = await saveConfig(settings, USER, {
  ...
  it('round-trips a reordered widget list unchanged', async () => {
    ...
    await saveConfig(settings, USER, ordered);
    const read = await getConfig(settings, USER);
  ...
  it('keeps one user out of another user config', async () => {
    const settings = createMemorySettingsRepo();
    await saveConfig(settings, USER, cfg('mine'));
    await saveConfig(settings, OTHER, cfg('theirs'));

    expect((await getConfig(settings, USER)).dashboards.map((d) => d.id)).toEqual(['mine']);
    expect((await getConfig(settings, OTHER)).dashboards.map((d) => d.id)).toEqual(['theirs']);
  });

  it('leaves the rest of the settings bag alone', async () => {
    const settings = createMemorySettingsRepo();
    await settings.set(USER, { ftpWatts: 260 });
    await saveConfig(settings, USER, cfg('main'));
