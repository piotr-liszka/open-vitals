<script lang="ts">
  ...
  const windowNote = $derived(
    profile.window.weeks > 0
      ? `z ostatnich ${profile.window.weeks} tygodni`
      : 'dopiero wtedy, gdy uzbiera się kilka tygodni historii'
  );
</script>

<Card title="Profil biegacza" subtitle="Pięć osi policzonych z Twoich zsynchronizowanych biegów">
  <div class="profile">
    <div class="plot">
      <RadarChart axes={radarAxes} ariaLabel="Profil biegacza — pięć osi" color="var(--lane-orange)" />
    </div>

    <div class="read">
      <div class="verdict">
        <span class="kicker">Twój typ</span>
        <h4 class="type">{profile.archetype.label}</h4>
        <p class="summary">{profile.archetype.summary}</p>
        {#if profile.strength || profile.weakness}
          <div class="marks">
            {#if profile.strength}
              <Badge tone="success">Mocna strona: {labelOf(profile.strength)}</Badge>
            {/if}
            {#if profile.weakness}
              <Badge tone="neutral">Do poprawy: {labelOf(profile.weakness)}</Badge>
            {/if}
          </div>
        {/if}
      </div>
      ...
      <dl class="axes">
        {#each profile.axes as axis (axis.key)}
          <div class="axis" class:missing={axis.score === null}>
            ...
            <dd>
              <span class="readout">{axis.readout ?? 'brak danych'}</span>
  ...
  <p class="scale">
    Skala odniesienia: <strong>0</strong> to poziom początkujący, <strong>100</strong> wyczynowy — to miara
    <em>kształtu</em>
    profilu, nie test sprawności. Osie tempa liczymy z rekordów życiowych, a objętość i regularność {windowNote}.
    Przerywana oś oznacza brak danych, nie zero.
  </p>
</Card>
