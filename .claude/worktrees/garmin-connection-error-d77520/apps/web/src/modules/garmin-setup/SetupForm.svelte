  ...
  async function submit(event: SubmitEvent): Promise<void> {
    ...
    try {
      ...
      if (res.status === 200 && 'outcome' in data && data.outcome === 'success') {
        toasts.success('Połączono konto Garmin.');
        password = '';
        mfaCode = '';
        mfaRequired = false;
        await invalidateAll();
      ...
      if (res.status === 202 && 'outcome' in data && data.outcome === 'mfa_required') {
        mfaRequired = true;
        error = 'Wpisz kod weryfikacyjny, który właśnie wysłał Garmin.';
        return;
      }
      if (res.status === 401) {
        error = 'Garmin odrzucił te dane. Sprawdź adres e-mail, hasło' + (mfaRequired ? ' i kod.' : '.');
        return;
      }
      error = 'error' in data ? data.error : 'Konfiguracja nie powiodła się. Spróbuj ponownie.';
    } catch {
...
<form onsubmit={submit} novalidate>
  ...
  {#if mfaRequired}
    <Field
      ...
      {error}
      help="Wysłany przez Garmin e-mailem lub w aplikacji uwierzytelniającej."
    >
      {#snippet children(control)}
        <Input
          id={control.id}
  ...
  <div class="row">
    ...
    >
      {mfaRequired ? 'Zweryfikuj i połącz' : 'Połącz Garmina'}
    </Button>
  </div>
</form>
