<script lang="ts">
  ...
  // ---- formatting ----
  const fmtDate = (day: string | null): string => (day ? formatDay(day, 'shortYear') : '—');
...
<div class="power">
  {#if !data.hasPower}
    <Card title="Brak danych o mocy" subtitle="Profil mocy wymaga aktywności z pomiarem mocy.">
      <p class="empty">
        Nie znaleziono strumieni mocy. Uruchom synchronizację w zakładce <a href="/data">Dane</a>.
      </p>
    </Card>
  {:else}
    <div class="grid-2">
        ...
        <div class="ftp">
          <div class="ftp-main">
            ...
            <span class="ftp-sub"
              ...
              {#if data.ftpSource === 'settings'}<Badge tone="info">z ustawień</Badge
    ...
    <Card title="Rekordy mocy (all-time)" subtitle="Najlepsza średnia moc dla każdego czasu trwania">
      <div class="tiles">
    ...
    {#if data.yearCurves.length > 0}
      ...
      {#if tableCols.length > 0}
        <Card title="Najlepsze wyniki wg roku">
          <Table zebra caption="Najlepsza moc (W) dla wybranych czasów, według roku">
            {#snippet head()}
              <th scope="col">Rok</th>
