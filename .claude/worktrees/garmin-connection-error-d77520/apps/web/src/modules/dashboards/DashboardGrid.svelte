<script lang="ts">
  ...
  import { bucketNoun } from '$lib/series';
        ...
        <div class="picker">
          {#each WIDGET_TYPES as t (t)}
            <button type="button" class="pick" onclick={() => addWidget(t)}>
              <span class="pick-name">{WIDGETS[t].label}</span>
              <span class="pick-desc">{WIDGETS[t].description}</span>
            </button>
          {/each}
        </div>
      ...
      >
        <Card>
          <div class="wtitle">
            ...
            {/if}
            <span class="wname">{def.label}</span>
            <!--
          ...
          -->
          {#if def.seeAlso}
            <a class="wsee" href={def.seeAlso.href}>{def.seeAlso.label} →</a>
          {/if}
        </Card>
      </section>
    {:else}
