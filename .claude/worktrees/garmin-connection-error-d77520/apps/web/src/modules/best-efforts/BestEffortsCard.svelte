...
>
  {#if data.hasData}
              ...
              <li>
                <a class="row" class:top={entry.rank === 1} href={`/activities/${entry.activityId}`}>
                  <span class="rank">
                    <RankMedal
                      rank={entry.rank}
                      label={entry.rank === 1 ? 'PR' : undefined}
                  ...
                  <span class="day">{formatDay(entry.day, 'shortYear')}</span>
                </a>
              </li>
            {/each}
          </ol>
    ...
    <p class="note">
      Odcinek to najszybsze okno pokrywające <em>co najmniej</em> dany dystans, wyszukane w zapisanym tempie
      całej sesji — dlatego 5 km z długiego wybiegania liczy się tak samo jak 5 km z zawodów. Pokazujemy do
      {data.topN} najlepszych wyników na dystans; kliknij wiersz, żeby otworzyć tę aktywność.
    </p>
  {:else}
    <p class="empty">
      Brak rekordów. Odcinki liczymy z zapisanego tempa biegów — pojawią się tutaj, gdy synchronizacja
      pobierze i przeliczy strumienie aktywności (zakładka <a href="/dane">Dane</a>).
    </p>
  {/if}
</Card>
