<script lang="ts">
  ...
  function niceDate(s: string | null): string {
    ...
    return isDayKey(head) ? formatDay(head, 'iso') : '—';
  }
  ...
  const logTime = (iso: string): string => formatInstant(iso, 'timeSeconds');
  const ago = (iso: string | null): string => (iso ? formatInstant(iso, 'dateTime') : 'nigdy');
  ...
  const sidecarTime = (t: number): string => formatInstant(new Date(t * 1000), 'timeSeconds');
...
<div class="data">
  ...
  {#if connected && watchBehind}
    <Banner tone="warning">
      Garmin ma dane najwyżej z <strong>{formatDay(coverage.freshness.lastDataDay!)}</strong>
    ...
    <div class="cov-table" role="table">
      ...
      {#each coverage.metrics as m (m.metric)}
        <div class="cov-row" role="row">
          <span role="cell">{metricLabel(m.metric)}</span>
