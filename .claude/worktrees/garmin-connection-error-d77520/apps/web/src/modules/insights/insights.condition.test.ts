...
describe('conditionSummary', () => {
  it('names the state and the two biggest movers, in Polish, in one sentence', () => {
    ...
    const text = conditionSummary('rested', [hrv, rhr], night);
  ...
  it('stays honest when there is nothing to interpret', () => {
    expect(conditionSummary('unknown', [], null)).toContain('Za mało danych');
    expect(conditionSummary('steady', [], null)).toBe('Regeneracja idzie swoim torem.');
  });
});
...
describe('computeCondition', () => {
  ...
  it('assembles sleep, the four recovery channels, readiness and a sentence', () => {
    const snapshot = computeCondition(series(), readiness(), TODAY)!;
  ...
  it('still reports when only some channels exist', () => {
    const snapshot = computeCondition([{ spec: spec('hrv'), days: days([50, 60]), raw: [] }], null, TODAY)!;
  ...
  it('measures how far behind today the snapshot is, and says so in the sentence', () => {
    const snapshot = computeCondition(series(), readiness(), '2026-08-08')!;
  ...
  it('returns null when there is genuinely nothing to show', () => {
    expect(computeCondition([], null, TODAY)).toBeNull();
    expect(
      computeCondition([{ spec: spec('hrv'), days: days([null, null]), raw: [] }], null, TODAY)
    ).toBeNull();
  });
});
...
describe('condition formatting', () => {
  it('renders durations, channel values and deltas the same way everywhere', () => {
    ...
    expect(fmtChannelValue(sleepTrend)).toBe('7 h 31 min');

    const hrv = buildConditionMetric(spec('hrv'), days([50, 50, 60]))!;
    expect(fmtChannelValue(hrv)).toBe('60');
    expect(fmtDelta(hrv)).toBe('+20,0%');

    const flat = buildConditionMetric(spec('hrv'), days([100, 100, 100]))!;
    expect(fmtDelta(flat)).toBeNull();
  });
});
