...
function frozen(minutes: number, day = '2026-08-14'): RecoveryTime {
  return { day, minutes, change: null, capturedAt: null, endsAt: null, superseded: false };
}

describe('garminSummary', () => {
  it('leads with the level, then the timer, then the shared clauses', () => {
    const text = garminSummary('low', frozen(2040), ['HRV poniżej bazy (88 ms)', 'sen 7 h 24 min']);
    expect(text).toBe(
  ...
  it('states that recovery is done at zero', () => {
    const text = garminSummary('high', frozen(0), []);
...
describe('toGarminReadiness', () => {
  it('assembles the card contract', () => {
    ...
    const readiness = toGarminReadiness(parsed, ['sen 7 h 24 min'], '2026-08-14');
...
describe('staleness (spec 072)', () => {
  ...
  it('carries the age into the card contract', () => {
    const parsed = parseTrainingReadinessDay('2026-08-14', payload())!;
    expect(toGarminReadiness(parsed, [], '2026-08-14').staleDays).toBe(0);
    expect(toGarminReadiness(parsed, [], '2026-08-16').staleDays).toBe(2);
  });

  it('says the day in the sentence, and marks the recovery timer as of that day', () => {
    const parsed = parseTrainingReadinessDay('2026-08-14', payload())!;
    const text = toGarminReadiness(parsed, [], '2026-08-16').summary;
  ...
  it('leaves a current reading untouched — no new chrome on the normal path', () => {
    const parsed = parseTrainingReadinessDay('2026-08-14', payload())!;
    const text = toGarminReadiness(parsed, ['sen 7 h 24 min'], '2026-08-14').summary;

    expect(text).not.toContain('nieaktualne');
    expect(text).not.toContain('dane z');
    expect(text).not.toContain('stan na ten dzień');
    expect(text).toBe(`Garmin: gotowość niska — do pełnej regeneracji ${fmtRecovery(2040)}, sen 7 h 24 min.`);
  });
});
...
describe('recovery as an instant (spec 075)', () => {
  ...
  it('turns the live row into the instant full recovery lands on', () => {
    ...
    expect(parsed.recovery).toEqual({
      day: '2026-08-16',
      minutes: 949,
      change: 'bez zmian po śnie', // the code the real account emits; it rendered nothing before
      capturedAt: CAPTURED,
      endsAt: ENDS,
      superseded: false
    });
  ...
  describe('the sentence', () => {
    ...
    it('counts the timer down instead of repeating the captured figure', () => {
      ...
      const text = toGarminReadiness(parsed(), [], '2026-08-16', CAPTURED + 6 * 60 * 60_000).summary;
      expect(text).toContain(`do pełnej regeneracji ${fmtRecovery(589)}`);
      expect(text).not.toContain(fmtRecovery(949));
    });
    ...
    it('drops the "as of that day" qualifier once the figure is genuinely current', () => {
      ...
      const text = toGarminReadiness(parsed(), [], '2026-08-18', CAPTURED + 3 * 60 * 60_000).summary;
    ...
    it('reaches "regeneracja zakończona" with no sync at all', () => {
      const text = toGarminReadiness(parsed(), [], '2026-08-17', ENDS + 60_000).summary;
    ...
    it('keeps the frozen wording when a later session superseded the reading', () => {
      ...
      const text = toGarminReadiness(superseded, [], '2026-08-18', CAPTURED + 6 * 3600_000).summary;
      // The stored figure, dated — never a countdown Garmin has already thrown away.
      expect(text).toContain(`do pełnej regeneracji ${fmtRecovery(949)}`);
      expect(text).toContain('stan na ten dzień');
    });
  });
});
