...
describe('fmtRecoveryEnd', () => {
  it('names the moment in the app timezone, relative to today', () => {
    ...
    expect(fmtRecoveryEnd(ENDS, CAPTURED)).toBe('jutro 09:43');
    expect(fmtRecoveryEnd(ENDS, ENDS - 60_000)).toBe('dziś 09:43');
  });

  it('dates anything past tomorrow rather than saying "in 3 days"', () => {
    const far = Date.UTC(2026, 7, 19, 7, 43, 57);
    expect(fmtRecoveryEnd(far, CAPTURED)).toBe('19 sie, 09:43');
  });

  it('renders in the zone it is given, not the host’s', () => {
    expect(fmtRecoveryEnd(ENDS, CAPTURED, 'UTC')).toBe('jutro 07:43');
  });

  it('has nothing to say without an instant or a clock', () => {
    expect(fmtRecoveryEnd(null, CAPTURED)).toBeNull();
    expect(fmtRecoveryEnd(ENDS, null)).toBeNull();
  });
});
