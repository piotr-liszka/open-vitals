<script lang="ts">
  ...
  const recoveryEnd = $derived(recovery ? fmtRecoveryEnd(recovery.endsAt, nowMs) : null);
...
>
  ...
  {:else}
    {#if stale}
      <Banner tone="warning">
        Garmin nie ma danych nowszych niż {formatDay(condition.day!)} — wszystko poniżej opisuje tamten dzień, nie
        dzisiaj. Nasza synchronizacja pobrała już wszystko, co Garmin ma: zsynchronizuj zegarek z aplikacją Garmin
        Connect na telefonie.
      </Banner>
    {/if}
          ...
          {#if recovery}
            <div class="recovery">
              ...
              <span class="recovery-label">
                ...
                {:else if stale}
                  do pełnej regeneracji wg Garmina — stan na {formatDay(recovery.day)}
                {:else}
                  do pełnej regeneracji wg Garmina
                {/if}
              </span>
