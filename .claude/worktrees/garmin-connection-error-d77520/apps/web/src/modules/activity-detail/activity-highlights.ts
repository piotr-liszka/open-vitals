...
/** Polish month count, correctly inflected: 1 miesiąca, 2-4 miesięcy … (genitive throughout). */
...
function spanText(history: readonly HighlightActivity[], day: string): string {
  const days = history
    .map((h) => h.day)
    .filter(isDayKey)
    .sort();
  const oldest = days[0];
  if (oldest === undefined) return 'zsynchronizowanej historii';
...
export function buildHighlights(input: HighlightsInput): ActivityHighlight[] {
  ...
  for (const metric of METRICS) {
    ...
    if (rank === 1 && tied > MAX_TIES_FOR_BEST) {
      ...
      text = coversAllHistory
        ? 'Wyrównany najlepszy wynik w historii'
        : `Wyrównany najlepszy wynik w ostatnich ${spanText(history, current.day)}`;
    } else if (rank === 1) {
      kind = 'record';
      text = coversAllHistory
        ? 'Rekord — najlepszy wynik w historii'
        : `Najlepszy wynik w ostatnich ${spanText(history, current.day)}`;
    } else if (monthsSinceBetter !== null && monthsSinceBetter >= NOTABLE_MONTHS) {
      kind = 'notable';
      text = `Najlepszy od ${monthsText(monthsSinceBetter)}`;
    } else if (rank <= TOP_RANK) {
      kind = 'notable';
      text = coversAllHistory
        ? `${rank}. najlepszy wynik w historii`
        : `${rank}. najlepszy wynik w ostatnich ${spanText(history, current.day)}`;
    }
...
export function buildSuspects(input: SuspectInput): SuspectValue[] {
