...
{#if splits.length > 0}
  <Card title="Bieg, marsz i postoje" subtitle="Klasyfikacja Garmina — udział czasu">
    <StackedBar
      {segments}
        ...
        <li>
          ...
          <span class="s-meta">
            ...
            {#if split.distanceM !== null}
              <span class="s-dot">·</span>{fmtNum(split.distanceM / 1000, 2)} km
            {/if}
...
{#if table}
  <Card title="Okrążenia" subtitle={`${table.rows.length} odcinków zapisanych przez zegarek`}>
    <Table zebra caption="Statystyki poszczególnych okrążeń">
      {#snippet head()}
