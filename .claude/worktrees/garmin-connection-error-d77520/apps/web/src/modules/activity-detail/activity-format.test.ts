...
describe('benefitLabel', () => {
  it('translates Garmin primary-benefit enums', () => {
    expect(benefitLabel('TEMPO')).toBe('Tempo');
    expect(benefitLabel('aerobic_base')).toBe('Baza tlenowa');
  });

  it('prettifies an unknown enum instead of hiding it', () => {
    expect(benefitLabel('SOME_NEW_THING')).toBe('Some New Thing');
...
describe('splitLabel', () => {
  it('names Garmin split classes in Polish', () => {
    expect(splitLabel('RWD_RUN')).toBe('Bieg');
    expect(splitLabel('RWD_WALK')).toBe('Marsz');
    expect(splitLabel('RWD_STAND')).toBe('Postój');
    expect(splitLabel(undefined)).toBe('Odcinek');
  });
});
...
describe('similarDeltaBadge (spec 065)', () => {
  ...
  it('reads the delta from TODAY, not from the older session', () => {
    ...
    const b = similarDeltaBadge({ abs: 13, pct: 5 }, metric('pace'))!;
  ...
  it('separates direction from the arrow: less power points down and is worse', () => {
    const b = similarDeltaBadge({ abs: 62, pct: 20 }, metric('power'))!;
    expect(b.arrow).toBe('down');
    expect(b.direction).toBe('worse');
  ...
  it('treats a sub-percent difference as unchanged rather than a tiny win', () => {
    const b = similarDeltaBadge({ abs: 1, pct: 0.2 }, metric('hr'))!;
  ...
  it('renders no badge at all when there is nothing to compare', () => {
    expect(similarDeltaBadge({ abs: null, pct: null }, metric('hr'))).toBeNull();
    expect(similarDeltaBadge({ abs: 5, pct: null }, metric('hr'))).toBeNull();
  });
});
