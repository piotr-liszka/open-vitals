<script lang="ts">
  ...
  let { streams, sport }: { streams: ActivityStreams; sport: SportGroup } = $props();
  ...
  const groups = $derived(
    (['effort', 'terrain', 'physiology', 'dynamics'] as ChartGroupKey[])
      .map((key) => ({ key, title: chartGroupTitle(key), charts: set.charts.filter((c) => c.group === key) }))
  ...
  const activeSecondary = $derived(
    activeDistance === null ? undefined : `${fmtNum(activeDistance / 1000, 2)} km`
  );
...
<div class="panel">
  <header class="head">
    ...
    <div class="readout" aria-live="polite">
      {#if active === null}
        <p class="hint">
          Najedź na dowolny wykres — ten sam moment zaznaczy się na wszystkich. Kliknięcie przypina go na
          stałe.
        </p>
      {:else}
