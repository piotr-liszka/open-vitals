...
describe('formatting', () => {
  it('renders Polish calendar days', () => {
    expect(formatDay('2026-08-03', 'short')).toBe('3 sie');
    expect(formatDay('2026-08-03', 'shortYear')).toBe('3 sie 2026');
    expect(formatDay('2026-08-03', 'long')).toBe('3 sierpnia');
    expect(formatDay('2026-08-03', 'longYear')).toBe('3 sierpnia 2026');
    expect(formatDay('2026-08-03', 'numeric')).toBe('3.08.2026');
    expect(formatDay('2026-08-03', 'iso')).toBe('2026-08-03');
    expect(formatDay('2026-08-03')).toBe('3 sie');
  });

  it('matches the month abbreviations the hand-rolled formatters used', () => {
    const months = ['sty', 'lut', 'mar', 'kwi', 'maj', 'cze', 'lip', 'sie', 'wrz', 'paź', 'lis', 'gru'];
    months.forEach((abbr, i) => {
      expect(formatDay(`2026-${String(i + 1).padStart(2, '0')}-15`, 'short')).toBe(`15 ${abbr}`);
    });
  });

  it('never drifts a day when formatting (first/last of month)', () => {
    expect(formatDay('2026-01-01', 'short')).toBe('1 sty');
    expect(formatDay('2026-12-31', 'short')).toBe('31 gru');
  });

  it('rejects non-day-keys', () => {
    expect(() => formatDay('2026-08-03T10:00:00Z')).toThrow(InvalidDayKeyError);
  });

  it('renders instants in an explicit timezone', () => {
    const iso = '2026-08-03T14:05:09Z'; // 16:05:09 in Warsaw (UTC+2)
    expect(formatInstant(iso, 'time')).toBe('16:05');
    expect(formatInstant(iso, 'timeSeconds')).toBe('16:05:09');
    expect(formatInstant(iso, 'date')).toBe('3 sie 2026');
    expect(formatInstant(iso, 'numeric')).toBe('3.08.2026');
    expect(formatInstant(iso, 'dateTime')).toBe('3 sie 2026, 16:05');
    expect(formatInstant(iso, 'time', 'UTC')).toBe('14:05');
  });

  it('formats a Date the same as its ISO string', () => {
    const iso = '2026-08-03T14:05:09Z';
    expect(formatInstant(new Date(iso), 'dateTime')).toBe(formatInstant(iso, 'dateTime'));
  });

  it('degrades to an empty string for unparseable instants', () => {
    expect(formatInstant('not-a-date', 'time')).toBe('');
    expect(formatInstant(new Date(NaN))).toBe('');
  });

  it('crosses midnight correctly when the zone shifts the day', () => {
    expect(formatInstant('2026-08-03T22:30:00Z', 'date')).toBe('4 sie 2026');
    expect(formatInstant('2026-08-03T22:30:00Z', 'date', 'UTC')).toBe('3 sie 2026');
  });
});
...
describe('calendar months (spec 037)', () => {
  ...
  it('throws a typed error rather than guessing at a bad month key', () => {
    ...
    expect(() => formatMonth('nope')).toThrow(InvalidMonthKeyError);
  });
  ...
  it('renders months in Polish, identically on server and client (UTC-pinned)', () => {
    expect(formatMonth('2026-08')).toBe('sie');
    expect(formatMonth('2026-08', 'long')).toBe('sierpień');
    expect(formatMonth('2026-08', 'shortYear')).toBe('sie 2026');
    expect(formatMonth('2026-01', 'longYear')).toBe('styczeń 2026');
  });
});
