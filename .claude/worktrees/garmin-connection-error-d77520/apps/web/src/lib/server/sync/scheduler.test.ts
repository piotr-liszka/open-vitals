...
describe('runScheduledSync', () => {
  it('runs one change-gated incremental sync per user and counts failures', async () => {
    ...
    expect(res).toEqual({ attempted: 3, skipped: 0, failed: 1, optedOut: 0, sessionsSwept: 0 });
  });
  ...
  it('counts a fast-returned tick as skipped, not as work (spec 027)', async () => {
    ...
    expect(res).toEqual({ attempted: 2, skipped: 1, failed: 0, optedOut: 0, sessionsSwept: 0 });
  });
  ...
  // Spec 071: "automatyczne pobieranie danych" is a per-user switch, and the tick is the only thing
  // that reads it. A user who turned it off must not be touched at all — not attempted-and-skipped,
  // which is what a fast-returning probe reports, but never asked in the first place.
  it('never syncs a user whose auto-sync switch is off, and counts them separately', async () => {
    const seen: string[] = [];
    const syncEngine = engine(async (userId) => {
      seen.push(userId);
      return run('succeeded');
    });
    const res = await runScheduledSync({
      users: { listIds: async () => ['on', 'off'] },
      syncEngine,
      logger: nullLogger,
      autoSyncEnabledFor: async (userId) => userId !== 'off'
    });

    expect(seen).toEqual(['on']);
    expect(res).toEqual({ attempted: 2, skipped: 0, failed: 0, optedOut: 1, sessionsSwept: 0 });
  });

  it('syncs everyone when no switch service is wired (the pre-071 behaviour)', async () => {
    const seen: string[] = [];
    const syncEngine = engine(async (userId) => {
      seen.push(userId);
      return run('succeeded');
  ...
  it('isolates a thrown per-user error', async () => {
    ...
    expect(res).toEqual({ attempted: 1, skipped: 0, failed: 1, optedOut: 0, sessionsSwept: 0 });
  });
  ...
  describe('expired-session sweep (spec 055)', () => {
    ...
    it('does not let a failed sweep cost anyone their sync', async () => {
      ...
      expect(seen).toEqual(['a', 'b']);
      expect(res).toEqual({ attempted: 2, skipped: 0, failed: 0, optedOut: 0, sessionsSwept: 0 });
    });
  });
});
